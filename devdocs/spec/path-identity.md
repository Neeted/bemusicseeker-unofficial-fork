...
| --- | --- | --- |
| `R2-EXACT-REMOVE` | 明示された旧exact keyだけをDB / storage / owned / packageから除去し、別exact rowを巻き込まない | `BmsLibraryStateApplierTests.ApplyLibraryMutationDelta_ExactRemovalKeepsOtherRowsAndPackages`; [CatalogMutationOwnerTests.ApplyCatalogMutation_CaseVariantPathCollisionRemovesOldExactRow](../../BeMusicSeeker.Tests/CatalogMutationOwnerTests.cs) |
...
| `R2-EXACT-UPSERT` | 同じexact keyだけを置換し、case-only / dot別表記のowner・保存値・hash所持を保持する | [CatalogMutationOwnerTests.ApplyInstalledTargetUpsert_PreservesEveryExactKey](../../BeMusicSeeker.Tests/CatalogMutationOwnerTests.cs); [OwnedChartCollectionInstalledOverlayTests.ApplyInstalledChartStorageTargets_BuiltLookupUpsertUsesOwnedPathExactView](../../BeMusicSeeker.Tests/OwnedChartCollectionInstalledOverlayTests.cs); `OwnedChartCollectionProjectionTests.FromStorageRows_FiltersPathlessMd5lessAndExactDuplicateRows` |
| `R2-PROTECT` | 通常moveの確定destination exact keyだけを保護し、別exact cleanup keyを過剰保護しない | [CatalogMutationOwnerTests.ApplyCatalogMutation_PathCleanupDoesNotRemoveRelocatedDestination](../../BeMusicSeeker.Tests/CatalogMutationOwnerTests.cs); `BmsLibraryStateApplierTests.ApplyLibraryMutationDelta_PathCleanupDoesNotPruneRelocatedDestination` |
