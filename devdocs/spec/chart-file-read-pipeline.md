...
path 集合の比較、stale 削除、保存値の relink は [path-identity.md](path-identity.md) を正本とする。case-only も通常の path 差分も、旧 exact key の削除と現在の exact path の追加・更新で収束させる。行集合と保存値引継ぎの判定は混ぜず、不完全走査・空走査保護等の範囲条件は [file-db-consistency.md](file-db-consistency.md) に従う。
...
file mutationの受付に使うcatalog path convergenceは、`FileScanFailures`が0件であることではなく、scan surfaceがauthoritativeで、そのsurfaceから作ったexact path diffとcanonical storage replacementが正常に完了したことを表す。個別譜面のread / lightweight parse failureはその譜面をcurrent catalog rowへ追加できなかった事実として集約するだけで、directory enumeration等のscan incompletenessとは区別する。したがってrecoverableな個別failureがあってもauthoritative file diff全体が正常に完了すればcatalog依存file mutation admissionを開いてよい。逆にscan surface自体がincomplete、empty-scan protection、またはcatalog apply failureの場合は開かない。詳細なidentityと受付境界は[path identity](path-identity.md#r2a-test-map)を参照する。
...
削除 commit は row-by-row の `song.hash` lookup / orphan check ではなく、削除対象の旧 exact path を temp table に入れて集合 SQL で処理する。現在の path 表記への変換や case-insensitive な重複排除はしない。BMS は削除前に対象 MD5 を temp table へ退避し、`song` と `maintenance` を削除した後、残存 `song.hash` owner が無い MD5 だけ `chart_digest_map` から消す。bmson は `bmson_song` と `maintenance` を同じ chunk transaction で削除する。メモリ側の `NextFiles` 構築では削除 path を `HashSet` 化し、ルート削除やルート近傍 rename のような大量削除でも `currentFiles * deletedPaths` の線形探索に戻さない。
...
file diff DB commit chunk は transaction 範囲を分けるための単位である。post-parse が作った staging chunk は bounded input queue へ流し、commit aggregator が `DbCommitChunkSize` 単位の immutable DB chunk へ集約し、別の DB writer queue へ渡す。DB writer は chunk commit だけを担当する。post-parse の完了時点で snapshot bytes は不要になるため、file diff UI 進捗は post-parse worker が 1 譜面分の staging data を作った時点で進め、DB commit 完了は待たない。DB が長期的な bottleneck の場合は bounded queue で backpressure するが、commit 中も aggregator は input queue を消費し続けられる。writer task は chunk commit ごとに writable `song.db` connection を開き、commit 後に閉じる。`song_tbl_file_check_breakdown` は post-parse -> aggregator 側の `commit_queue_wait_ms` と、aggregator -> DB writer 側の `commit_writer_queue_wait_ms` を分けて出す。chunk log は `applyMs`、`schemaMs`、`bmsUpsertMs`、`maintenanceUpsertMs`、`chartInfoMs`、`sqliteCommitMs` などを出し、summary には `db_commit_*_ms` として累積する。BMS `song` / `chart_digest_map` と `maintenance` の commit は row-by-row upsert ではなく temp table を使う。

`maintenance` の temp table は `path TEXT PRIMARY KEY` の exact key を使い、同じ exact path に対してだけ last-write-wins に畳む。実 table へも exact match による既存 row update + missing row insert として反映する。case-only の別 path は別行であり、`COLLATE NOCASE` で統合しない。

moved-hash relink は削除候補の BMS user song columns を file diff 開始時に旧 exact key で snapshot し、同一 MD5 の旧候補・新規候補が一対一の場合に DB commit 後の `song_tbl_file_check user_column_restore_*` で exact destination へ復元する。候補の一意性は file diff 全体で判定し、worker 完了順や commit chunk ごとに確定しない。対象列・既存 destination の保護・曖昧な候補の扱いは [relink 規則](path-identity.md#relink-policy) に従う。現行コードには case-only 除外が残っており、その解除は [ライブラリ変更計画](../plan/BeMusicSeeker-library-mutation-performance.md) の RELINK-1 で行う。maintenance は relink する保存列に含めず、現在の入力から評価する。
