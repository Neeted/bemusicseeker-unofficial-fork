...
moved-hash relink は削除候補の BMS user song columns を file diff 開始時に旧 exact key で snapshot し、同一 MD5 の旧候補・新規候補が一対一の場合に DB commit 後の `song_tbl_file_check user_column_restore_*` で exact destination へ復元する。通常差分と case-only 差分を同じ候補規則で扱い、候補の一意性は file diff 全体で判定して worker 完了順や commit chunk ごとに確定しない。対象列・既存 destination の保護・曖昧な候補の扱いは [relink 規則](path-identity.md#relink-policy) に従う。maintenance は relink する保存列に含めず、現在の入力から評価する。実装とテストの対応は [RELINK-1 対応表](path-identity.md#relink-implementation-map) を参照する。
