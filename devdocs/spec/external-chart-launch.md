...
The Settings dialog exposes this aggregate as a dialog-local draft under the `Right-click settings` category. Opening the category only parses a snapshot; editing a field, reordering an action, or choosing an executable does not mutate `Settings.RightClickActionsJson`. Save validates and serializes the complete aggregate once, then lets the existing settings edit session persist it. Cancel and native close discard the child draft. The page labels the two lists `Open web pages` and `Open with a program`; `Restore defaults` is always available, replaces only the draft, and becomes persistent only when Save succeeds. When the persisted value is invalid, the page continues to show a localized settings error until the draft is restored or replaced with another valid draft. Raw parser details remain internal diagnostics. The executable picker accepts `*.exe` and all files, and fills a blank program name from the accepted basename without overwriting a user name.
...
Every standard-library context menu, missing-chart context menu, and play-history context menu first selects exactly one chart context row and resolves it through this contract. A multi-selection or a mixed-kind aggregate must not be passed to the resolver. Rows without the required digest expose no corresponding web action; a play-history row may still expose hash-only web actions when its raw MD5/SHA-256 is valid. No hash-only or missing-chart row exposes a program action. A play-history row resolved to a local chart also exposes the existing associated-open command, with the `Open with program` submenu inserted immediately after it.
...
At menu-open time the MainWindow owner resolves the current settings and inserts the enabled web actions in persisted order. A local owned chart gets an `Open with program` submenu immediately after the existing associated-open command; each child carries the stable action ID and action kind rather than a captured row or URL. Play-history rows that resolve to a local chart use the same associated-open terminal and receive the `Open with program` submenu directly after it; hash-only play-history rows expose Web actions only. Missing playlist entries never get that submenu. On click, the exact context row is recovered from the context menu, the current settings are read again, and the action is re-resolved before it is sent to a gateway. A stale action, changed settings, invalid settings, missing chart, missing executable, or gateway failure produces no launch and a localized terminal error.
...
| Restore defaults is always enabled and routed click does not confirm, including a valid draft | `SettingsWindowPresentationTests.SettingsWindow_RightClickRestoreDefaultsIsEnabledForValidDraftAndDoesNotPrompt` | extend shared WPF fixture through `TestUiDispatcherHost`; assert enabled control, canonical draft, and zero confirmation requests |
| Restore defaults remains draft-only across Cancel and persists exactly once on Save | `SettingsDialogBehaviorTests.RightClickRestoreDefaultsIsDraftOnlyUntilSave` | extend pure settings edit-session fixture; assert backing JSON is unchanged before Save, restored by Cancel, and saved once by Save |
| Restore defaults draft is discarded by native close | `SettingsWindowPresentationTests.SettingsWindow_RightClickRestoreDefaultsIsDiscardedByNativeClose` | extend shared WPF fixture through `TestUiDispatcherHost`; assert raw JSON remains unchanged after lifecycle completion without fixed waits |
