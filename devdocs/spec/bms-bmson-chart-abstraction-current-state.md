...
| owned chart collection | `OwnedChartCollectionState` | `BMSLibrary` 内の所持譜面 `ChartFile` identity collection。BMS / bmson とも current installed source としては path と md5 を必須とし、canonical full path は BMS / bmson をまたいで app-wide unique とする。storage owner を保持し、snapshot 作成時に owner の現在値を再投影する。chart-common な snapshot / lookup / mutation の primary source として使い、永続化 owner や BMS-only / bmson-only producer は storage row collection に残す。 |
| pathless owned chart | なし | path を持たない chart は current installed source ではない。DB load / external replacement 由来の pathless storage row は owned collection 入口で除外し、internal mutation が pathless chart を owned collection に追加しようとする場合は invariant violation として扱う。 |
| md5less owned chart | なし | md5 を持たない chart は current installed source ではない。DB load / external replacement 由来の md5less storage row は owned collection 入口で除外し、install / scan / internal upsert が md5less chart を追加しようとする場合は invariant violation として扱う。sha256 は digest / chart_info / playlist sha-only entry 用の補助 identity であり、owned installed / duplicate / install estimation の fallback identity にはしない。 |
| duplicate-path owned chart | なし | 同一 canonical path を持つ複数 chart は current installed source ではない。DB load / external replacement 由来の duplicate path storage row は BMS rows、bmson rows の入力順で first-wins とし、後続 row を除外して skip count を log に出す。internal add / upsert / path change が duplicate path を作ろうとする場合は invariant violation として検出する。 |
| pathless projection chart | `ChartFile` projection | playlist metadata / projection-only / repair fallback など、所持判定や file operation を行わない一時 projection。low-level projection helper では pathless projection を許容するが、installed lookup / resource maintenance / playlist owned resolve / folder operation には入れない。 |
| owned chart digest update event | `LibraryChartDigestChange` | chart_info inline / backfill が storage owner の MD5 / SHA-256 を実ファイル digest に同期した時の差分 event。BMS missing SHA-256 補完も含む。factory は pathless / md5less projection から owned event を作らず、explicit owned event は path + md5 必須として検出する。maintenance は producer ではない。 |
...
通常 library root / folder view は virtual view を正規経路にする。`RegularChartListOwner.ApplyRegularView(...)` は root / folder / keyword / mode / sort 更新を受け、通常 library で `ChartListOrder` に未登録の sort column が指定された場合は診断ログを残して default title sort へ戻す。`CustomTableColumn.SortMemberPath` と `ChartListOrder` の対応は test で検証し、sortable に見える通常 library 列が virtual sort 未対応のまま残らないようにする。default virtual または subset virtual が必須の mode で source を構築できない場合は full `ChartFile` list materialize へ fallback して隠さず、empty regular view に落として異常をログで見える状態にする。
...
duplicate view は `DuplicateChartGroups` / `SearchDuplicateChartGroups()` を入口にし、現行 snapshot は owned collection の duplicate row adjacent index から取得する。初回 search で current owned chart の storage owner identity を軽量 `DuplicateChartRow` へ投影し、以後は remove / path change / upsert / MD5 digest update の collection mutation に同期する。検索 API は adjacent index の immutable snapshot を返し、mutation 側は copy-on-write で snapshot を差し替えるため、per-call に全 row list を複製しない。ここは full duplicate search という明示的な全件 operation だが、`BMSFiles` / `BmsonSongs` を caller 側で束ねず、行 identity snapshot を owned collection 側に保持する。install destination runtime overlay は duplicate 判定に混ぜない。通常 merge の existing hash lookup は installed lookup index を使う。

duplicate search の結果 property は `DuplicateChartGroups` のまま残すが、cache invalidation signal とは分ける。library mutation / dispatcher が duplicate cache を dirty にする時は `DuplicateChartGroupsInvalidationVersion` を進め、ViewModel はその version を受けて duplicate tree と duplicate view を更新する。`DuplicateChartGroups` property change は search 結果そのものが置き換わった時の通知であり、cache を null にしたことを伝える汎用 invalidation signal としては使わない。これにより、結果 data property と mutation side-effect の境界を分け、将来 refresh effect queue へ寄せる時も duplicate full search を明示 operation として保てる。

`DuplicateChartRow` は grouping / duplicate 判定用の path / md5 lookup hash と storage owner reference を持つ immutable row である。row collection は immutable snapshot として扱い、owned collection mutation 時は copy-on-write で snapshot ごと差し替える。storage owner reference は warning clear と fresh projection のため live owner を指すが、row の path / md5 identity は mutation 時に新 row へ置き換えるため、分析中の row collection 自体は変化しない。storage owner row の `CreateChart()` は `Chart` field へ代入せず、呼び出しごとに fresh `ChartFile` projection を返す。full search の row snapshot 段階では BMS / bmson storage owner と md5 / path だけを保持し、duplicate group へ入る row だけ `CreateChart()` で `ChartFile` 化する。sha256-only projection は duplicate grouping に使わない。`BmsLibraryDuplicateService` は snapshot builder ではなく analyze / BMS duplicate warning apply の責務に閉じる。BMS duplicate row の storage owner は row の `BmsFile` または materialized chart の `GetBmsStorageOwner()` から読む。bmson duplicate row は `BmsonSong` または `ChartFile.GetBmsonStorageOwner()` で storage owner へ降りる。bmson duplicate row は `PendingChartEntry` compatibility adapter を materialize しない。duplicate warning clear は owned duplicate snapshot が保持する path + md5 を満たす BMS storage rows に対して行う。pathless / md5less storage row は owned collection 入口で current installed source から除外されるため、duplicate full search の warning clear 対象にもならない。duplicate subset の仮想一覧表示は `DuplicateGroup.ChartFiles` をそのまま `ChartListSourceRow` へ渡すため、projection-only warning も落とさない。
...
### Right-click external actions

Web links and configured external programs use the same `RightClickActionResolutionInput` for BMS and bmson. The MainWindow creates that immutable input from the exact context-menu row, including the chart kind and any MD5/SHA-256 available from the chart or `ChartInfo`; it does not aggregate the current table selection. BMS-IR is BMS-only, while the other built-in web definitions are available to both kinds when their required digest exists. A missing playlist entry can contribute hashes but never a local program path. A play-history row can contribute a valid hash-only web context even when the library chart is unresolved; it cannot contribute a program path until a local owned chart is resolved.

The resolver preserves enabled settings order and filters by digest capability. At click time the owner re-resolves the current exact row and settings before sending a web request to the shell gateway or a local-file request to the one-shot program gateway. Program launch uses the local chart path only; it does not infer a path from a hash, playlist URL, or storage-row compatibility adapter. This keeps BMS/bmson capability policy at the chart-native `ChartOperationTarget` boundary and avoids reintroducing a BMS-only adapter for external actions.

`RepairInstalledLocation` は BMS / bmson の両方に付与される。UI handler は `ChartOperationTarget` を ViewModel に渡し、ViewModel が UI thread 上で repair 用 snapshot を作る。snapshot の `HasTargets` は `ChartFile` だけで判定し、context menu の表示可否確認だけでは bmson compatibility adapter を materialize しない。search / clear の実行時も、UI handler は background task に入る前に `MaterializeRepairEntries()` で snapshot の `RepairEntries` を確定させるが、`RepairEntries` は `PackageEntry` または `ChartFile` projection から作る `PackageChartEntry` の集合であり、bmson loose target の lazy compatibility provider は呼ばない。BMS は storage row、bmson は operation target 作成時点の `ChartFile` snapshot を修復 payload として扱う。model facade と install estimation service は `PackageChartEntry` を対象に search / clear を行う。一方、fix 実行は `RepairCharts` を model 層へ渡す。`RepairCharts` は基本的に snapshot の `ChartFile` をそのまま使うが、playlist 詳細 row などで snapshot の `ChartFile` が古い導入先状態を持つ場合に備え、現在の `RepairEntries` に install destination / metadata / suggestions があればそれを `ChartFile` に重ねる。model 層は `PackageChartEntry.FromChart(...)` と `ChartFile.InstallDestination` から移動対象を組み立てるため、fix の mutation 入口は chart-native である。bmson 修復候補は `ChartFile` に投影済みの state を見るため、background task が shared adapter cache を初期化しない。model 側の修復では、BMS / bmson の path 更新をどちらも `LibraryCatalogMutationFacts.ChartPathChanges` として返し、`BmsLibraryStateApplier` が BMS storage row と `bmson_song` に分けて DB row を更新する。修復後の maintenance 再計算対象は BMS / bmson とも `LibraryFixInstallationResult.MaintenanceCharts` の `ChartFile` として返し、BMSFile-only target list へは戻さない。

resource health と folder auto rename の UI handler は `ChartOperationTarget` を top-level request DTO へ変換してから ViewModel に渡す。resource health の強制再チェックと warning ignore / unignore は `ChartResourceHealthRequest`、folder auto rename は `ChartFolderAutoRenameRequest` を使い、ViewModel から `ChartFile` のまま model 層へ渡すため、UI / ViewModel 側では bmson compatibility adapter を materialize しない。model 層では `BmsLibraryMaintenanceService` が `ChartFile` を受け、service 内部で BMS storage row と `bmson_song` に分け、resource health projection / ignore state は `ChartFile` から解決する。folder auto rename も request 内の `ChartFile` を使い、plan 生成では bmson compatibility adapter を materialize しない。UI handler 側に BMSFile adapter 選択 helper は残さない。installed package record clear は package identity を落とさないため、`ChartOperationTarget` をそのまま ViewModel に渡して `ChartPackage` に解決する。
...
ViewModel / UI 層の pending package 操作は `SearchInstallDestinationForPendingPackages` / `SearchPendingInstallDestination`, `ForceInstallPendingPackages` / `InstallPendingCharts`, `RemovePendingPackages` / `RemovePendingPackagesAll`, `DeleteInstallPackageRecords`, `ClearPendingInstallDestination`, `SetPendingInstallDestination`, `GetPendingPackagesContainingOnlyInstalledCharts`, `DeletePendingPackageSources` を入口にする。これらは package 内 chart を扱う操作であり、BMS 専用 API ではない。UI から選択 chart を渡す経路は `ChartOperationTarget` payload から top-level request DTO を作り、package 所属 target は `PackageChartEntry` identity で package operation へ展開する。search / merge / clear の batch 操作は `PendingInstallDestinationSearchRequest` / `PendingInstallDestinationClearRequest` を作り、package target は source `PackageChartEntry` を保持し、background task 側では package target identity を現行 `ChartPackagesPending` へ再解決する。package 外 target は request の `LooseEntries` として `ChartOperationTarget.ToPackageChartEntry()` から確定するが、bmson では lazy compatibility provider を呼ばず `ChartFile` projection から一時 entry を作る。導入先セル手動編集は `PendingInstallDestinationEditRequest` を作り、loose bmson compatibility adapter を materialize しない。`LibraryChartRow.instl_dst` は表示用 getter のみで、セル編集の書き戻しには使わない。`PackageEntry` があれば adapterless bmson も package-level API に降り、compatibility adapter を作らず entry へ書き戻す。package 外 target の mutable install-destination state は operation target 作成時点の `ChartFile` snapshot にある state だけを使い、後段処理のために shared adapter を作って書き戻す旧挙動は残さない。
...
pending install destination クリアの UI 経路も、pending section では選択 `ChartOperationTarget` を `PendingInstallDestinationClearRequest` に変換してから `MainWindowViewModel.ClearPendingInstallDestination(...)` へ渡す。package row から作られた `LibraryChartRow` / `ChartOperationTarget` は source `PackageChartEntry` を保持し、package に属する chart は entry identity で `ChartPackage` を解決して `PackageChartEntry.ClearInstallDestination()` を呼ぶため、adapterless bmson package entry のクリアでは compatibility adapter を materialize しない。package 外に残った target は fallback として request の `LooseEntries` に確定し、`ChartOperationTarget.ToPackageChartEntry()` で BMS storage owner だけを adapter entry 化し、bmson は `ChartFile` projection から entry 化して ViewModel 側で model の clear API へ渡す。`ToPackageChartEntry()` 自体は package entry があればそれを最優先するが、pending batch request では package target を先に分離する。

pending install destination search / merge search の UI 経路も、pending section では選択 `ChartOperationTarget` を `PendingInstallDestinationSearchRequest` に変換してから ViewModel に渡す。package row 由来 target は `PackageChartEntry` identity で package に展開し、package-level estimation / merge search を走らせる。package 外に残った target は background task 前に request の `LooseEntries` として確定し、ViewModel 側で model 層へ渡す。この fallback は loose chart として処理し、再び path で pending package に吸い込まない。model の manual estimate / merge / clear 入口も loose chart については `PackageChartEntry` を受け、旧 `BMSFile` batch overload は残さない。adapterless bmson package entry の search / merge のためだけに compatibility adapter を materialize しない。
...
merge 先探索や installed-only package destination resolve は package 内 chart を `PackageChartEntry` として列挙し、BMS / bmson 共通の installed hash index で既所持 directory を採点する。owned installed 判定と directory scoring の primary key は md5 であり、sha256 は directory snapshot / diagnostics / playlist sha-only entry 解決の補助 bucket として保持する。md5 がない package chart は installed / duplicate / install estimation の fallback identity として sha256 lookup しない。
...
library folder operation は root/search-directory の public / user-facing 名に `BMSDirectory` が残るが、chart 行に対する folder 操作は `RenameChartFolder(...)` / `MergeChartDirectory(...)` / `AutoRenameChartFolders(...)` / `AutoRenameAllChartFolders(...)` に寄せる。`AutoRenameAllChartFolders(...)` の対象抽出は ViewModel で `BMSFiles` / `BmsonSongs` を直接結合せず、BMSLibrary の owned real path directory view から source folder list を作る。root 除外 / nested skip / collision 判定後に actionable plan がある場合だけ UI 側は再生停止と sort invalidation に進む。metadata 生成に必要な chart snapshot は target folder の direct child bucket だけを materialize する。`BuildFolderMoveFacts(...)` は `LibraryChartRef` snapshot を受け、path 更新 payload は `LibraryCatalogMutationFacts.ChartPathChanges`、unregister payload は owner-backed `ChartRemoveRequests` として保持する。folder move / merge path では BMS / bmson path と installed package / pending package の install destination も合わせて更新対象になる。root folder move の UI 経路は `ChartLibraryMoveRequest` を `MoveLibraryCharts(...)` に渡してから `MoveLibraryRootFolder(...)` に入り、request 内の `LibraryChartRef` を通して BMS / bmson chart を扱う。旧 `MoveBMSRootFolder(...)` wrapper は production 参照がなく、テストだけの旧名互換 API になっていたため削除済みである。`BMSDirectory` 系 UI / settings vocabulary は BMSFile-based root-folder / LR2 search-root 境界として残っている。
...
auto install workflow の pending / auto-install 分類では、resource reference count は `PackageChartEntry.ResourceSnapshot` を先に読み、SingleBmsFile / SingleBmsonFile / resource health warning は対象 `PackageChartEntry` に書き戻す。installed 判定は `ChartFile` callback で行い、AlreadyInstalled warning も entry state として保持する。resource health は package 代表 entry だけではなく、resource reference を持つ全 entry に pending projection として書き戻す。BMS entry は resource snapshot 更新のために `ChartFile.GetBmsStorageOwner()` の parser / cache API を通ることがあるが、bmson entry は warning 書き戻しのためだけに compatibility adapter を materialize しない。
...
install table load result は pending warning 初期化の対象 adapter list を公開しない。warning count と package / stale row の結果だけを返し、pending warning 初期化は `PackageChartEntry` を走査して entry の warning state へ直接書き戻す。起動時 pending package の installed 判定は `ChartFile` callback で行い、AlreadyInstalled / SingleFile / ResourceHealth warning も `PackageChartEntry` の warning state として保持する。ResourceHealth warning 判定と health 表示 projection は resource reference を持つ全 entry に対して行い、warning 初期化のためだけに adapterless bmson entry を materialize しない。
pending package tree の「導入先を開く」は、package 内 chart の `ChartFile.InstallDestination` と md5 identity を読む。これは explorer を開く先を解決するだけの UI 読み取りなので、adapterless bmson entry を `BMSFile` に materialize しない。pending package の install destination clear は `PackageChartEntry` の install destination state を clear し、adapterless bmson entry も entry 内 state と `ChartFile` projection を更新するため、clear だけでは compatibility adapter を materialize しない。
...
pending / newly installed package の一覧表示 row は `PackageChartEntry` から作る。virtual package subset は `RegularChartListOwner.CreatePackageEntrySnapshot(...)` で `IReadOnlyList<PackageChartEntry>` を捕捉し、`ChartListSourceRow.BuildPackageRows(IEnumerable<PackageChartEntry>)` へ渡す。`ChartListSourceRow` は package entry を live provider として保持するため、導入先推定や resource health projection が background で entry に反映された後も、画面遷移で snapshot を作り直さなくても `INSTL DST` / WARNING / WAV/BGA/MOVIE health を読み直せる。virtual subset の source row signature には `PackageChartEntry.ProjectionVersion` を含め、pending projection 変更が sort / cache reuse に埋もれないようにする。BMS / bmson の storage row 分割は virtual package entry snapshot には持たせないため、package view の sort / keyword filter に入るだけでは adapterless bmson entry を materialize しない。
...
split した pending package の regroup 判定は `PackageChartEntry.Chart` の path / md5 identity / install destination を読む。全 entry が同じ expected destination に解決できることを確認した後、regrouped package は `PackageChartEntry` list として書き戻す。regroup 後の warning 再初期化も起動時 pending warning 初期化と同じく `PackageChartEntry` を走査し、resource health projection は BMS / bmson とも `ApplyPendingResourceHealthProjection(PackageChartEntry)` が一時 maintenance snapshot から warning と health 表示値を entry へ書き戻す。resource health は package 代表ではなく全 entry の pending projection として復元する。bmson も AlreadyInstalled / SingleBmsonFile / ResourceHealth / nested chart warning を entry state として保持し、warning 再初期化のためだけに adapterless bmson entry を materialize しない。
...
target chart list は `PackageChartEntry` として snapshot builder に渡され、読み取り専用で参照する代表譜面を `RepresentativeChart` として `ChartFile` 化する。pending package の通常推定では `MissingEntries` を直接渡すため、missing target を snapshot 化するだけなら追加の adapter 再解決は不要である。package-level target が空の場合は package の `ChartEntries` 全体を使うため、adapterless bmson entry も snapshot から落ちない。package-level pending estimation の request / batch state は `PackageEntries` / `MissingEntries` / `AlreadyInstalledEntries` だけを保持し、mutation writeback も entry を入口にする。install / move 実行のような実ファイル操作でも、対象は `PackageChartEntry` / `ChartFile` を正本にし、BMS-only parser / storage mutation が必要な時だけ `ChartFile.GetBmsStorageOwner()` へ明示的に降りる。`DefinedResources` は `PackageChartEntry.Chart` から作るため、bmson pending chart では `PendingChartEntry` adapter ではなく `bmson_song` の resource refs を使う。metadata profile は `ChartFile` projection の title / artist / path から作る。bmson pending chart では `Kind=Bmson` と `BmsonSong` owner を保持し、BMS 専用 storage owner とは分ける。

source surface ownership は chart format ではなく package kind で決まる。BMS / bmson のいずれでも、directory package は package root を bounded scan し、`BundledResources` / `SourceCandidateResources`、scan metrics、batch source surface を構築してdirectory root単位で共有する。single-file package と loose chart は親 directory を列挙せず、`BundledResources` と `SourceCandidateResources` は独立した空 entry、scan metrics は0、backend は空、limit exceeded は false とする。file / loose でも `SourceDirectory`、`DefinedResources`、`RepresentativeChart`、metadata profile は保持する。relative-path resource は `ChartResourceSnapshot` の path-aware keyとして保持し、recursive scanのtriggerにはしない。background batchのroot、shared surface、fallback surface、source baseline prefilterはdirectory package rootだけを対象にし、file packageがdirectory packageと同じ親を指してもsurfaceを共有しない。

loose chart 推定の model 入口は `PackageChartEntry` を受け取り、snapshot も entry から組み立てる。installed hash での除外や既に install destination が入っている対象の skip 判定は md5 identity / `ChartFile.InstallDestination` を読む。UI から standalone chart target を渡す場合も、ViewModel が background task 前に `ChartFile` を確定し、`PackageChartEntry` に包んでから model 層へ渡す。manual destination validation の standalone 所持判定は owned collection の owner/path canonical lookup を使い、owner-backed exact match と kind-aware path exact lookup で判定する。ここで `BMSFiles` / `BmsonSongs` を都度結合して全件走査しない。BMS-only parser / storage owner が必要な場合だけ `ChartFile.GetBmsStorageOwner()` へ降り、bmson loose target のために compatibility adapter を作らない。
...
- `BmsLibraryMaintenanceService.UpdateMaintenanceInfo(...)` は `ChartFile` を入力として受ける。service 内部で BMS storage owner と `bmson_song` に分けるが、resource health の defined / existing count は BMS / bmson とも `ChartResourceSnapshot` / `ChartFileProjection` から作る chart-common 計算を使う。BMS は同じ `maintenance` row に encoding / reload 結果も保持するため、resource health 計算結果を既存 `BMSFile.maintenanceInfo` へ merge し、encoding / `is_encoding_fixed` は BMS 専用 pipeline が更新する。maintenance は file bytes digest を再計算しないため、owned chart digest update event の producer ではない。
...
- resource health / folder auto rename の chart-domain 境界: `ChartResourceHealthRequest.Charts` / `ChartFolderAutoRenameRequest.Charts`
...
- pending package / pending chart の context menu batch 操作: `GetSelectedChartTargets(..., isPendingSection: true)` から `PendingInstallDestinationSearchRequest` / `PendingInstallDestinationClearRequest` を作り、background task へ渡す
- pending install destination のセル編集: `ChartOperationTarget` から `PendingInstallDestinationEditRequest` を作り、background task へ渡す
...
- owner/path canonical lookup: UI 入力や path-only ref を current owned chart ref へ戻す exact lookup。owner reference を優先し、次に kind + canonical full path で解決する。owned current source では canonical path が app-wide unique であるため複数候補は正常系ではなく、raw utility で非一意が見えた場合だけ unresolved / invariant violation として扱う。hash だけの解決は delete safety を広げるため入れない。
...
playlist summary owned hash snapshot は owned chart collection の lightweight hash index から作る。startup readiness 外の best-effort warmup 対象であり、snapshot 専用 invalidation version、owned collection version、storage rows version によって stale build result を publish しない。playlist reference apply は参照 map の selected key に一致する owned refs だけを current owner hash で抽出し、全 owned refs snapshot を毎回複製しない。playlist detail open 用の playlist library resolve index は、BMSLibrary が `PlaylistLibraryResolveIndexSnapshot` として md5 / sha256 -> representative `LibraryChartRef` を持つ cached snapshot を返し、ViewModel は `ResolveChartForPlaylistEntry(...)` 相当の model snapshot API を使う。代表選択は deterministic に path 昇順最小を採用し、entry selected key が md5 なら md5 map だけ、sha256 なら sha256 map だけを読む。pathless / md5less owned chart は playlist detail の owned 判定対象から除外する。BMSLibrary 側の cache は invalidation version、owned collection version、storage rows version で currentness を保証し、ViewModel 側には prewarm task と readiness 表示だけを残す。md5 / sha256 辞書構築、representative selection、cache publish は ViewModel に置かない。代表 `LibraryChartRef` は表示 / score / chart_info 用に immutable `ChartFile` identity snapshot を持ち、BMS player など BMS-only 操作に必要な owner 参照は playlist row の別フィールドへ渡す。chart_info / digest update window 中でも公開済み snapshot は owner の途中 digest を読まず、新規 build result だけ window 終了後に publish する。
...
- `LibraryChartRef` view: kind、path、owner、md5 identity だけで、folder move / delete / merge の canonical resolve に使う。
...
- owner/path canonical lookup: input chart を current library chart へ canonical resolve する。delete / repair / rename で使う。owner reference がある場合は owner match、path-only input は kind + canonical path の exact match に限定する。owned current source では canonical path が app-wide unique なので、複数候補は破損・不整合の検出対象であり正常な fallback 条件にはしない。
- hash/directory/path index: installed lookup、duplicate merge の existing hash、duplicate full search の duplicate row adjacent index、install estimation、resource-only merge display package に使う。owned collection の current path/md5 を source にし、`primaryHashCounts`、`primaryHash -> paths`、`md5 -> directories`、`sha256 -> directories`、`known chart directories` を差分更新する。primary hash は md5 identity で、sha256 は owned installed / duplicate / install estimation の fallback identity にはしない。`CreateExcludingLookup` はこの index の primary hash count から作り、merge source 自身を除外する時も full chart snapshot を作らない。duplicate full search は owned collection が cached する md5-based `DuplicateChartRow` を読み、remove / path change / upsert / MD5 digest update で同期される row identity を使う。group materialize までは全件 `ChartFile` rich projection を行わない。resource-only merge display は original package の md5 identity から candidate path を引き、destination 直下の candidate だけを path-only exact lookup で `PackageChartEntry` 化する。owned current source の path は unique なので、この materialize でも同一 path 複数 owner を正常系として保持しない。pending package や install destination overlay は owned current source ではないため、この uniqueness invariant とは別に扱う。
...
- storage owner view: maintenance hydration や installable maintenance 件数のように storage owner へ値を attach する producer 境界で使う。BMS / bmson owner list と owner path set だけを owned collection から返し、caller が `BMSFiles` / `BmsonSongs` を直接結合しない。実装上は `CreateStorageOwnerView()` のように rich snapshot と区別できる名前にし、resource refs が必要な場合は resource maintenance target view を使う。
...
mutation API は、owner-backed current mutation と explicit path cleanup を分ける。duplicate merge / 実ファイル削除 / 拡張子変更 / folder move unregister のような通常 operation は storage owner reference を持つ `OwnerReference` remove request を渡し、同じ path の別 owner を silent fallback で削除しない。path-only stale input や cleanup が必要な場合だけ `PathCleanup` request を明示し、kind + canonical path の exact match が 1 件に解決できる時だけ削除する。miss は no-op、複数候補は duplicate-path owned chart の invariant violation として扱う。
...
- 各 entry は `Kind`、path、directory、md5、sha256、storage owner、必要な runtime projection key を持つ。
- collection は stable identity として、kind + storage owner identity、path、md5 identity を扱える。
...
- collection は add / remove / move / upsert / digest update / storage owner replace を差分 mutation として受ける。
...
現在の `ApplyLibraryMutationFactsForFileMutationUnderExistingLease(...)` は、storage row mutation と owned collection mutation を行った後、`OwnedChartCollectionMutationResult` を `DispatchOwnedChartCollectionMutation(...)` に渡す。install upsert も `ChartStorageTargetSet` から追加 chart mutation result を作り、同じ dispatcher に流す。file scan diff は `SongTableFileCheckResult` の added / removed / next rows から file-scan 専用 mutation result を作り、隣接 index には差分 delta を dispatch する。ただし removed payload は owned collection が既に構築済みの場合だけ current owner identity view から作る。未構築時は removed payload 作成のために owned collection を build せず、必要な派生 index を full invalidate / dirty 扱いに落とす。file scan は `NextFiles` / `NextBmsonSongs` を storage rows の丸ごと置換として持つため、owned collection が既に構築済みの場合は next rows snapshot から owned collection 本体を置き換え、差分 mutation だけでは表現できない row order / no-op replacement を current 扱いにしない。duplicate merge は source unregister を `ApplyLibraryMutationFactsForFileMutationUnderExistingLease(...)`、final storage upsert を `ApplyInstalledChartStorageTargetsForFileMutation(...)` に通し、merge 中も owned collection / installed lookup を差分同期する。外部 full replacement は引き続き setter 境界で full invalidate する。

installed lookup、md5 primary hash -> path lookup、install destination overlay、parent folder cache、playlist summary owned hash、playlist detail resolve index、duplicate cache、resource health index は同じ mutation boundary から同期または無効化される。real path directory view と owner/path canonical lookup は owned collection の `LibraryChartRefIndexSnapshot` として mutation に追従する。`NormalLibraryRefreshNotification` は normal library refresh hint queue として、dispatcher result から `SourceChanged` / `InstallDestinationOverlayChanged` / `WarningPresentationChanged` / `MaintenancePresentationChanged` の effect、owned collection version、kind 別 storage row coverage を運ぶ。ViewModel はこの notification version range を消費し、source generation、source view refresh、overlay transient state、warning / maintenance / install destination / ref-table dependency generation、BMS row cache prune、bmson row cache sync の判定を setter 副作用から切り離している。remove-only の internal mutation では notification に removed BMS / bmson storage owner delta を載せ、通常一覧 row cache は全 owner view を取り直さず該当 row だけを prune する。add / path change / external replacement / digest update のように remove delta だけでは表現できない storage row 変更は従来どおり full sync fallback を使う。`NormalLibraryRefreshNotificationVersion` handler が source refresh と storage row cache sync の normal view 入口であり、`BMSFiles` / `BmsonSongs` property handler は normal view cleanup 用には登録しない。resource warning ignore / unignore、chart_info / zero-note producer 由来の warning 表示更新、manual resource maintenance producer 由来の warning / maintenance 表示更新、internal storage / digest mutation 由来の warning subset 表示更新は dispatcher 経由の effect に寄せる。duplicate / garbled hydration などの互換表示 refresh も dispatcher effect へ寄せ、外部 storage replacement だけ setter reset barrier で扱う。
...
- moved charts: old path と new path の対応が取れる chart。kind、old directory、new directory、old md5 identity、new md5 identity を保持する。
- digest updated charts: chart_info inline / backfill により storage owner の md5 / sha256 が変わった chart。path が同じでも hash index は更新する。projection-only の pathless / md5less digest update は owned event にしない。explicit owned digest event が path / md5 を欠く場合は invariant violation であり、full invalidate fallback で隠さない。
...
mutation result は rich `ChartFile` list を必須にしない。path / digest / owner / kind だけで足りる派生 index には lightweight entry を渡し、resource health や maintenance のように resource references が必要な index だけ subset `ChartFile` target を作る。

処理順は固定する。

1. mutation payload から必要な old-side lightweight entry / one-shot chart snapshot を作る。old path / old digest が必要な index のため、storage row を変更する前に取得する。
2. storage row owner を更新する。internal mutation は public setter ではなく BMSLibrary の core mutation helper を通すため、setter 由来の full invalidation / property notification は発生しない。`BmsLibraryStateApplier` は DB / installed package / pending package の applier 境界に限定し、`BMSFiles` / `BmsonSongs` collection writeback は BMSLibrary の mutation apply 内で行う。
...
owned collection の currentness は storage row collection version で判定する。外部 setter / DB reload は version を進めて full invalidate し、internal mutation は storage row owner 更新と owned collection mutation を同じ boundary で行った後、owned collection に処理済み version を記録する。したがって owned view / index 取得のたびに `BMSFiles` / `BmsonSongs` と owned collection を全件照合しない。storage row collection の参照と version は同一 lock 境界で capture / update し、incremental mutation は適用前 version と owned collection の記録 version が一致する場合だけ適用する。処理済み version を記録できるのは、owned mutation が storage rows の同じ projection を表現している場合だけである。file scan のように next rows 丸ごと置換を受け取る経路は、owned collection 本体も next rows snapshot から置き換える。storage owner object の warning / maintenance など owner property の projection は owner reference から current value を読むが、path / digest は `LibraryChartRefIndexSnapshot` や installed lookup などの隣接 index にも入るため、path mutation / digest update は dispatcher payload として同期する。`BMSFiles` / `BmsonSongs` の public getter は read-only view を返し、collection の add / remove / reorder は setter または internal mutation helper を通す。

full invalidate に落とす条件も固定する。

- `BMSFiles` / `BmsonSongs` の丸ごと置換。
- delta が old path / new path / digest / owner identity の対応を持たない。
...
- md5 primary hash count
- md5 primary hash -> current path lookup
...
installed lookup は owned collection の current installed source に隣接する hash/directory/path index として扱う。初回 build は `BMSFiles` / `BmsonSongs` を直接列挙せず、owned collection の lightweight entry view から行う。mutation では unregister / install upsert / merge path change / folder move を add / remove / move / digest update として表現し、表現できない外部 replacement だけ full invalidate する。API は `IPrimaryHashLookup` と `IInstalledChartLookupIndex` の用途を分け、重複 skip や安全削除は md5 primary hash lookup、install destination 推定は directory lookup snapshot を読む。md5 primary hash から修復候補 path や resource-only merge display の候補 path だけを引く用途は state の隣接 path lookup を使い、directory snapshot に全 path list を載せて重くしない。

playlist detail resolve index は owned collection の current hash / path / representative rule に隣接する index として扱う。ViewModel は md5 / sha256 辞書を構築する責務を持たず、playlist entry の selected key と filter 種別だけを渡して解決結果を受け取る。index は md5 と sha256 の両方を持つが、解決は `PlaylistEntryLookupKey` が選んだ片方だけを読み、同一 hash の代表は path 昇順最小で固定する。代表選択を ViewModel に残すと model 側の owned collection と UI cache が別々の正本になりやすいため、代表選択は model/index 側へ移す。
...
| real path directory view / subtree counts | added / removed / moved の real path directory | affected directory bucket を更新。表現不能なら directory view invalidate。startup 完了後は summary 外の best-effort warmup で `LibraryChartRefIndexSnapshot` を温め、folder / duplicate merge の初回操作に cold full build が乗らないようにする。 |
| owner/path canonical lookup | added / removed / moved / storage owner replace | affected owner/path key を更新。owned current source の canonical path は unique なので、複数候補は fallback ではなく invariant violation として検出する。 |
...
| playlist detail resolve index | added / removed / moved / digest updated / pathless state changed | BMSLibrary cached snapshot を invalidate。未構築なら mutation 時に build せず、playlist prewarm または次回 detail open 時に再構築する。rebuild publish は invalidation version、owned collection version、storage rows version で guard する。 |
| playlist reference apply hash subset | playlist entries / table replacement の selected key set | 永続 index ではなく、apply 時に reference map key で owned collection から matching refs だけを抽出する on-demand view。全 owned refs snapshot を作ってから caller 側で filter しない。 |
...
| duplicate row index / duplicate groups cache | added / removed / moved / digest updated / duplicate warning changed | row index は構築済みなら remove / path change / upsert / MD5 digest update を delta apply し、未構築なら mutation 時に build しない。MD5 digest update は kind / path / old MD5 が 1 row に解決できる時だけ row を置き換える。複数 row に当たる key は duplicate-path owned chart の invariant violation であり、full invalidate で隠さない。SHA256-only digest update は duplicate identity に影響しない。group result cache は当面 invalidate。dirty signal は `DuplicateChartGroupsInvalidationVersion`、result replacement は `DuplicateChartGroups` property change に分ける。incremental duplicate group update は後続最適化でよい。 |
| normal library source / dependency generation | added / removed / moved / source identity changed / displayed warning / maintenance / install destination / ref-table changed | source generation は owned collection version ごとに一度だけ消費する。warning / maintenance / install destination / ref-table だけの変更は source generation と identity sort generation を進めず、該当 dependency generation と表示 refresh に閉じる。full regular row fallback は使わない。 |

dispatcher の log は、全 index に個別詳細 log を増やすのではなく、mutation result と更新結果の summary を 1 つ出す。例: `owned_collection_mutation_dispatch reason=merge_folder added=0 removed=6 moved=0 digestChanged=0 installedLookup=delta parentFolder=invalidate duplicate=invalidate playlistResolve=invalidate resourceHealth=defer elapsedMs=...`。各 index の既存 performance log は、full build / incremental update / unexpected full invalidate のように意味がある場合だけ残す。
...
- **playlist detail resolve index の所有場所**: md5 / sha256 辞書と representative selection は ViewModel ではなく BMSLibrary / owned collection 隣接 index に置く。代表選択は path 昇順最小、解決は `PlaylistEntryLookupKey` の selected key のみ、pathless / md5less owned chart は kind に関係なく owned 判定から除外する。BMSLibrary は `PlaylistLibraryResolveIndexSnapshot` の cache / currentness / publish を持ち、ViewModel は model 側 snapshot の取得、prewarm task、readiness 表示だけを扱う。
- **playlist index invalidation boundary**: library charts / bmson changes だけでなく、digest update、path 変更、owned collection rebuild、chart digest backfill 後の hash currentness で index を invalidate する。現行の playlist detail resolve index は BMSLibrary cached snapshot を丸ごと invalidate し、未構築なら mutation 時に build しない。index 内の `LibraryChartRef` は immutable `ChartFile` snapshot を持ち、公開済み snapshot が storage owner の後続 mutation を読むことはない。representative path / pathless / md5less owned chart 除外は index snapshot の責務になる。
...
- **resource health の全件入力取得**: mutation の no-op / invalidate / defer、成功する delta、失敗時 invalidate 指定の delta は full owned 入力を取得しない。`ResourceHealthIndexOwner.Apply` は実 full rebuild / 必要な delta fallback に入力が不足する場合だけ同期 provider を呼ぶ。取得は owner の state lock 外で行い、storage owner から不変入力へ切り離した後の current version と照合し、公開直前にも照合する。指定済み full 入力はその版を検証する。owned collection の変更前に取得した入力は dispatcher で破棄し、実 full 分岐で必要な場合に再取得する。延期後の view 読み取りによる構築・以後の再利用、writer 予約、失効・通知の契約は維持する。不要な全件処理の除去は `ResourceHealthIndexOwnerTests` の取得観測と既存 library mutation / merge tests で検証し、大規模の操作完了時間は別途計測する。
...
- **duplicate cache の差分更新粒度**: duplicate group の incremental update は必須ではない。まずは collection mutation result から duplicate cache を invalidate し、full duplicate search は user action / explicit refresh の明示 operation として残す。差分 duplicate update を入れる場合も、md5 primary hash / directory bucket の correctness を test できる段階で別 phase とする。
...
- collection mutation result を作る前に old-side 情報を失わない。storage row owner を先に書き換えると old directory / old digest が取れなくなるため、move / digest update / install destination change は pre-mutation capture を必須にする。
...
- playlist detail open で library hash index build が owned collection lock 内の作業を軽量 `LibraryChartRef` snapshot 取得に留め、md5 / sha256 map 構築は lock 外で行う。cached snapshot が current の場合はそれを読む。
...
`RegularChartListOwner` の chart-common owner contract は、旧 `RegularChartListOwnerTests` の76 caseを5つの owner fixtureへ分けて検証する。Functional は一つの canonical six-host planを使い、`portable-settings`（1 worker / `ClassLevel`）の完了後、`bass-collectible`、`serial-state-a`、`serial-state-b`、`remaining-bms-library`、`remaining` を待機 waveなしで fanout する。この5 fixtureの FQN は exact 45 class selectorにも `BeMusicSeeker.Tests.BmsLibrary` logical prefixにも一致しないため、`remaining` の `R & FullyQualifiedName!~BeMusicSeeker.Tests.BmsLibrary` logical-negative partition（`ProcessorCount` workers / `ClassLevel`）で一度だけ実行する。元の method body、assertion、GUID付き song DB / filesystem root、task / event completion、failure watchdogは変更しない。共通の test double と builder は `RegularChartListOwnerTestSupport` という一つの narrow supportに閉じ、mutable static state、DNP、新process、production seamを追加しない。

| Behavior / failure contract | Owner fixture | Existing case set | Lane / completion signal | Retired route |
| --- | --- | --- | --- | --- |
| tree / maintenance / install navigation, summary transition, refresh request | `RegularChartNavigationTests` | declaration indices 1-7 | `remaining` logical-negative partition, `ProcessorCount` workers / `ClassLevel`; existing synchronous presentation events and refresh assertions | monolithic `RegularChartListOwnerTests` methods 1-7 |
| normal-library source attach, replacement, drain, version, apply failure | `RegularChartNormalLibraryRefreshTests` | 8-10 and 14-19 | same `remaining` logical-negative partition; existing task / event receipts, catalog gate, and failure watchdog | monolithic methods 8-10 and 14-19 |
| folder rename serialization, queued refresh, shutdown drain | `RegularChartFolderRenameTests` | 11-13 | same `remaining` logical-negative partition; existing mutation completion and shutdown receipt | monolithic methods 11-13 |
| materialized / virtual build, filter, ordering, cache and prewarm behavior | `RegularChartViewBuildAndOrderingTests` | 20-45 | same `remaining` logical-negative partition; existing request leases, cancellation signals, and publication assertions | monolithic methods 20-45 |
| commit, nested request retirement, presentation notification, disposal and lifecycle failure | `RegularChartCommitAndLifecycleTests` | 46-76 | same `remaining` logical-negative partition; existing terminal receipts, event ordering, and failure watchdog | monolithic methods 46-76 |
...
The owned-chart collection contract is covered by the existing 105-case and 18-case canonical fixtures split into owner-local `ClassLevel` fixtures. The old `owned-chart-collection` named performance shard is retired; these fixtures now use the canonical `remaining` logical-negative partition (`R & FullyQualifiedName!~BeMusicSeeker.Tests.BmsLibrary`, `ProcessorCount` workers / `ClassLevel`). The same route applies to the five regular-chart owner fixtures above; no additional process or DNP boundary is introduced. Each replacement preserves the original GUID-owned song database / filesystem root, immutable snapshot, task / event completion, failure watchdog, and cleanup semantics.

| Behavior / failure contract | Owner fixture(s) | Existing case set | Lane / route | Retired fixture |
| --- | --- | --- | --- | --- |
| storage-row projection, snapshot and storage-owner index | `OwnedChartCollectionProjectionTests` | `OwnedChartCollectionStateTests` methods 1-9 | `remaining` logical-negative partition, `ProcessorCount` workers / `ClassLevel` | monolithic `OwnedChartCollectionStateTests` methods 1-9 |
| library chart-reference index resolution and mutation | `OwnedChartCollectionReferenceIndexTests` | methods 10-41 | same `remaining` logical-negative partition and resource contract | monolithic methods 10-41 |
| owned hash / path lookup, membership and duplicate snapshots | `OwnedChartCollectionLookupMembershipTests` | methods 42-61 | same `remaining` logical-negative partition and resource contract | monolithic methods 42-61 |
| library mutation delta and resource-health invalidation | `OwnedChartCollectionLibraryMutationTests` | methods 62-73 | same `remaining` logical-negative partition and resource contract | monolithic methods 62-73 |
| installed and install-destination overlay lookup | `OwnedChartCollectionInstalledOverlayTests` | methods 74-92 | same `remaining` logical-negative partition and resource contract | monolithic methods 74-92 |
| normal-library refresh and file-scan notifications | `OwnedChartCollectionRefreshTests` | methods 93-97 | same `remaining` logical-negative partition and resource contract | monolithic methods 93-97 |
| inline chart-info digest mutation and event validation | `OwnedChartCollectionInlineDigestTests` | methods 98-105 | same `remaining` logical-negative partition and resource contract | monolithic methods 98-105 |
| playlist summary count and presentation projection | `PlaylistSummaryCountAndPresentationTests` | `PlaylistSummaryAggregationTests` methods 2-6, 15-18 | same `remaining` logical-negative partition and resource contract | monolithic methods 2-6, 15-18 |
| owned hash index build and cancellation | `PlaylistSummaryOwnedHashTests` | methods 1, 7 | same `remaining` logical-negative partition and resource contract | monolithic methods 1, 7 |
| owned hash mutation and warm / rebuild lifecycle | `PlaylistSummaryMutationAndWarmTests` | methods 8-11 | same `remaining` logical-negative partition and resource contract | monolithic methods 8-11 |
| playlist detail resolve-index cache and digest window | `PlaylistSummaryResolveIndexTests` | methods 12-14 | same `remaining` logical-negative partition and resource contract | monolithic methods 12-14 |
| regular chart navigation, refresh, rename, view ordering and lifecycle | `RegularChartNavigationTests`, `RegularChartNormalLibraryRefreshTests`, `RegularChartFolderRenameTests`, `RegularChartViewBuildAndOrderingTests`, `RegularChartCommitAndLifecycleTests` | `RegularChartListOwnerTests` methods 1-76 | same `remaining` logical-negative partition and resource contract | monolithic `RegularChartListOwnerTests` |

These fixtures have one `remaining` host descriptor with `ProcessorCount` workers and `ClassLevel` scope. The complementary `remaining-bms-library` descriptor uses the same `R` base and `FullyQualifiedName~BeMusicSeeker.Tests.BmsLibrary` positive selector; the two logical-prefix partitions are disjoint and cover `R` exactly once. The validator checks the shared exact 45-class exclusion, logical-prefix polarity, cross-host uniqueness, and absence of the retired monolith selectors.
