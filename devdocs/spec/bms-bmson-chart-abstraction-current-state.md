...
1. resource health / maintenance service 内の残存 BMSFile 境界を分類する。`BmsLibraryMaintenanceService` / inline maintenance は `bmson_song` と `ChartResourceSnapshot` を直接扱う状態まで進んだため、次は BMS 専用 maintenance API と chart-common warning projection の境界を分ける。
