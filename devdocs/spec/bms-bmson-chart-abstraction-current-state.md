...
package entry から BMS storage owner を扱う production 操作は、原則として `PackageChartEntry.Chart.BmsFile` を読む。`GetExistingBmsFormatAdapter()` は BMS parser / maintenance warning writeback のように、entry の BMS format mirror へ明示的に降りる境界だけに限定する。generic な `GetOrCreateCompatibilityAdapter()`、`CompatibilityAdapter` property、`GetOrCreateBmsFormatAdapter()` の production API は削除済みであり、bmson を package entry から新規 `PendingChartEntry` として materialize する入口や、既存 bmson adapter を chart-common state の正本として読む入口は残さない。
