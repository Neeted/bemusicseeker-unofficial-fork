...
2. **Install destination state の chart-domain 化**
   ...
   - 2026-05-22 現在、package entry と ViewModel row 表示 / loose chart transient cache の境界は chart-common state へ寄っている。`LibraryInstallDestinationChange.GetCurrentInstallDestination()` は `Entry.Chart` / `Chart` を読むだけにし、folder move / merge の target enumeration と BMS player temporary install fallback も `ChartFile.InstallDestination` を読む。repair fix 後の BMS install destination clear、deleted-folder cleanup 後の BMS install destination full clear、startup file scan diff の stale install destination cleanup は、service 内の直接 mutation ではなく `LibraryMutationDelta.UpdatedInstallDestinations` に載せる。ただし、startup cleanup の stale 判定入力はまだ BMS owner の `instl_dst` であり、folder move / merge の library row enumeration も `LibraryChartRef.ToChartFile()` projection を通るため、現時点で拾える loose library chart の install destination は storage-backed projection と `PackageChartEntry` state に限られる。ViewModel transient cache にだけ存在する bmson install destination を model-layer へ持ち込む経路はまだ未完了である。残る主要対象は `BmsLibraryStateApplier.ApplyLibraryMutationDelta(...)` の BMS owner writeback、startup cleanup の BMS owner `instl_dst` read、ViewModel transient state を mutation input / output として運ぶ経路である。これらは DB 永続化ではなく runtime state 更新として再分類し、可能な範囲で `ChartFile` / `PackageChartEntry` / ViewModel transient state へ寄せる。
...
playlist reference 統合と `BMSFile.RefTables*` legacy API cleanup が終わった一方で、`BMSFile` 内部には install destination / warning / chart_info / maintenance-resource health の chart-common state がまだ残っている。install destination については package entry と ViewModel transient cache への移行が進み、通常表示 / playlist detail / loose edit、BMS player temporary install fallback の正本は chart state 側へ寄っている。folder move / merge / deleted-folder cleanup は `ChartFile.InstallDestination` と `LibraryMutationDelta` を読み、startup cleanup は stale 判定入力に BMS owner `instl_dst` を使いつつ mutation 出力を `LibraryMutationDelta` に寄せている。`LibraryChartRef` projection が ViewModel transient cache を保持しないため、loose bmson の transient install destination を完全に扱える状態ではない。model-layer の state apply には BMS owner fallback も残る。現在の完了度は **90% 前後** と見る。残りは `BMSFile` 参照数の削減ではなく、`BMSFile` member surface を BMS-only / LR2 storage row / BMS parser / score producer として説明できる状態へ寄せる作業である。
