...
auto install discovery で directory scan 済みの package を明示 chart list 付きで作る時は `ChartPackage.FromChartEntries(...)` を使う。これにより、scan 結果の adapterless bmson entry を `BMSFile` list に戻さず package 正本として保持できる。既存 UI / mutation が adapter を必要とした時だけ `GetChartAdapters()` または対象 entry から materialize する。
...
pending package フォルダー削除の失敗時集計は `PackageChartEntry.Chart.Path` を使う。削除成功時は削除済み chart を `FilesToRemove` に返す必要があるため対象 entry の adapter を materialize するが、削除失敗時は adapterless bmson entry を materialize しない。
