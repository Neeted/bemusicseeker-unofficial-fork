...
playlist reference 表示は、BMS / bmson を分けず `ChartFile` identity から解決する。playlist entry の md5 / sha256 から作る `PlaylistReferenceIndex` を通常一覧 row / virtual source row / playlist detail source row に注入し、lookup は md5 優先、見つからない場合だけ sha256 fallback とする。`BMSLibrary` の playlist reference 更新も BMS storage row の `BMSFile.RefTables` / `RefTablesSymbols` / `RefTablesNames` へは書き戻さず、UI 更新は `PlaylistReferenceIndex` の version / table 更新通知と chart row の playlist reference projection invalidation で行う。`BMSFile.RefTables*` API はまだ legacy storage-row surface として残っているが、表示 / sort / keyword search / `BMSLibrary` mutation の正本ではなく、最終 cleanup で削除対象である。
...
ただし `BMSFile.RefTables` API 自体の削除まで含めると、playlist reference は未完了の大きな boundary として再分類される。現在の完了度は、当初の active plan 全体ではおおよそ **80〜85%**、`BMSFile.RefTables` 削除まで含めた最終形では **75〜80%** 程度と見る。`BMSFile.RefTables*` legacy API cleanup が終われば **90% 前後** まで進み、残りは BMS-only と説明できる参照の監査、test/doc cleanup、必要なら小さな symbol rename になる見込み。
...
- playlist reference の adapter materialization boundary と BMS storage row への `RefTables` writeback boundary は閉じており、pending bmson のために adapter は materialize せず、BMS storage row にも playlist reference cache を書き戻さない。外部から任意の `IEnumerable<BMSFile>` を渡して playlist reference を部分適用 / 表示更新する production API は残さず、更新は `PlaylistReferenceIndex` / chart identity / row invalidation へ寄せる。未完了なのは `BMSFile.RefTables*` legacy API surface の削除である。
...
- `PackageChartEntry.GetPlaylistReferenceAdapter(...)` は削除済み。playlist reference のために adapter を作る入口は残さない。BMS storage row への `RefTables` writeback も削除済みであり、残っている `BMSFile.RefTables*` は production mutation / display 正本ではない legacy API surface として最終 cleanup 対象である。
...
- playlist reference 表示 / sort / keyword search は `PlaylistReferenceIndex` / chart identity / row projection を正本にし、BMS storage row への `RefTables` writeback は production に残っていない。`BMSFile.RefTables*` legacy API surface とそのテストは削除対象として残っている。
