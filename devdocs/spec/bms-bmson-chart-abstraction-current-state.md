...
4. resource health / maintenance service 内の bmson adapter materialization を削る。UI surface は chart-native になったため、次は `BmsLibraryMaintenanceService` / inline maintenance が `bmson_song` と `ChartResourceSnapshot` を直接扱うようにする。
...
- estimated install batch の deferred maintenance state は BMS と bmson を分け、BMS は `DeferredBmsMaintenanceTargets`、bmson は `DeferredBmsonMaintenanceSongs` に保持する。`setMaintenanceInfo(...)` は BMS row と `bmson_song` を別入力として受け、`BmsLibraryMaintenanceService.UpdateMaintenanceInfo(...)` の legacy `BMSFile` 実行境界でだけ bmson の一時 `PendingChartEntry` を作る。旧実装では deferred target list 自体に bmson adapter を混ぜていたが、batch result と BMSLibrary の maintenance orchestration では `bmson_song` を正本にする。inline chart_info は direct deferred bmson songs と installed package から解決した bmson songs を path dedupe して渡し、resource-only display package 由来の既存挙動を落とさない。
- folder merge 後の maintenance 再計算も BMS row と `bmson_song` を分けて `setMaintenanceInfo(...)` に渡す。旧実装では移動済み bmson と移動先既存 bmson を `PendingChartEntry` adapter にして BMS target list へ混ぜていたが、merge orchestration の正本は `bmson_song` であり、adapter 作成は maintenance service の legacy 実行境界へ閉じ込める。
