...
1. resource health / maintenance warning の BMS-only writeback 境界
...
2. library file operation の chart-common cleanup 境界
...
1. library file operation cleanup を chart-common 化する。folder delete / install destination cleanup が chart target 全体に効くべき箇所と、BMS storage row だけに効く箇所を分ける。
2. resource health / maintenance service 内の残存 BMSFile 境界を分類する。`BmsLibraryMaintenanceService` / inline maintenance は `bmson_song` と `ChartResourceSnapshot` を直接扱う状態まで進んだため、次は BMS 専用 maintenance API と chart-common warning projection の境界を分ける。
