...
5. **Warning / source generation / sort-key の分離: 進行中**
   ...
   - 現状では source / overlay / maintenance producer 由来の warning・maintenance sort-key invalidation は refresh notification の effect と normal library refresh notification version で管理している。`ChartFilesNeedResourceFix` / ignored / garbled 系 property notification は互換表示通知として残っているため、次の段階ではこれらを refresh effect queue または後継 notification へ寄せ、warning-only / maintenance-only の表示更新を個別 property handler に依存しない形へ移す。
