...
1. playlist reference mutation 境界  
   playlist detail / 通常一覧表示は `ChartFile` / `PlaylistReferenceIndex` に寄っており、owned bmson の参照表示のためには adapter を materialize しない。一方、playlist table の差し替え・削除・一括 refresh では pending package target を `ChartFile` で照合した後、BMS storage owner だけに `RefTables` を書き戻す経路が残っている。残る課題は、BMS `RefTables` writeback を BMS-only mutation として明確化し、bmson の表示 state は `PlaylistReferenceIndex` / chart identity だけで完結させることである。

2. resource health / maintenance warning の BMS-only writeback 境界  
   `ChartResourceSnapshot.Create(ChartFile)` と bmson-native maintenance pipeline は存在する。resource health は BMS / bmson 共通なので、一覧表示・filter・projection の入口は `ChartFile` に寄せる。残る課題は BMS-only encoding / zero-note / strict BMS parser state と、chart-common warning / ignore state をさらに分離することである。

3. library file operation の chart-common cleanup 境界  
   folder move / merge / delete は `LibraryChartRef` / `ChartFile` へ寄っているが、一部の cleanup はまだ `LibraryChartRef.BmsFile` から BMS storage row だけを取り出す。bmson の install destination / transient state を同じ chart-common cleanup に含めるか、BMS-only cleanup と明示するかを実装側で整理する。
...
1. playlist reference mutation を分離する。pending / bmson の ref 表示は `PlaylistReferenceIndex` と chart identity から解決し、`BMSFile.RefTables` writeback は BMS storage owner に限定する。
2. library file operation cleanup を chart-common 化する。folder delete / install destination cleanup が chart target 全体に効くべき箇所と、BMS storage row だけに効く箇所を分ける。
...
4. test と docs の adapter 表現を cleanup する。behavior test は `bmson_song` / `ChartFile` / `PackageChartEntry` を正本にし、旧 adapter 非生成を明示する必要がある場合だけ historical wording を残す。
...
- pending package の resource health warning 計算では、旧実装のように `BMSFile.SetHealthStatus()` / `requiresPendingWarning(BMSFile)` へ bmson adapter を降ろさず、`ChartFile` / `bmson_song.MaintenanceInfo` / `ChartResourceSnapshot` から warning list を返す。旧実装では adapter-backed bmson の場合だけ warning 計算の副作用で adapter の warning collection を更新し得たが、bmson の正本は `bmson_song` と package entry state なのでこの副作用は移植しない。
