...
`ChartFileProjection.FromBmsFile(...)` は通常 BMS では `BmsFile` に実体を入れ、`PendingChartEntry` の bmson adapter では `Kind=Bmson`, `BmsFile=null`, `BmsonSong=adapter owner` とする。`FromBmsonSong(...)` は storage owner として `bmson_song` を持ち、必要な場合だけ caller が `ChartFileTransientState` を渡して表示 snapshot を作る。`ChartFileTransientState` は adapter object ではなく subtitle / install destination / warning / health / encoding の一時状態だけを受け取る。
...
`ChartPackage(BMSFile)` / `ChartPackage(IEnumerable<BMSFile>)` constructor は production API から削除済みである。package を明示 chart list 付きで構築する場合は `ChartPackage.FromChartEntries(...)` を使い、`BMSFile` 継承 adapter から package entry に落とす必要がある境界では caller が `PackageChartEntry.FromChartAdapter(...)` を明示する。tests 側だけで必要な fixture construction は `ChartPackageTestExtensions.CreatePackage(...)` に閉じ込め、production に BMSFile-list constructor を互換 API として残さない。
...
1. resource health / maintenance warning 境界  
   `ChartResourceSnapshot.Create(ChartFile)` と bmson-native maintenance pipeline は存在する。resource health は BMS / bmson 共通なので、BMS-only encoding / zero-note と分離し、`BMSFile` adapter を warning / ignore state の正本にしない。残る課題は package / pending 側に残る既存 adapter 読み取りをさらに狭め、入力と保存先を `ChartFile`、`ChartResourceSnapshot`、maintenance row、または既存 storage owner の責務として整理し切ることである。
...
3. package / pending install entry 境界  
   `ChartPackage.ChartEntries` と `PackageChartEntry.Chart` は package 内 chart の正本になっている。generic な `PackageChartEntry.GetOrCreateCompatibilityAdapter()`、`PackageChartEntry.CompatibilityAdapter`、`PackageChartEntry.GetOrCreateBmsFormatAdapter()` production API は削除済みであり、BMS format chart 用の `GetExistingBmsFormatAdapter()` だけが残る。pending warning / install destination / suggestions / resource snapshot は `PackageChartEntry` と storage row に寄せ、adapter は BMS-only legacy API に降りる境界へ限定する。

4. UI row / operation target 境界  
   `LibraryChartRow`、`PlaylistDetailSourceRow`、`PlaylistDetailRow`、`GridRowResolver`、`ChartOperationTarget` は `ChartFile` を正本にし、owned bmson 用 `CompatibilityBmsFile` surface は削除済みである。残る課題は、後続の model / package / maintenance 境界が `ChartOperationTarget.Chart` から直接進めるか、内部で adapter を要求していないかを潰すことである。
...
3. `PackageChartEntry` から残りの BMS-only adapter surface を削る。`CompatibilityAdapter` 読み取りは production surface から削除済みであり、次は BMS-only adapter 取得を state apply、initialization、resource warning の順に消し、entry は `ChartFile` と package transient state の owner にする。
4. resource health / maintenance service 内の bmson adapter materialization を削る。`BmsLibraryMaintenanceService` / inline maintenance は `bmson_song` と `ChartResourceSnapshot` を直接扱う状態まで進んだため、次は package / pending 側で残る adapter writeback 境界へ寄せる。
...
- `PackageChartEntry.GetOrCreateCompatibilityAdapter()` は production surface から削除する。BMS format chart で BMS parser / maintenance API が必要な箇所は `GetExistingBmsFormatAdapter()` を使い、これは BMS storage owner または既存 BMS adapter だけを返す。bmson adapter materialization は production の package entry API では提供しない。旧 test が adapter writeback を検証する場合は test-local helper に閉じ込め、後続 cleanup で chart state assertion へ寄せる。
- `PackageChartEntry` 内の mutable writeback target は BMS format mirror のみとする。entry が materialized bmson adapter を保持している古い形でも、install destination / warning / installed path などの chart-common state は `PendingChartEntry` adapter へ書き戻さず、`bmson_song` と package entry state に保持する。旧実装では adapter-backed bmson の adapter fields も同時に更新され得たが、bmson adapter は最終形の storage owner ではないため、この副作用は移植しない。
...
- `PackageChartEntry.GetOrCreateBmsFormatAdapter()` は production surface から削除済みである。旧実装では名前上 adapter creation 境界に見えたが、最終的には BMS storage owner / 既存 BMS adapter を読むだけになっていたため、`GetExistingBmsFormatAdapter()` に統一する。起動時 pending warning 初期化や BMS extension rename snapshot のように BMS parser / maintenance API が必要な箇所だけが `GetExistingBmsFormatAdapter()` を呼び、bmson では null を返す。旧 `GetOrCreateCompatibilityAdapter()` は bmson も materialize できるため、chart-common 経路から直接呼ばない。
- `PackageChartEntry.GetExistingBmsFormatAdapter()` は既に存在する BMS format storage row / adapter を読むだけの境界である。playlist reference の `RefTables` 書き戻し、install result の `AddedBmsFiles` 抽出、install estimation の lock target、folder merge 後の BMS upsert target のように BMS-only side effect が必要な箇所ではこの helper を使う。bmson は adapter-backed であっても null とし、chart-common 表示 / resource / install metadata は `ChartFile` / `PackageChartEntry` 側を読む。package source row / playback stop target snapshot は `PackageChartEntry.Chart.BmsFile` を直接読むため、表示・snapshot のためだけに BMS-only helper へ降りない。
- install 後の source folder safe cleanup は、残存 chart がすべて導入済み hash かだけを確認する。bmson 残存 chart は `BmsonSongParser.Parse(...)` で `bmson_song` として lookup hash を読み、`PendingChartEntry.CreateFromFilePath(...)` へは通さない。旧実装では hash 確認だけのために bmson `PendingChartEntry` を作っていたが、cleanup 判定は chart identity の照合であり adapter state を必要としないため移植しない。
...
- `CompatibilityBmsFile` / `CompatibilityAdapter` は production surface に残っていない。`GetOrCreateCompatibilityAdapter()` は production surface から削除済みであり、BMS-only adapter 取得も BMS 専用処理に限定されている。
