...
playlist reference 統合と `BMSFile.RefTables*` legacy API cleanup が終わった一方で、`BMSFile` 内部には install destination / warning / chart_info / maintenance-resource health の chart-common state がまだ残っている。現在の完了度は **90% 前後** と見る。残りは `BMSFile` 参照数の削減ではなく、`BMSFile` member surface を BMS-only / LR2 storage row / BMS parser / score producer として説明できる状態へ寄せる作業である。
