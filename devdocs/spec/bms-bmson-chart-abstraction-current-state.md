...
この dispatcher はまだ final contract の途中段階である。installed lookup、primary hash -> path lookup、install destination overlay、parent folder cache、playlist summary owned hash、playlist detail resolve index、duplicate cache、resource health index は同じ mutation boundary から同期または無効化されている。real path directory view と owner/path canonical lookup は owned collection の `LibraryChartRefIndexSnapshot` として mutation に追従する。`LibraryChartChangeNotification` は install destination 専用 queue ではなく normal library refresh hint queue へ移行中で、dispatcher result から `SourceChanged` / `InstallDestinationOverlayChanged` / `WarningPresentationChanged` / `MaintenancePresentationChanged` の effect と owned collection version を運べる。ViewModel はこの notification version range を消費し、source generation と overlay transient state の判定を旧 setter 副作用から少しずつ切り離す。warning / maintenance effect は payload として予約し、表示更新の実消費は各 warning / maintenance property handler から段階的に寄せる。
...
5. **Warning / source generation / sort-key の分離: 進行中**
   ...
   - `LibraryChartChangeNotification` / batch は refresh effect と owned collection version を持つ。dispatcher は source mutation / install destination overlay mutation を notification として publish し、`BMSFiles` / `BmsonSongs` property handler は未処理 notification range を消費して source generation と transient overlay cache を更新する。
   - `BMSFiles` / `BmsonSongs` property change と通常 refresh 中の bmson source sync は、notification がある場合は batch の owned collection version と effect を見て source generation を消費し、notification がない旧経路だけ current owned collection version に fallback する。BMS と bmson の通知順が前後しても、同じ owned collection mutation では source generation を二重に進めない。既に消費済みの version で bmson source cache だけが更新された場合は、virtual source row cache を破棄して次回再構築させる。
   ...
   - 現状では source / overlay は refresh notification の effect と library chart change notification version で管理している。次の段階では warning / maintenance property handler も同じ effect queue または後継 notification へ寄せ、warning-only / maintenance-only の表示更新を `BMSFiles` / `BmsonSongs` property handler に依存しない形へ移す。
...
- **internal storage row notification の切り分け**: `BMSFiles` / `BmsonSongs` setter は external full replacement の full invalidate 境界として残す。internal mutation で property setter を迂回する場合は、代替として `OwnedChartCollectionVersion`、`LibraryChartChangeNotification`、`BMSFiles` / `BmsonSongs` のどれを UI refresh signal にするかを同じ commit で固定する。単に setter を避けて通知を消す変更はしない。
