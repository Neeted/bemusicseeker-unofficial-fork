...
- BMSLibrary の folder move / merge service 呼び出しは、service 境界で `BMSFile` + `bmson_song` の二系統を受けてから `LibraryChartRef` / `ChartFile` delta へ変換する箇所が残る。次の大きめの境界整理候補は、呼び出し側で installed chart snapshot / library chart ref snapshot を作り、service は chart-native input を受ける形に寄せること。
