...
6. 旧 callback / suppression scope は、新しい dispatcher 経路と重複しなくなったものから削除する。新規 index 同期は必ず dispatcher に置き、setter / stateApplier callback へ新しい責務を増やさない。残る大きな候補は、internal mutation が `BMSFiles` / `BmsonSongs` property notification をどこまで汎用 refresh signal として維持するかであり、ここは UI refresh / source generation の互換を確認しながら切る。
...
この dispatcher はまだ final contract の途中段階である。installed lookup、primary hash -> path lookup、install destination overlay、parent folder cache、playlist summary owned hash、playlist detail resolve index、duplicate cache、resource health index は同じ mutation boundary から同期または無効化されている。real path directory view と owner/path canonical lookup は owned collection の `LibraryChartRefIndexSnapshot` として mutation に追従する。`NormalLibraryRefreshNotification` は normal library refresh hint queue として、dispatcher result から `SourceChanged` / `InstallDestinationOverlayChanged` / `WarningPresentationChanged` / `MaintenancePresentationChanged` の effect、owned collection version、後続の `BMSFiles` / `BmsonSongs` property notification coverage を運ぶ。ViewModel はこの notification version range を消費し、source generation、source view refresh、overlay transient state、warning / maintenance sort-key invalidation の判定を旧 setter 副作用から切り離している。`NormalLibraryRefreshNotificationVersion` handler が source refresh の第一経路であり、`BMSFiles` / `BmsonSongs` handler は通知順が逆転した場合と external replacement の fallback だけを受け持つ。resource warning ignore / unignore、chart_info / zero-note producer 由来の warning 表示更新、manual resource maintenance producer 由来の warning / maintenance 表示更新、internal storage / hash mutation 由来の warning subset 表示更新は dispatcher 経由の effect に寄せている。duplicate / garbled hydration / external storage replacement などの互換表示 refresh は各 property handler から段階的に寄せる。
...
owned collection の currentness は storage row collection version で判定する。外部 setter / DB reload は version を進めて full invalidate し、internal mutation は storage row owner 更新と owned collection mutation を同じ boundary で行った後、owned collection に処理済み version を記録する。したがって owned view / index 取得のたびに `BMSFiles` / `BmsonSongs` と owned collection を全件照合しない。storage row collection の参照と version は同一 lock 境界で capture / update し、incremental mutation は適用前 version と owned collection の記録 version が一致する場合だけ適用する。処理済み version を記録できるのは、owned mutation が storage rows の同じ projection を表現している場合だけである。file scan のように next rows 丸ごと置換を受け取る経路は、owned collection 本体も next rows snapshot から置き換える。storage owner object の warning / maintenance など owner property の projection は owner reference から current value を読むが、path / hash は `LibraryChartRefIndexSnapshot` や installed lookup などの隣接 index にも入るため、path / hash mutation は dispatcher payload として同期する。`BMSFiles` / `BmsonSongs` の public getter は read-only view を返し、collection の add / remove / reorder は setter または internal mutation helper を通す。
...
| duplicate groups cache | added / removed / moved / hash changed / duplicate warning changed | 当面は duplicate cache invalidate。dirty signal は `DuplicateChartGroupsInvalidationVersion`、result replacement は `DuplicateChartGroups` property change に分ける。incremental duplicate group update は後続最適化でよい。 |
...
1. **Mutation dispatcher の formalization: 完了、拡張中**
   ...
   - 残タスクは、result payload を source / overlay / warning / maintenance の表示更新 flags へ分け、ViewModel property handler が旧 setter 副作用を読まずに済む形へ近づけること。
...
4. **Resource / maintenance target の dispatcher contract 化: 進行中**
...
5. **Warning / source generation / sort-key の分離: source refresh notification 化済み、storage fallback 縮小中**
   ...
   - `NormalLibraryRefreshNotification` / batch は refresh effect、owned collection version、後続の storage row property notification coverage を持つ。coverage は `NotifiesStorageRows` の aggregate に加え、`NotifiesBmsFiles` / `NotifiesBmsonSongs` を持つ。dispatcher は source mutation / install destination overlay mutation を notification として publish し、ViewModel は `NormalLibraryRefreshNotificationVersion` handler で未処理 notification range を消費して source generation、source view refresh、transient overlay cache を更新する。
   - `BMSFiles` / `BmsonSongs` property change と通常 refresh 中の bmson source sync は、notification がある場合は batch の owned collection version と effect を見て source generation を消費し、notification がない旧経路だけ current owned collection version に fallback する。storage row property handler が notification を先に消費した場合でも、source refresh は source generation が進んだ時だけ行い、warning / maintenance / install destination overlay の presentation effect は notification effect handler へ渡す。`NormalLibraryRefreshNotificationVersion` handler が先に処理した notification は property kind ごとの marker で記録し、後続の `BMSFiles` / `BmsonSongs` handler は自分の coverage が処理済みであれば main view source refresh を繰り返さない。BMS と bmson の通知順が前後しても、同じ owned collection mutationでは source generation と source view refresh を二重に進めない。既に `NormalLibraryRefreshNotificationVersion` handler で消費済みの version で bmson source cache だけが更新された場合は、virtual source row cache を破棄して次回再構築させる。
   - warning / maintenance computed subset property notification は dispatcher effect に統一済みであり、warning / maintenance 用の legacy coverage marker は残さない。`WarningPresentationChanged` / `MaintenancePresentationChanged` effect は常に notification 側で sort-key invalidation と表示 refresh を行う。storage owner boundary 用の `NotifiesBmsFiles` / `NotifiesBmsonSongs` と install destination overlay boundary 用の `NotifiesInstallDestinationOverlayProperties` は別契約として残す。overlay state は `NormalLibraryRefreshNotification` 側で消費し、overlay-only notification と storage-row notification が同じ未処理 range や同じ mutation result に混在しても overlay refresh を隠さない。
   - internal mutation 由来の storage row property notification は、該当 kind の owner collection が実際に変わる場合、または path change を既存 owner row property に反映した場合に限定する。`LibraryMutationDelta.NotifyStorageRowPathChanges` は path change 済みの `BMSFile` / `bmson_song` row を kind-specific storage owner boundary として通知するためのフラグであり、source refresh の正本ではない。source refresh は `NormalLibraryRefreshNotification` と owned collection version へ寄せ、`BMSFiles` / `BmsonSongs` を汎用 library-changed signal として使わない。install upsert の dispatcher payload は storage write 用 rows と chart-common `AddedCharts` を分け、installed lookup / resource health / warning presentation などの派生 index は rows を再結合せず `AddedCharts` を読む。
   ...
   - file scan diff は internal storage mutation として扱い、`BMSFiles` / `BmsonSongs` property notification は該当 kind の owner collection が実際に変わる場合だけ発行する。source refresh / warning / maintenance refresh は normal library refresh notification の effect を正本にする。
