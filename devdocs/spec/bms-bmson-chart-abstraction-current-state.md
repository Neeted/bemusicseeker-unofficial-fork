...
`GridRowResolver` は playlist row から `ChartOperationTarget` を作る際、row の `Chart.Kind` と `RealFile` / `ResolvedBmson` を組み合わせて source scope と capability を決める。owned bmson playlist row では `ChartFile.BmsonSong` を storage owner として保持し、operation target 作成のためだけには compatibility adapter を作らない。`PlaylistDetailRow` と `PlaylistDetailSourceRow` はどちらも playlist entry row として扱い、entry duplication / playlist cell edit policy / root folder drop preservation の判定で同じ `BMSTableEntry` を返す。
