...
- playlist reference の全 table 同期は、常に library BMS snapshot と pending `PackageChartEntry` snapshot を内部で取得して適用する。旧実装にあった `AddReferenceBMSTables(tables, files)` / `RefreshReferenceDisplayForTable(table, files)` / `RemoveReferenceBMSTables(table, files)` のような BMSFile subset 指定 API は、production からは使われず、chart-common boundary に BMSFile list を再導入する口になっていたため残さない。BMS-only `RefTables` mutation 自体は private helper と `BmsLibraryPlaylistReferenceService.ApplyReferenceMap(IEnumerable<BMSFile>, ...)` に残す。
