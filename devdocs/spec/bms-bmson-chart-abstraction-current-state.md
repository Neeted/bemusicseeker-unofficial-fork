...
resource health の選択系 UI handler も `ChartOperationTarget` を ViewModel に渡し、ViewModel が `ChartCompatibilityTargetSnapshot` を作って legacy model API 境界の `BMSFile` / bmson adapter に解決する。model 側の maintenance / health engine はまだ `BMSFile` payload を保守対象として使うため、この snapshot は互換境界であり、UI handler から直接 `GetSelectedChartCompatibilityAdapters(...)` は呼ばない。
