...
| owned chart digest update event | `LibraryChartDigestChange` | chart_info inline / backfill が storage owner の MD5 / SHA-256 を実ファイル digest に同期した時の差分 event。BMS missing SHA-256 補完も含む。pathless owner からは作らず、maintenance は producer ではない。 |
...
`DuplicateChartRow` は grouping / duplicate 判定用の path / primary lookup hash と、必要になった時だけ materialize する `ChartFile` を持つ。full search の row snapshot 段階では BMS / bmson storage owner と hash / path だけを保持し、duplicate group へ入る row だけ `CreateChart()` で `ChartFile` 化する。`BmsLibraryDuplicateService` は snapshot builder ではなく analyze / BMS duplicate warning apply の責務に閉じる。BMS duplicate row の storage owner は row の `BmsFile` または materialized chart の `GetBmsStorageOwner()` から読む。bmson duplicate row は `BmsonSong` または `ChartFile.GetBmsonStorageOwner()` で storage owner へ降りる。bmson duplicate row は `PendingChartEntry` compatibility adapter を materialize しない。duplicate warning clear は owned duplicate snapshot が保持する全 BMS storage rows に対して行い、path 欠損で duplicate 判定 row にならない BMS row の古い duplicate warning も消す。duplicate subset の仮想一覧表示は `DuplicateGroup.ChartFiles` をそのまま `ChartListSourceRow` へ渡すため、projection-only warning も落とさない。
...
- hash/directory/path index: installed lookup、duplicate merge の existing hash、duplicate full search の lightweight row snapshot、install estimation、resource-only merge display package に使う。owned collection の current path/hash を source にし、`primaryHashCounts`、`primaryHash -> paths`、`md5 -> directories`、`sha256 -> directories`、`known chart directories` を差分更新する。`CreateExcludingLookup` はこの index の primary hash count から作り、merge source 自身を除外する時も full chart snapshot を作らない。duplicate full search は owned collection の storage owner identity から `DuplicateChartRow` を作り、group materialize までは全件 `ChartFile` rich projection を行わない。resource-only merge display は original package の primary hash から candidate path を引き、destination 直下の candidate だけを path-only exact lookup で `PackageChartEntry` 化する。この path-only materialize は同一 path の複数 owner も保持し、canonical resolve の ambiguous 扱いとは分ける。
...
- digest updated charts: chart_info inline / backfill により storage owner の md5 / sha256 / primary lookup hash が変わった chart。path が同じでも hash index は更新する。pathless digest update event は invariant violation であり、full invalidate fallback で隠さない。
...
| :--- | :--- | :--- |
| installed lookup | added / removed / moved / digest updated の lightweight path/hash entry | 構築済みなら delta apply。未構築なら build しない。表現不能なら invalidate。pathless digest update event は異常系として検出する。 |
| primary hash -> path lookup | added / removed / moved / digest updated | installed lookup と同じ state に同居させ、candidate path lookup だけを返す。 |
