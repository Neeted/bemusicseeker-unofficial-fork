...
playlist reference 統合と `BMSFile.RefTables*` legacy API cleanup が終わった一方で、`BMSFile` 内部には install destination / warning / chart_info / maintenance-resource health の chart-common state がまだ残っている。install destination については package entry、ViewModel transient cache、model-side runtime overlay への移行が進み、通常表示 / playlist detail / loose edit、BMS player temporary install fallback、installed directory index / folder move library ref snapshot の正本は chart state 側へ寄っている。folder move / merge / deleted-folder cleanup / startup cleanup は `ChartFile.InstallDestination` と `LibraryMutationDelta` を読む。model-layer の state apply には BMS owner fallback も残る。現在の完了度は **90% 前後** と見る。残りは `BMSFile` 参照数の削減ではなく、`BMSFile` member surface を BMS-only / LR2 storage row / BMS parser / score producer として説明できる状態へ寄せる作業である。
