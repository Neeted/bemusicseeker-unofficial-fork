...
playlist reference 表示は、BMS では従来どおり `ChartFile.GetBmsStorageOwner()` で得た storage row の `RefTables` を読む。所持 bmson row は BMS storage owner を持たないので、playlist entry の md5 / sha256 から作る読み取り専用の `PlaylistReferenceIndex` を通常一覧 row / virtual source row / playlist detail source row に注入し、chart identity から参照 table 表示を解決する。lookup は既存の参照適用と同じく md5 優先、sha256 fallback とする。
...
- 追加後の playlist reference 表示更新は、追加元 row から解決した `ChartFile` list を `BMSLibrary.AddReferenceBMSTablesToCharts(...)` へ渡す。BMS storage row には従来どおり `RefTables` を書き戻し、bmson は `PlaylistReferenceIndex` 更新で表示へ反映する。旧実装のように追加後の対象を `BMSFile` list へ戻すと bmson が参照更新対象から落ちるため、その中間表現は残さない。
...
2026-05-20 時点で、active plan で追っていた adapter materialization boundary は閉じている。現在は migration というより audit / cleanup フェーズであり、残る `BMSFile` 参照を BMS storage row / LR2 song row / BMS-only operation として説明できるかを境界単位で確認する。chart-common 入口に BMS / bmson storage row overload が残っている場合は、`ChartFile` / `PackageChartEntry` / `LibraryChartRef` を正本にできるかを優先して検討する。
...
- playlist reference mutation は `PackageChartEntry` / `ChartFile` で pending target を照合し、BMS `RefTables` writeback だけを `ChartFile.BmsFile` に限定する。bmson の参照表示は `PlaylistReferenceIndex` / chart identity で完結し、pending bmson のために adapter を materialize しない。外部から任意の `IEnumerable<BMSFile>` を渡して playlist reference を部分適用 / 表示更新する production API は残さず、BMS storage owner list への書き戻しは `BMSLibrary` 内部 helper に閉じる。
...
- package install service の post-processing callback は `PackageInstallExecutionResult` を payload にする。storage row upsert / maintenance / library state apply は result から `AddedBmsFiles` と `AddedBmsonSongs` を分けて処理し、BMS-only の zero-note / score callback だけが `IEnumerable<BMSFile>` を受ける。これにより bmson install 後処理のために BMSFile list へ adapter を混ぜる入口を残さない。
- main view row source は virtual / non-virtual fallback の両方で `ChartFile` list を入口にする。通常 root / folder view は `CreateStandardLibraryChartSnapshot(...)` で BMS / bmson storage row を `ChartFile` へ投影してから filter / materialize し、BMS row cache へは row factory 内で `ChartFile.BmsFile` を明示的に降ろす。garbled / zero-note などの BMS-only subset も UI source selector は `ChartFile` を返し、`ChartListSourceRow` 入口へ BMSFile list を戻さない。
- duplicate detection の snapshot 入口は `ChartFile` list であり、`BMSFile` / `bmson_song` の二系統引数は残さない。BMS / bmson storage row の結合は `BMSLibrary` の installed chart snapshot 作成に閉じ、duplicate service は `ChartFile.PrimaryLookupHash` と `ChartLookupKey.GetPrimaryHashKind(ChartFile)` だけを見る。BMS storage row への warning writeback は `ChartFile.BmsFile` がある row に限定する。
...
- playlist reference の全 table 同期は、常に library `ChartFile` snapshot と pending `PackageChartEntry` snapshot を内部で取得して適用する。旧実装にあった `AddReferenceBMSTables(tables, files)` / `RefreshReferenceDisplayForTable(table, files)` / `RemoveReferenceBMSTables(table, files)` のような BMSFile subset 指定 API は、production からは使われず、chart-common boundary に BMSFile list を再導入する口になっていたため残さない。`BmsLibraryPlaylistReferenceService.ApplyReferenceMap(...)` の installed 側入口も `IEnumerable<ChartFile>` とし、md5 / sha256 matching は BMS / bmson を同じ chart として扱う。BMS-only `RefTables` mutation 自体は `ChartFile.BmsFile` が存在する場合にだけ行う。細かい実装メモとして、旧 log の `matchedSongFiles` は BMS row の matching だけを示していたが、現行 log は `matchedLibraryCharts` として bmson の match も含める。bmson の表示は `PlaylistReferenceIndex` が正本なので、match count が増えても bmson storage row に `RefTables` を持たせる副作用は移植しない。
...
- non-virtual fallback の `BuildStandardLibraryRowsForView(...)` も `ChartFile` list を入口にする。旧実装では `IEnumerable<BMSFile>` と bmson `LibraryChartRow` list を分けて受け、BMS row cache と bmson row cache を呼び出し側で結合していた。現行実装では ViewModel が BMS / bmson storage row を warning なしの `ChartFile` snapshot に投影し、row factory が `ChartFile.BmsFile` のある row だけ BMS row cache へ降り、`ChartFile.BmsonSong` のある row は bmson row cache を参照する。これにより通常一覧の fallback 経路でも chart source 境界へ `BMSFile` list と bmson row list の二本立て API を残さない。BMS-only subset の garbled / garble fixed / unregistered / zero-note は挙動として BMS 専用のままだが、virtual source selector は `ChartFile` snapshot を返す。
...
- `ChartFile.GetBmsStorageOwner()` / `GetBmsonStorageOwner()` は、domain `ChartFile` から永続化 owner へ降りる明示的な境界である。row cache / BMS player / package entry materialization など BMS-only または owner-backed な処理はこの helper を読む。`ChartFile.BmsFile` / `ChartFile.BmsonSong` property 自体は storage owner を保持する domain model の内部 surface として残すが、ViewModel 側で `chart.Kind` と raw owner property を直接組み合わせる分岐は増やさない。
...
- install execution の storage row upsert / maintenance / library state apply callback は `PackageInstallExecutionResult` を受け取る。旧実装では callback payload が BMSFile list だったため、通常 install の bmson は callback の外側で追加 upsert / maintenance / `BmsonSongs` 差し替えを行っていた。現行実装では BMS と bmson を同じ result-level callback で処理し、BMS-only の zero-note / score だけを `AddedBmsFiles` に限定する。通常 install の bmson storage row upsert は BMS と同じ song-db phase に寄せるため、後続の inline chart_info persist でも再 upsert され得るが、これは既に BMS で行っていた冪等 upsert と同じ扱いにする。
- resource maintenance の model-facing entry は `ChartFile` を primary にする。`BMSLibrary` は受け取った `ChartFile` list を内部で BMS storage owner と `bmson_song` に分け、`BmsLibraryMaintenanceService.UpdateMaintenanceInfo(...)` へ渡す。service 内の BMS / bmson 二本立て pipeline と `MaintenanceWorkflowResult` の合算は維持するが、UI / ViewModel / filter 側へ `BMSFile` list を chart-common entry として公開しない。
...
- `LibraryMutationDelta` の path 更新 / unregister payload は `ChartFile` list に統一する。旧実装では BMS 用の `FilePathChanges` / `FilesToUnregister` と bmson 用の `BmsonSongPathChanges` / `BmsonSongsToUnregister` が並列に存在し、mutation result の外部 surface が BMS-first だった。現行実装では `ChartPathChanges` / `ChartsToUnregister` を applier が BMS storage row と `bmson_song` に分割し、DB apply / unregister 直前だけ種別別処理へ降りる。旧実装の細かい挙動として、folder move の BMS path change は `OldPath` を明示しないため `ReplaceBmsFilePath(...)` が現在 path を old path として扱う一方、repair fix のように file move 後の BMS row では `OldPath` を持つ。この差は既存 DB 更新の安全確認に関わるため移植し、bmson path change は従来どおり `OldPath` を明示する。
