...
1. pending / install warning projection の BMS-only writeback 境界
   `ChartResourceSnapshot.Create(ChartFile)`、bmson-native maintenance pipeline、resource health index / ignore の `ChartFile` 入口は存在する。残る課題は pending package discovery / install table load / pending regroup 後の warning 再初期化で、BMS entry だけ `BMSFile.SetHealthStatus()` / strict check / `BMSFile.Warnings` mutation を中間状態として使い、bmson entry は `PackageChartEntry` projection に直接 warning を持つ点である。resource health は BMS / bmson 共通なので、pending / install warning projection も `ChartFile` と `BMSFileMaintenanceInfo` から `ChartWarning` list を作り、最終 writeback を `PackageChartEntry.ReplaceWarningsByCategory(...)` に寄せる。
...
1. pending / install warning projection から BMS row mutation を外す。`BmsLibraryInitializationService.LoadInstallTable(...)`、`BmsLibraryPackageInstallService.BuildPendingResourceHealthWarnings(...)`、`BMSLibrary.ReinitializePendingWarningsForPackageUnsafe(...)` の `Func<BMSFile, bool>` / `BMSFile.Warnings` 中間状態を `ChartFile` / `BMSFileMaintenanceInfo` / `ChartWarning` list へ置き換える。
