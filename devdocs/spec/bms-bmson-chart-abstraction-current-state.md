...
playlist reference 表示は、BMS / bmson を分けず `ChartFile` identity から解決する。playlist entry の md5 / sha256 から作る `PlaylistReferenceIndex` を通常一覧 row / virtual source row / playlist detail source row に注入し、lookup は md5 優先、見つからない場合だけ sha256 fallback とする。現状は BMS row だけ `BMSFile.RefTables` / `RefTablesSymbols` / `RefTablesNames` へも書き戻しているが、これは旧 UI 更新 cache であり、ChartFile domain model へ寄せる最終形では削除対象である。最終的な UI 更新は `PlaylistReferenceIndex` の version / table 更新通知と chart row の playlist reference projection invalidation で行い、BMS storage row と bmson storage row のどちらにも playlist reference cache を持たせない。
...
production の旧 compatibility BMSFile adapter surface は削除済みである。bmson は `BMSFile` 継承 adapter に変換せず、BMS storage owner が必要な経路だけ `ChartFile.GetBmsStorageOwner()` / `PackageChartEntry.FromBmsFile(...)` を使う。
...
2026-05-22 時点で、active plan で追っていた adapter materialization boundary は閉じている。一方で、playlist reference については BMS だけ `BMSFile.RefTables` を mutable cache として持つ二重構造が残っており、これは ChartFile domain model への移行を完了する上で新しい大きな compatibility boundary として扱う。現在は audit / cleanup フェーズではなく、`BMSFile.RefTables` 削除を含む playlist reference boundary の統合フェーズである。
...
ただし `BMSFile.RefTables` を削除対象に含めると、playlist reference は未完了の大きな boundary として再分類される。現在の完了度は、当初の active plan 全体ではおおよそ **75〜80%**、`BMSFile.RefTables` 削除まで含めた最終形では **65〜70%** 程度と見る。playlist reference 統合が終われば **85〜90%** まで進み、残りは BMS-only と説明できる参照の監査、test/doc cleanup、必要なら小さな symbol rename になる見込み。
...
- playlist reference の adapter materialization boundary は閉じており、pending bmson のために adapter は materialize しない。一方、BMS storage row への `RefTables` writeback はまだ閉じた境界ではなく、`playlist reference 統合 plan` で削除する未完了 boundary である。外部から任意の `IEnumerable<BMSFile>` を渡して playlist reference を部分適用 / 表示更新する production API は残さず、今後の更新は `PlaylistReferenceIndex` / chart identity / row invalidation へ寄せる。
- library file operation cleanup は `LibraryChartRef` / `PackageChartEntry` / `ChartFile` を入口にする。pending package の install destination cleanup / rewrite は `PackageChartEntry` を直接更新し、installed library row 側の persistent install destination writeback は `LibraryInstallDestinationChange.Chart` から BMS storage owner へ明示的に降りる。
...
- 音声 / 画像 / 動画の resource extension は `ChartResourceExtensions` を正本にする。`BMSFile` にあった `wavExtensions` / `bgaImageExtensions` / `bgaMovieExtensions` などの public resource extension 定数は production 参照がなく、BMS storage row の責務ではなく chart-common resource lookup の責務だったため削除する。`BMSFile.bmsExtensions` は BMS 形式 chart の拡張子であり resource extension ではないため残す。
...
- package install service の post-processing callback は `PackageInstallExecutionResult` を payload にする。storage row upsert / maintenance / library state apply は result の `AddedCharts` から BMS / bmson storage row を派生させて処理し、BMS-only の zero-note / score callback だけが `IEnumerable<BMSFile>` を受ける。これにより bmson install 後処理のために BMSFile list へ adapter を混ぜる入口を残さない。
...
- playlist table replace / external resync 後は `NormalLibraryReferenceTablesChangedReason` で通常一覧 sort key と playlist reference 表示を明示的に無効化する。`PlaylistTableUpdateContext.Updated` は `last_update` 更新有無を表すため、旧 DB 修復や hash 初期化のように playlist entry persistence だけが必要な場合は `ReferenceEntriesChanged` で replace callback を走らせる。現行暫定では BMS row が `RefTables` property change で表示更新され得るが、最終形では BMS / bmson とも `PlaylistReferenceIndex` provider から表示値を読むため、index 更新と row/provider invalidation を UI 更新契約にする。
- folder delete / move / merge に伴う install destination cleanup は、pending package では `PackageChartEntry.Chart.InstallDestination` を読み、library row では `LibraryChartRef` を入口にして BMS storage owner を持つ `ChartFile` だけを `LibraryInstallDestinationChange.Chart` に保持する。`bmson_song` には `instl_dst` 相当の永続列がないため、library bmson row の persistent install destination mutation は新設しない。旧実装では cleanup helper が最初から `IEnumerable<BMSFile>` を受けていたが、現行実装では chart-common operation 境界を保ちつつ、BMS-only persistent writeback は applier 直前の `GetBmsStorageOwner()` に限定する。
...
- playlist reference 表示 / sort / keyword search は `PlaylistReferenceIndex` / chart identity / row projection を正本にし、`BMSFile.RefTables*` と BMSFile playlist reference mutation API は production / tests / docs に残っていない。
