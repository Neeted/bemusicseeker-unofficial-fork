...
5. **Warning / source generation / sort-key の分離: source refresh notification 化済み、storage property fallback 撤去中**
   ...
   - `NormalLibraryRefreshNotificationVersion` handler は normal library source refresh の dispatcher 入口である。handler は notification batch の `NotifiesBmsFiles` / `NotifiesBmsonSongs` coverage を見て BMS row cache prune と bmson row cache sync を先に行い、その後 `SourceChanged` / `WarningPresentationChanged` / `MaintenancePresentationChanged` / `InstallDestinationOverlayChanged` effect で source generation、sort-key invalidation、表示 refresh を決める。`BMSFiles` / `BmsonSongs` property handler は互換通知と cache cleanup のために残すが、`ApplyNormalLibraryRefreshNotification()` や current `OwnedChartCollectionVersion` fallback を呼ばず、main view source refresh の正本として使わない。BMS と bmson の storage row property 通知順が前後しても、source generation と source view refresh は notification handler で一度だけ進める。
