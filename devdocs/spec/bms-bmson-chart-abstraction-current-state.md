...
- main view row source は virtual / non-virtual fallback の両方で `ChartFile` list を入口にする。通常 root / folder view は `CreateStandardLibraryChartSnapshot(...)` で BMS / bmson storage row を `ChartFile` へ投影してから filter / materialize し、BMS row cache へは row factory 内で `ChartFile.GetBmsStorageOwner()` を明示的に降ろす。cache prune も `ChartFile` snapshot を受け、BMS storage owner reference key は `NormalLibraryRowCache` 内部へ閉じる。prune のように owner identity だけが必要な経路では `ChartFileProjection.FromBmsStorageOwnerIdentity(...)` を使い、resource refs / warning / health を含む表示 projection は作らない。garbled / zero-note などの BMS-only subset も UI source selector は `ChartFile` を返し、`ChartListSourceRow` 入口へ BMSFile list を戻さない。
