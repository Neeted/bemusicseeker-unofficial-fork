...
4. **Resource / maintenance target の dispatcher contract 化: 進行中**
   ...
   - full owned target を作る入口は `setOwnedMaintenanceInfo(...)` と explicit full index build に限定する。resource health filter で force full rebuild が必要な時は `FullOnUpdates` と `force_resource_health_filter` reason を明示し、通常の bounded filter / selected rescan は subset delta reason を渡す。
...
5. **Warning / source generation / sort-key の分離: source refresh notification 化済み、storage fallback 縮小中**
   ...
   - `BMSFiles` / `BmsonSongs` property change と通常 refresh 中の bmson source sync は、notification がある場合は batch の owned collection version と effect を見て source generation を消費し、notification がない旧経路だけ current owned collection version に fallback する。`NormalLibraryRefreshNotificationVersion` handler が先に処理した notification は property kind ごとの marker で記録し、後続の `BMSFiles` / `BmsonSongs` handler は自分の coverage が処理済みであれば main view source refresh を繰り返さない。BMS と bmson の通知順が前後しても、同じ owned collection mutationでは source generation と source view refresh を二重に進めない。既に `NormalLibraryRefreshNotificationVersion` handler で消費済みの version で bmson source cache だけが更新された場合は、virtual source row cache を破棄して次回再構築させる。
