...
package entry から adapter が必要な production 操作は、BMS format chart では `GetOrCreateBmsFormatAdapter()`、既存 BMS format adapter / storage owner の読み取りでは `GetExistingBmsFormatAdapter()` を使う。generic な `GetOrCreateCompatibilityAdapter()` と `CompatibilityAdapter` property の production API は削除済みであり、bmson を package entry から新規 `PendingChartEntry` として materialize する入口や、既存 bmson adapter を chart-common state の正本として読む入口は残さない。
...
3. package / pending install entry 境界  
   `ChartPackage.ChartEntries` と `PackageChartEntry.Chart` は package 内 chart の正本になっている。generic な `PackageChartEntry.GetOrCreateCompatibilityAdapter()` と `PackageChartEntry.CompatibilityAdapter` production API は削除済みであり、BMS format chart 用の `GetOrCreateBmsFormatAdapter()` / `GetExistingBmsFormatAdapter()` だけが残る。pending warning / install destination / suggestions / resource snapshot は `PackageChartEntry` と storage row に寄せ、adapter は BMS-only legacy API に降りる境界へ限定する。
