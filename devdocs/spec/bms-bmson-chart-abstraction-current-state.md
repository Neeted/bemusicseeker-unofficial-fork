...
2. **Install destination state の chart-domain 化**
   ...
   - `PackageChartEntry` は BMS owner を持つ entry でも `instl_dst` / title / artist / suggestions を BMS storage row へ同期せず、entry projection state として保持する。表示 / validation / regroup / warning digest が `BMSFile` property を正本にしない状態へ寄せる。
