...
- `PendingCharts` は `ChartEntries` から compatibility adapter を materialize し、`PendingChartEntry` だけに絞った view である。
...
auto install discovery で directory scan 済みの package を明示 chart list 付きで作る時は `ChartPackage.FromChartEntries(...)` を使う。これにより、scan 結果の adapterless bmson entry を `BMSFile` list に戻さず package 正本として保持できる。既存 UI / mutation が adapter を必要とした時だけ `GetChartAdapters()` / `PendingCharts` から materialize する。
