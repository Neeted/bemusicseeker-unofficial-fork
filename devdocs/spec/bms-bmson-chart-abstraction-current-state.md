...
3. package / pending install entry 境界  
   `ChartPackage.ChartEntries` と `PackageChartEntry.Chart` は package 内 chart の正本になっている。一方、`PackageChartEntry.CompatibilityAdapter` / `GetOrCreateCompatibilityAdapter()` が残り、install execution、resource warning、state apply、initialization の一部が adapter を要求する。pending warning / install destination / suggestions / resource snapshot は `PackageChartEntry` と storage row に寄せ、adapter は legacy `BMSFile` API に降りる境界から外す。
...
3. `PackageChartEntry` から compatibility adapter surface を削る。`CompatibilityAdapter` / `GetOrCreateCompatibilityAdapter()` を呼ぶ理由を install execution、state apply、initialization、resource warning の順に消し、entry は `ChartFile` と package transient state の owner にする。
...
- `CompatibilityBmsFile` / `CompatibilityAdapter` / `GetOrCreateCompatibilityAdapter()` は production surface に残っていない。
