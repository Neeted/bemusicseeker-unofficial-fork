...
force install の normal install 上書き確認は、package に `ChartFile.InstallDestination` を持つ entry があるかで判定する。確認ダイアログを出すかどうかだけなら adapter mutation target は不要なので、adapterless bmson entry は確認判定だけでは materialize しない。install 成功後の install destination clear はまだ BMS owner の `instl_dst` へ触る箇所があり、chart-common state への移行対象である。
...
- `BMSFile.instl_dst` / `InstallDestinationTitle` / `InstallDestinationArtist` / `InstallDestinationSuggestions` / popup state が、DB 永続化 state ではなく chart 共通 runtime state として扱われているか。
...
2. **Install destination state の chart-domain 化**
   ...
   - BMS owner へ `instl_dst` / title / artist / suggestions を同期する経路は transitional として扱い、表示 / validation / regroup / warning digest が `BMSFile` property を正本にしない状態へ寄せる。
...
- playlist detail source row の constructor は解決済み `ChartFile` を受け取る。旧実装では `BuildPlaylistSourceRows(...)` が `BMSFile realFile` と `bmson_song resolvedBmson` を tuple として保持し、その二股 payload を constructor に渡していた。現行実装では entry resolve の正本を `ChartFile` にし、constructor 内で BMS / bmson storage owner を `ChartFile` から読む。score probe も `BMSFile realFile` ではなく `ChartFile` の md5 / sha256 を読むため、playlist source row の chart identity 入口を保つ。playlist detail の title / artist / genre / mode / tag / hash / sha256 / path / warning / health / encoding / status / level 表示は、storage owner を個別に読み直すのではなく、まず `CreateChartFile()` が作る `ChartFile` projection を読む。bmson の install destination / warning / health 表示は、ViewModel の `ChartFileTransientState` provider がある場合は provider から再投影し、provider がない場合は constructor へ渡された stateful `ChartFile` snapshot を保持する。これにより、通常の playlist detail build と、テスト / snapshot 再構築のように stateful chart を直接渡す経路の両方で表示 state を落とさない。細かい実装メモとして、`ChartInfo` だけは playlist source 再利用中の hydration 反映を維持するため、owned BMS / bmson storage owner の現在値をまだ読む。これは chart_info index 更新時に missing row だけを patch し、owned row は storage owner の attach 済み chart_info を読む既存挙動を維持するためであり、bmson を `BMSFile` adapter 化する境界ではない。
...
- pending package の install destination projection は BMS entry でも `PackageChartEntry` が保持する。現時点では BMS owner の旧 transient property へも同期しているが、`PackageChartEntry.Chart` は BMS owner から作った `ChartFile` に package state を重ねて返すため、表示 / validation / regroup の正本は entry chart projection へ寄せる。
