...
duplicate view は `DuplicateChartGroups` / `SearchDuplicateChartGroups()` を入口にし、現行 snapshot は owned collection の duplicate row snapshot から作る。ここは full duplicate search という明示的な全件 operation だが、`BMSFiles` / `BmsonSongs` を caller 側で束ねず、owned collection の storage owner identity を軽量 `DuplicateChartRow` へ投影する。install destination runtime overlay は duplicate 判定に混ぜない。通常 merge の existing hash lookup は installed lookup index を使う。
...
- hash/directory/path index: installed lookup、duplicate merge の existing hash、duplicate full search の lightweight row snapshot、install estimation、resource-only merge display package に使う。owned collection の current path/md5 を source にし、`primaryHashCounts`、`primaryHash -> paths`、`md5 -> directories`、`sha256 -> directories`、`known chart directories` を差分更新する。primary hash は md5 identity で、sha256 は owned installed / duplicate / install estimation の fallback identity にはしない。`CreateExcludingLookup` はこの index の primary hash count から作り、merge source 自身を除外する時も full chart snapshot を作らない。duplicate full search は owned collection の storage owner identity から md5-based `DuplicateChartRow` を作り、group materialize までは全件 `ChartFile` rich projection を行わない。resource-only merge display は original package の md5 identity から candidate path を引き、destination 直下の candidate だけを path-only exact lookup で `PackageChartEntry` 化する。この path-only materialize は同一 path の複数 owner も保持し、canonical resolve の ambiguous 扱いとは分ける。
...
| duplicate groups cache | added / removed / moved / digest updated / duplicate warning changed | 当面は duplicate cache invalidate。dirty signal は `DuplicateChartGroupsInvalidationVersion`、result replacement は `DuplicateChartGroups` property change に分ける。incremental duplicate group update は後続最適化でよい。 |
