...
pending package install / resource overwrite 前の再生停止対象は、package 内 `PackageChartEntry.Chart.BmsFile` を持つ BMS storage row だけを snapshot する。これは BMS 再生中の file lock / process を閉じるための UI 側ガードであり、bmson や adapterless entry を再生停止対象にするためだけに materialize しない。
...
- playlist library index は `ChartFile` representative を md5 / sha256 ごとに持つ。旧実装では BMS storage row index を md5 → sha256 の順で解決し、BMS が見つからない場合だけ bmson storage row index を md5 → sha256 の順で解決していたため、entry.md5 が bmson に一致しても entry.sha256 が BMS に一致する場合は BMS が優先された。現行実装でも playlist score resolution が BMS owner を必要とするため、この BMS 優先順は維持する。bmson 同士で同じ hash に一致する場合は旧実装どおり path の ordinal-ignore-case 昇順で代表を選ぶ。
