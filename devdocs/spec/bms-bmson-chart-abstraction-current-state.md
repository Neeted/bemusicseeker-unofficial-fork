...
5. **Warning / source generation / sort-key の分離: 進行中**
   ...
   - `NormalLibraryRefreshNotification` / batch は refresh effect と owned collection version を持つ。dispatcher は source mutation / install destination overlay mutation を notification として publish し、`BMSFiles` / `BmsonSongs` property handler は未処理 notification range を消費して source generation と transient overlay cache を更新する。
