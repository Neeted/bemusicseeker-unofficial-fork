...
- pending zero-note / invalid-extension snapshot は `ChartFile` snapshot を返す。BMS-format の zero-note 判定 / 実ファイル rename だけが package install service 内で `ChartFile.BmsFile` へ降りる。
