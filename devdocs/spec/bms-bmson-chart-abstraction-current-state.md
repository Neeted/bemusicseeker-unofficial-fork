...
duplicate view は `DuplicateChartGroups` / `SearchDuplicateChartGroups()` を入口にし、現行 snapshot は owned collection の duplicate row adjacent index から取得する。初回 search で current owned chart の storage owner identity を軽量 `DuplicateChartRow` へ投影し、以後は remove / path change / upsert / MD5 digest update の collection mutation に同期する。検索 API は adjacent index から per-call の list snapshot を返すため、chart_info などの background digest update と duplicate analyze が同時に走っても、分析中の row collection は変化しない。ここは full duplicate search という明示的な全件 operation だが、`BMSFiles` / `BmsonSongs` を caller 側で束ねず、行 identity snapshot を owned collection 側に保持する。install destination runtime overlay は duplicate 判定に混ぜない。通常 merge の existing hash lookup は installed lookup index を使う。
