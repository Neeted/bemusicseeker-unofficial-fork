...
2. **Install destination state の chart-domain 化**
   ...
   - 2026-05-22 現在、package entry と ViewModel row 表示 / loose chart transient cache の境界は chart-common state へ寄っている。残る主要対象は model-layer の folder cleanup / repair / startup cleanup に残る BMS owner `instl_dst` 参照である。特に `LibraryInstallDestinationChange.GetCurrentInstallDestination()` の BMS fallback、`EnumerateInstallDestinationTargetsUnderFolder(...)` の BMS-only scan、`BmsLibraryStateApplier.ApplyLibraryMutationDelta(...)` の BMS owner writeback、`FixInstallationDirectory(...)` の BMS owner clear は、DB 永続化ではなく runtime state 更新として再分類し、可能な範囲で `ChartFile` / `PackageChartEntry` / ViewModel transient state へ寄せる。
...
playlist reference 統合と `BMSFile.RefTables*` legacy API cleanup が終わった一方で、`BMSFile` 内部には install destination / warning / chart_info / maintenance-resource health の chart-common state がまだ残っている。install destination については package entry と ViewModel transient cache への移行が進み、通常表示 / playlist detail / loose edit の正本は chart state 側へ寄っているが、model-layer の folder cleanup / repair / startup cleanup には BMS owner fallback が残る。現在の完了度は **90% 前後** と見る。残りは `BMSFile` 参照数の削減ではなく、`BMSFile` member surface を BMS-only / LR2 storage row / BMS parser / score producer として説明できる状態へ寄せる作業である。
...
- library file operation cleanup は `LibraryChartRef` / `PackageChartEntry` / `ChartFile` を入口にする。pending package の install destination cleanup / rewrite は `PackageChartEntry` を直接更新する。installed library row 側の install destination mutation は DB 永続化ではなく chart runtime state の更新として扱い、BMS storage owner へ `instl_dst` を書き戻す経路は移行途中の互換 surface として監査対象にする。ViewModel の表示 cache は BMS / bmson 共通の `ChartFileTransientState` へ寄せているが、model-layer から ViewModel cache へ mutation delta を戻す経路はまだ完全ではない。このため、BMS owner `instl_dst` writeback は「保存契約」ではなく「残存 runtime fallback」として扱い、次の install destination audit で閉じる。
