...
この節は、層を薄く順番に chart-native 化するのではなく、production に残っている compatibility 境界を境界単位で閉じる方針で進めた active migration の完了状態を記録する。
...
`startup_background_summary elapsedMs` が 60000ms 以上なら異常として扱い、抽象化作業を進める前に原因を直す。ただし background tail だけを見て初期化全体の悪化を見落とさない。抽象化作業前の large library 起動では `startup_ready_operable elapsedMs=23405` 程度で、Everything scan が約 20 秒ならその数秒後に operable になるのが期待値である。`startup_ready_operable` が 30 秒台後半へ伸びる場合は、song table load / file diff / install destination cleanup / initial main view build / first render など前段を含めて regression として扱う。`bin\Release\net472\install-performance.log` は append-only なので、過去の悪い cycle や最大値だけではなく最新の完了済み起動 cycle を baseline と比較する。特に次の値は regression guard として見る。
...
- `startup_ready_operable` / `startup_background_summary`: 起動操作可能化と背景処理完了の総時間。Everything scan が 20 秒前後なのに `startup_ready_operable` が 30 秒台後半になる場合は、background task ではなく前段の projection / diff / cleanup / initial view build も疑う。
...
- `custom_table_render`: 初回 `items_source_changed` の `renderWorkMs` と breakdown。`rowHighlightMs` が大きい場合は WARNING / resource health projection の同期 build、`cellValueMs` が大きい場合は row getter / reflection、`textLayoutMs` / `drawTextMs` が大きい場合は visible cell text rendering を疑う。render 比較は visible row / column / cell 数が違うと単純比較できないため、`visibleCellCount` と併記して判断する。
...
2026-05-23 の最終監査後の最新完了 cycle では、`song_tbl_load_projection readMs=3327 materializeMs=3317`、`song_tbl_load_breakdown bmsfiles_assign_ms=6`、`everything_scan totalMs=18296`、`song_tbl_file_check_breakdown scan_ms=18395 apply_ms=254 instl_dst_cleanup_ms=157`、`init_library total_ms=19442`、`startup_ready_operable elapsedMs=22256`、初回 `main_view_build totalMs=1177`、初回 `custom_table_render renderWorkMs=184 rowHighlightMs=44 visibleCellCount=155`、`startup_background_summary elapsedMs=42084 failed=0` であり、抽象化作業前の `startup_ready_operable elapsedMs=23405` / `startup_background_summary elapsedMs=47923` からの regression は出ていない。過去 cycle には `startup_ready_operable` が 30 秒台後半へ伸びたログが残るが、最新 cycle では解消済みである。今後の性能確認では同じ DB / library roots / Release net472 build の最新完了 cycle を比較し、必要に応じて複数 run の中央値を見る。

### 現在のフェーズ

2026-05-23 時点で、active plan で追っていた adapter materialization boundary は閉じている。playlist reference についても、BMS storage row の mutable cache と `PlaylistReferenceIndex` の二重構造を解消し、BMS / bmson とも `ChartFile` identity と `PlaylistReferenceIndex` を正本にしている。

`BMSFile` の型参照と member surface は、BMS storage row / LR2 `song` row / BMS parser / BMS player / score・IR・encoding・zero-note producer として説明できる範囲に整理されている。`BMSFile.Warnings` は BMS-only producer state として残るが、表示・sort・keyword search は `ChartFile.Warnings` / `ChartWarningProjectionFormatter` / `ResourceHealthWarningProjection` を読む。chart_info attachment、install destination runtime property、warning 表示 alias、maintenance health / encoding 表示 alias は storage owner から分離済みである。

### compatibility 境界の監査結果

2026-05-23 現在、active plan で明示的に追っていた `CompatibilityBmsFile` / `PendingChartEntry : BMSFile` / playlist reference cache のような構造互換 boundary は閉じている。残存 `BMSFile` 参照と `BMSFile` member surface は意味で監査し、BMS-only / LR2 storage row / BMS parser / score producer として説明できる状態になっている。

今後同じ領域を触るときは、次を regression audit として維持する:
...
以下は 2026-05-23 の最終監査で確認済みの分類である。今後の変更で再び compatibility surface が見つかった場合は、helper 単位ではなく audit category 単位で閉じる。
...
3. **BMSFile member surface の chart-common 責務 audit**
   ...
   - maintenance health / encoding の表示 alias は `BMSFile` から削除済みである。`BMSFile` は `maintenanceInfo` の所有・由来管理、BMS parser 由来の resource refs、encoding fix producer を持つ。resource health の production 更新は `BmsLibraryMaintenanceService.BuildResourceHealthMaintenanceInfo(ChartFile, ...)` を通り、BMS owner 由来の refs が必要な時だけ `ChartResourceSnapshot.Create(ChartFile)` 内で BMS parser resource refs へ降りる。`ChartFileProjection.FromBmsFile(...)` は有効な `maintenanceInfo` snapshot を直接読んで `ChartFile` へ投影し、lazy placeholder は表示正本として作らない。`BMSFile` からの通知は `maintenanceInfo` 変更に集約し、row 側で health / encoding 表示 property へ展開する。
   ...
   - `BMSFile` の BMS parser / encoding / zero-note / resource hash helper は production で必要だが、アプリ外部互換 API ではない。`CreateBMSFileFromFile(...)`、`SetBMSComponentFilesFromBMSFile(...)`、`ReloadBMSFileWithEncoding(...)`、`DetectEncodingOfBMSFile(...)`、`IsZeroNoteBMSFile(...)`、`MaintenanceInfoOrigin`、`SetMaintenanceInfo(...)` などは app internal surface に閉じ、chart-common operation へ公開する入口にしない。`ChartResourceKeyHash` も resource lookup 用の内部 helper として扱い、外部 public API にはしない。UI / BMS player の binding に必要な `status` / `BMSFileStatus` や LR2 song row の basic property は public のまま残す。
   - playlist table context menu の「ローカルBMSの難易度をこの表で上書き」は、playlist entry の level を LR2 `song.level` に反映する互換機能である。playlist 参照表示や playlist entry 解決は chart-common だが、この writeback は bmson_song の level を更新する chart-common 操作ではないため、`BMSFile` / LR2 song row 境界に残す。UI 文言も既存機能名として変えない。
...
- bmson resource health 計算は `ChartResourceSnapshot` と `ResourceHealthLookupContext` を使い、directory resource index を優先し、missing cache entry の時だけ実ファイル確認へ fallback する。旧実装の `PendingChartEntry.SetHealthStatusUsingLookupContext(...)` が内部で使っていた cache-hit / fallback counter は維持するが、`BMSFile` の旧 health mutation / `memClear` 副作用は bmson storage row には移植しない。
- `ChartFile` は chart domain model として audio / visual resource refs と optional image refs（stagefile / backbmp / banner）を持つ。production の resource-health / package / estimation flow は `ChartFile` projection から `ChartResourceSnapshot` を作るため、bmson flow が BMSFile adapter へ戻る必要はない。`ChartResourceSnapshot.Create(bmson_song)` は parser / test など storage owner 直近の snapshot 作成用 helper としてまだ残るが、chart-common operation surface へ `bmson_song` 入口を広げるものではない。BMS は parser 由来 resource refs を `BMSFile` に保持するが、chart-common operation では projection 済み refs を正本にする。
- `ChartResourceSnapshot.Create(ChartFile)` は `ChartFile` に投影済みの resource refs を優先し、BMS owner の parser refs へ降りるのは projection に refs が無い場合だけにする。`ChartResourceSnapshot` の lookup key は拡張子 alias 正規化後に拡張子を落とすため、chart-common resource health 計算では movie も `ChartResourceExtensions.MovieExtensions` を使って existence check する。旧 BMS-only `SetHealthStatus(...)` は production / test の双方から削除済みであり、pending package の resource health 判定は `BuildResourceHealthMaintenanceInfo(ChartFile, ...)`、installed maintenance / playlist org_md5 補助探索の BMS storage owner 境界は missing-file guard 付きの `BuildBmsResourceHealthMaintenanceInfo(...)` wrapper から同じ chart-common 計算へ入る。
...
- aggregate resource snapshot の production 入口は `IEnumerable<ChartFile>` のみである。旧 `IEnumerable<BMSFile>` overload は production 参照がなく、テストだけの旧入口になっていたため削除する。単一 BMS chart の parser refs 読み取りは `ChartResourceSnapshot.Create(ChartFile)` 内部の BMS storage owner 分岐に閉じ、package / estimation / resource surface の集約は `ChartFile` list を正本にする。
...
- pending package discovery、install table load、pending regroup 後の warning 再初期化では、bmson の `ResourceHealth` warning は `PackageChartEntry` に直接書き戻す。BMS entry も warning classification は `ChartFile` projection から一時的に組み立てた resource health snapshot を使い、BMS owner の `maintenanceInfo` を中間 state として読まない。discovery 時に BMS parser resource refs を読むことはあるが、pending warning 判定の正本は `PackageChartEntry.Chart` である。旧実装のような `Func<BMSFile, bool>` callback や `BMSFile.Warnings` を pending package warning の中間 state として使う経路は残さない。
...
以下は最終的に満たす判定条件である。2026-05-23 の監査時点では、active migration plan で追っていた production compatibility boundary は閉じている。`BMSFile.Warnings` / `maintenanceInfo` のように chart 表示へ投影される state は残るが、BMS-only producer / LR2 storage owner の state として分類でき、chart-common 表示・operation の正本は `ChartFile` / projection provider / transient state / package entry / `PlaylistReferenceIndex` 側にある。
