...
5. **Warning / source generation / sort-key の分離: 進行中**
   ...
   - `NormalLibraryRefreshNotification` / batch は refresh effect、owned collection version、後続の storage row property notification 有無を持つ。dispatcher は source mutation / install destination overlay mutation を notification として publish し、ViewModel は `NormalLibraryRefreshNotificationVersion` handler または `BMSFiles` / `BmsonSongs` fallback handler で未処理 notification range を消費して source generation と transient overlay cache を更新する。
   - `BMSFiles` / `BmsonSongs` property change と通常 refresh 中の bmson source sync は、notification がある場合は batch の owned collection version と effect を見て source generation を消費し、notification がない旧経路だけ current owned collection version に fallback する。BMS と bmson の通知順が前後しても、同じ owned collection mutation では source generation を二重に進めない。既に `NormalLibraryRefreshNotificationVersion` handler で消費済みの version で bmson source cache だけが更新された場合は、virtual source row cache を破棄して次回再構築させる。
   - batch の legacy property coverage は effect 別に判定する。`NotifiesStorageRows` は source generation の storage fallback marker として残すが、install destination overlay の表示 refresh は storage row property で covered 扱いにしない。overlay state は `NormalLibraryRefreshNotification` 側で必ず消費し、overlay-only notification と storage-row notification が同じ未処理 range や同じ mutation result に混在しても overlay refresh を隠さない。
   - internal mutation 由来の storage row property notification は、該当 kind の owner collection が実際に変わる場合に限定する。`RaiseLibraryChartsChanged` は root move などの source refresh intent であり、bmson-only path change のために `BMSFiles` を汎用 library-changed signal として叩かない。source refresh の正本は `NormalLibraryRefreshNotification` と owned collection version へ寄せ、`BMSFiles` / `BmsonSongs` は kind-specific storage owner boundary として残す。
   - install destination overlay だけの `RaiseLibraryChartsChanged` は source generation を進めず、`install_destination_changed` sort-key invalidation と `InstallDestination` data dependency の表示 refresh に閉じる。install destination sort column は identity sort ではなく overlay projection dependency として扱い、overlay 変更時は source row rebuild ではなく現在 view の依存判定で display refresh / full refresh を選ぶ。install destination state は install-estimation warning projection にも影響するため、warning sort 中は再ソートする。
   - install destination overlay の変更は refresh notification の `InstallDestinationOverlayChanged` effect として version 付き queue に publish する。overlay changed chart がある場合はその chart subset を載せ、storage add/remove のように chart subset がなく current storage row に prune するだけの mutation は effect だけを載せる。
   ...
   - 現状では source / overlay / maintenance producer 由来の warning・maintenance sort-key invalidation は refresh notification の effect と normal library refresh notification version で管理している。notification batch は後続の legacy storage / warning / maintenance property notification があるかを effect 別に持ち、同じ effect の全 notification が legacy property で覆われている場合だけ notification 側の sort-key invalidation / 表示 refresh を抑止する。ViewModel は legacy property notification がない warning / maintenance effect を notification 側で処理する。resource warning ignore / unignore、chart_info index 更新、zero-note 再確認、chart_info parse failure 更新、manual resource maintenance、maintenance hydration、internal storage / hash mutation は warning / maintenance effect を publish する。zero-note filter の source getter は残すが、zero-note 再確認の表示更新は property handler ではなく notification effect で行う。external `BMSFiles` / `BmsonSongs` replacement は source refresh と active maintenance filter refresh で chart_info parse failure subset を更新し、setter から `ChartInfoParseFailedChartFiles` を個別通知しない。maintenance hydration completed version は startup progress 用に限定し、resource health / maintenance 表示更新は refresh notification から行う。ただし maintenance hydration は `ChartFilesNeedResourceFix` / `ChartFilesNeedResourceFixIgnored` / `BMSFilesGarbled` / `BMSFilesGarbledFixed` / `ChartInfoParseFailedChartFiles` property notification を互換 filter refresh としてまだ併発するため、warning presentation effect は property-covered として扱う。duplicate cache invalidation は `DuplicateChartGroupsInvalidationVersion` へ分離済みで、search result replacement だけ `DuplicateChartGroups` property notification を使う。残る warning subset property notification は次の段階で refresh effect queue または後継 notification へ寄せ、warning-only / maintenance-only の表示更新を個別 property handler に依存しない形へ移す。
...
6. **Full snapshot helper の分解: 完了扱い、監査継続**
   ...
   - 残る全件 `ChartFile` materialize は chart_info full backfill、resource maintenance full target / full rebuild、duplicate full search のような明示 full operation に限定する。hot path の caller 側 full materialize 後 filter は見つかっていないため、今後は新規追加を監査する。
