...
- estimated install batch の deferred maintenance state は BMS と bmson を分け、BMS は `DeferredBmsMaintenanceTargets`、bmson は `DeferredBmsonMaintenanceSongs` に保持する。`setMaintenanceInfo(...)` がまだ `BMSFile` API のため、bmson の一時 `PendingChartEntry` は `BuildEstimatedInstallMaintenanceTargets(...)` の `setMaintenanceInfo` 直前だけで作る。旧実装では deferred target list 自体に bmson adapter を混ぜていたが、batch result の状態としては `bmson_song` を正本にする。inline chart_info は direct deferred bmson songs と installed package から解決した bmson songs を path dedupe して渡し、resource-only display package 由来の既存挙動を落とさない。
