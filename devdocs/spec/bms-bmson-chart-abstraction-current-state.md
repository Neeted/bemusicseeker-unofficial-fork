...
導入先推定 snapshot builder の primary input は `PackageChartEntry` である。`BMSFile` compatibility adapter list を受け取る overload は残るが、互換入口として即座に `PackageChartEntry` へ変換する。
