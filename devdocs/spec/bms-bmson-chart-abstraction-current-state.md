...
- playlist library index は `ChartFile` representative を md5 / sha256 ごとに持つ。旧実装では BMS storage row index を md5 → sha256 の順で解決し、BMS が見つからない場合だけ bmson storage row index を md5 → sha256 の順で解決していたため、entry.md5 が bmson に一致しても entry.sha256 が BMS に一致する場合は BMS が優先された。現行実装でも同じ代表選択順を維持し、playlist score resolution は選ばれた `ChartFile` identity の md5 / sha256 を読む。bmson 同士で同じ hash に一致する場合は旧実装どおり path の ordinal-ignore-case 昇順で代表を選ぶ。
