...
5. **Warning / source generation / sort-key の分離: 進行中**
   ...
   - 現状では source / overlay は refresh notification の effect と normal library refresh notification version で管理している。次の段階では warning / maintenance property handler も同じ effect queue または後継 notification へ寄せ、warning-only / maintenance-only の表示更新を `BMSFiles` / `BmsonSongs` property handler に依存しない形へ移す。
