...
pending chart 削除の UI 経路は `ChartOperationTarget.Chart` を `BMSLibrary.RemovePendingCharts(IEnumerable<ChartFile>)` へ渡し、model 層では `BmsLibraryPackageInstallService.DeletePendingCharts(...)` が path と任意の BMS compatibility file を持つ deletion target に正規化する。adapterless bmson は `ChartFile.Path` でファイル削除と pending package mutation を行い、削除成功時は `PendingFileDeletionResult.ChartPathsToRemove` に載せるため、削除のためだけに `PackageChartEntry.GetOrCreateCompatibilityAdapter()` を呼ばない。既存の `BMSFile` payload 入口は standalone / legacy caller 用に同じ内部処理へ合流している。
