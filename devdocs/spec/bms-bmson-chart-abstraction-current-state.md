...
2. **Install destination state の chart-domain 化**
   ...
   - 2026-05-22 現在、package entry と ViewModel row 表示 / loose chart transient cache の境界は chart-common state へ寄っている。`LibraryInstallDestinationChange.GetCurrentInstallDestination()` は `Entry.Chart` / `Chart` を読むだけにし、folder move / merge の target enumeration と BMS player temporary install fallback も `ChartFile.InstallDestination` を読む。repair fix 後の BMS install destination clear と deleted-folder cleanup 後の BMS install destination full clear は、service 内の直接 mutation ではなく `LibraryMutationDelta.UpdatedInstallDestinations` に載せる。ただし、folder move / merge の library row enumeration は `LibraryChartRef.ToChartFile()` projection を通るため、現時点で拾える loose library chart の install destination は storage-backed projection と `PackageChartEntry` state に限られる。ViewModel transient cache にだけ存在する bmson install destination を model-layer へ持ち込む経路はまだ未完了である。残る主要対象は `BmsLibraryStateApplier.ApplyLibraryMutationDelta(...)` の BMS owner writeback、startup cleanup に残る BMS owner `instl_dst` 参照、ViewModel transient state を mutation input / output として運ぶ経路である。これらは DB 永続化ではなく runtime state 更新として再分類し、可能な範囲で `ChartFile` / `PackageChartEntry` / ViewModel transient state へ寄せる。
...
playlist reference 統合と `BMSFile.RefTables*` legacy API cleanup が終わった一方で、`BMSFile` 内部には install destination / warning / chart_info / maintenance-resource health の chart-common state がまだ残っている。install destination については package entry と ViewModel transient cache への移行が進み、通常表示 / playlist detail / loose edit、BMS player temporary install fallback の正本は chart state 側へ寄っている。folder move / merge / deleted-folder cleanup も `ChartFile.InstallDestination` と `LibraryMutationDelta` を読むが、`LibraryChartRef` projection が ViewModel transient cache を保持しないため、loose bmson の transient install destination を完全に扱える状態ではない。model-layer の state apply / startup cleanup には BMS owner fallback も残る。現在の完了度は **90% 前後** と見る。残りは `BMSFile` 参照数の削減ではなく、`BMSFile` member surface を BMS-only / LR2 storage row / BMS parser / score producer として説明できる状態へ寄せる作業である。
...
- library file operation cleanup は `LibraryChartRef` / `PackageChartEntry` / `ChartFile` を入口にする。pending package の install destination cleanup / rewrite は `PackageChartEntry` を直接更新する。folder move / merge の library row 側 target enumeration は `LibraryChartRef.ToChartFile()` で作った `ChartFile.InstallDestination` を読む。ただし `LibraryChartRef` は ViewModel transient state を保持しないため、transient cache にだけある loose bmson install destination はこの経路へまだ届かない。repair fix の BMS install destination path clear と deleted-folder cleanup の BMS install destination full clear は `LibraryMutationDelta.UpdatedInstallDestinations` として返し、state applier が storage owner mutation を担当する。deleted-folder cleanup の full clear は path だけでなく title / artist / suggestions / popup / install-estimation warning を消す必要があるため、`LibraryInstallDestinationChange.ClearInstallDestinationState` で path-only update と区別する。installed library row 側の install destination mutationは DB 永続化ではなく chart runtime state の更新として扱い、BMS storage owner へ `instl_dst` を書き戻す経路は移行途中の互換 surfaceとして監査対象にする。ViewModel の表示 cache は BMS / bmson 共通の `ChartFileTransientState` へ寄せているが、model-layer から ViewModel cache へ mutation delta を戻す経路はまだ完全ではない。このため、BMS owner `instl_dst` writeback は「保存契約」ではなく「残存 runtime fallback」として扱い、次の install destination audit で閉じる。
