...
- `BuildStandardLibraryRowsForView(...)` は production と test の両方で row factory 付き overload を使う。旧 test-only overload は `BMSFile` list を渡すだけで `LibraryChartRow.FromBmsFile(...)` を暗黙に使う入口だったが、production の cache / row materialization 境界と違う BMSFile-list API surface を残すだけだったため削除する。通常一覧の storage snapshot 入力として BMS row list が残るのは、LR2 song row を `LibraryChartRow` に materialize する境界に限定する。
