...
- playlist library index はまだ md5 / sha256 lookup の内部 dictionary に BMS storage row と bmson storage row を分けて持つ。playlist detail source row は `ChartFile` を正本にできているため、後続 cleanup では index も `ChartFile` representative へ寄せる余地がある。
