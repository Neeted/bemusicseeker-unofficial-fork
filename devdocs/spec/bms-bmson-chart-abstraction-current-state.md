...
pending package install 前の再生停止対象は、package 内 `PackageChartEntry` の既存 BMS compatibility adapter だけを snapshot する。これは BMS 再生中の file lock / process を閉じるための UI 側ガードであり、bmson や adapterless entry を再生停止対象にするためだけに materialize しない。
