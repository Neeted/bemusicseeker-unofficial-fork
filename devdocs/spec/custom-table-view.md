...
| playlist summary | 既存 row model | 既存 materialized sort | 各画面固有 | 次フェーズで registry/cache contract 適用対象を分解する |
