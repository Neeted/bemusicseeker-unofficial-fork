...
`BMSFilesView` は `List<LibraryChartRow>` だけでなく、`IChartListViewMetadata` を実装した仮想 `IList` になり得る。通常ライブラリの default 表示と、その状態からの `ChartListOrder` registry 対応列 sort では、`ChartListSourceRow` と `ChartListOrder` を全件分作り、`CustomTableView` からの `Count` と index access に応じて可視行だけ `LibraryChartRow` を生成する。現行 registry は identity / install destination / ref-table 系に加えて、warning digest (`WarningDigestText`)、score 系 (`clear`, `rateDouble`, `score`, `maxcombo`, `minbp`, `rankingString`, `rankingLastupdate`, `stddevVal`, `scoreDifficulty`)、chart_info 系 (`ChartLevelSortKey`, BPM, duration, judge, feature, notes, TOTAL, density, soflan count など)、maintenance 直読列 (`WAVHealth`, `BGAHealth`, `MovieHealth`, `encoding`) を含む。`rank` は既存 sort と同じく `rateDouble` の alias として扱う。

仮想 `ChartListOrder` は列定義レジストリから Asc / Desc の order を作る。レジストリは normalized column name、alias、key kind、key selector、sort profile、prewarm priority、main view data dependency を持ち、仮想 sort 対応列と通常 sort cache 候補、refresh dependency 判定の source of truth になる。通常ライブラリの `STANDARD` 初期可視列で `SortMemberPath` を持つ列は、すべてこの registry で正規化できることをテストで固定する。全 main column を可視化した場合でも、playlist detail 専用の `EntryLevelSortKey` 以外は registry 対応列でなければならない。文字列比較は `StringComparer.OrdinalIgnoreCase`、`mode` や score / chart_info 数値列は typed nullable 比較で、Desc は Asc の反転ではなく対象 key 降順 + `Title` 昇順の secondary key とする。order cache の正当性は `sourceGeneration + sortKeyGeneration + dependency generation + column + direction + rowCount` で保証する。`IdentitySortKey` は追加 generation なし、`Score` は `ScoreSnapshotVersion`、`ChartInfo` は `ChartInfoIndexVersion`、`Maintenance` は maintenance hydration generation を使う。`Warning` は source warning / resource health projection / install destination の合成値なので、該当 mutation 経路で `warning_changed` または既存の sort-key invalidation を必ず発行する。fingerprint 再走査は行わない。`Title` / `path` / `Folder` / install destination / ref-table 表示など sort 対象値が変わる経路は、row cache の実体化状態に依存せず mutation source 側で必ず `sortKeyGeneration` を進める。score / chart_info hydrate は identity order cache を stale にしないが、対応 dependency の order cache は generation 差で再構築される。

仮想 order prewarm は registry の priority とユーザーの現在表示列から対象を決める。priority 1 は `Title` / `Folder` / `path` / `Artist` で、非表示でも常に prewarm する。priority 2 は `clear` / `rateDouble` / `minbp` / judge / notes / BPM / soflan / total / duration / density 系で、該当列が表示されている場合だけ prewarm する。`maxcombo` を含むその他の仮想対応表示列は priority 3 とし、非表示なら on-demand のみとする。`WarningDigestText` と maintenance 直読列は、現段階では仮想 sort 対応のみで prewarm 対象にはしない。表示列の判定は `CustomTableColumnFactory.CreateMainColumns(ColumnsSettingsBMSFilesView)` の `SortMemberPath` を `ChartListOrder` registry で正規化して行うため、`Rank` と `Rate` のように同じ sort key へ alias される列は重複 prewarm しない。

通常ライブラリの folder filter、keyword filter、mode filter は、まず全件 source row に対する現在 sort order を取得し、その index 列を `ChartListSourceRow` 上の predicate で絞り込む。filter 変更時に同じ sort column / direction の全件 order cache が有効なら、filter subset に対して `ChartListOrder.TryCreate(...)` を再実行しない。keyword filter は identity / playlist reference / score / chart info field を通常の文字列検索・regex 検索とも source row から直接読むため、summary cache の filter identity には score snapshot version と chart info index version を含める。`FileMissing` / `Garbled` / `Unregistered` / `ZeroNote` / `ChartInfoParseError` など BMSFile 集合をそのまま表示する maintenance 系 tree mode と、`DuplicateFilterSelected` の全件 / group / folder / list 選択も、同じ `ChartListSourceRow` + `ChartListVirtualView` 経路で可視行だけを実体化する。playlist detail、install 系など未対応の表示条件は既存の materialized 経路を使う。通常ライブラリの仮想候補 request が materialized 経路へ落ちる場合は、`main_view_virtual_fallback` に `unsupported_sort_column` や `keyword_requires_materialized` などの reason を出し、fallback 境界をログから追えるようにする。
...
| 通常ライブラリ BMSFile subset | `ChartListSourceRow` + `ChartListVirtualView` | `ChartListOrder` registry | subset source rows、column、direction | `FileMissing` / `Garbled` / `Unregistered` / `ZeroNote` / `ChartInfoParseError` / duplicate 系。order cache は現段階では root/filter のみ |
...
| install / playlist summary | 既存 row model | 既存 materialized sort | 各画面固有 | 次フェーズで registry/cache contract 適用対象を分解する |
