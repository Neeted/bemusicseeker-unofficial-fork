...
| 通常ライブラリ BMSFile subset | `ChartListSourceRow` + `ChartListVirtualView` | `ChartListOrder` registry | subset source rows、column、direction | `FileMissing` / `Garbled` / `Unregistered` / `ZeroNote` / `ChartInfoParseError` 系。order cache は現段階では root/filter のみ |
