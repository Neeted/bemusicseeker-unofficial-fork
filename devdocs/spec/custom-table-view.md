...
| 通常ライブラリ BMSFile subset | `ChartListSourceRow` + `ChartListVirtualView` | `ChartListOrder` registry | source generation、sort-key generation、dependency generation、tree mode、subset name、source row signature、column、direction、row count | maintenance / duplicate / install package 系。同じ subset 並びの keyword / mode / sort 更新では order cache を再利用する |
...
- `main_view_build`
  - 通常ライブラリ / FullScanAllCharts / BMSFile subset の仮想経路は `sortEngine=virtual`, `virtual=True`, `viewRowsCreated` が可視範囲程度になる。
