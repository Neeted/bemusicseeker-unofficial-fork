...
自動・推定先・強制導入、manual shared core、resource-only / cleanup-only 導入は operation-scoped install session へ移行済みであり、package ごとの executor durable receipt を確定境界にしない。auto rename、手動 rename / 複数 folder move、library chart delete、通常 invalid-extension rename、導入先修正も session を使う。merge 等に残る既存 executor helper の限定補償契約と、移行済み導入 route の session terminal は区別する。pending-only invalid-extension rename は owned catalog を変更しないため package/install lifecycle route のまま維持する。multi-change 操作の恒久契約は [Mutation session 契約](#mutation-session-契約) とし、immutable な preflight / destination type guard、source保全等の有用な局所安全策は維持してよい一方、DB durable receiptをitemごとに作ること自体は要件ではない。session移行後は、確認済みphysical successをchangeとして蓄積し、canonical durable applyをoperation境界へ集約する。source cleanupをdurable apply後へ遅延できる操作ではsession終端まで遅延する。
