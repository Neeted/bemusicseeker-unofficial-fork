...
| 契約 | 実装 | 検証・識別する誤実装 |
| --- | --- | --- |
| `S5-INSTALL-ADMISSION`: 通常変更の相互排他、確認取消・実行失敗後の解放 | `PendingPackageWorkflowOwner.TryEnterPendingOperation` / `InstallPendingAsync` / `InstallPackagesAsync`、`PackageInstallWorkflowOwner.TryEnqueue` | `PendingPackageWorkflowOwnerTests.PendingInstall_RejectsDropDuringConfirmationAndReleasesAdmission` / `DropQueue_RejectsPendingInstallUntilAllAcceptedBatchesFinish`。後者は worker 開始前・各 batch 実行中の拒否と、全 batch 終了後の完了通知時の受付解放を分けて検証する。別 gate の使用、確認後だけの受付、未受理対象への副作用を識別する |
| `S5-INSTALL-QUEUE`: 予約時から drain 終端まで一つの lease | `PackageInstallWorkflowOwner.TryEnqueue` / `PublishQueueStatus`、既存 `DropInstallQueueProcessor` | `PackageInstallWorkflowOwnerTests.Enqueue_RejectsBusyBeforeQueueingThenRunsAfterRelease` / `GateBusy_AbandonsOwnedIngressWithoutCallingInstaller` / `Enqueue_PublishesCompletionAfterLiveInstallReturnsAndContinuesAfterFailure`、同 fixture の generation / cancel / shutdown tests。worker 開始時だけの排他、受理済み予約の消失、遅い通知による解放、後続 batch 実行中の先行 terminal 発行を識別する |
| `S5-INSTALL-HANDOFF`: URL / API 取得成功と導入未受理を分離 | `PlaylistWorkspaceViewModel.QueuePlaylistUrlInstallPathsAsync` → `PackageInstallWorkflowOwner.Enqueue` | `PlaylistUrlAcquisitionOwnershipTests.DownloadedPackages_BusyHandoffWarnsWithoutFallbackOrReplay`（single / url / diff / api）。通信中は通常変更を許可し、引渡しだけを拒否する。一般例外化、ブラウザ fallback、再実行、source 削除を識別する |
| `S5-INSTALL-NOTIFICATION`: 非同期情報表示と mutation 結果の分離 | `FileDbMutationReport.ShowOperationMessagesAsync`、保留 / 自動導入 owner の終了処理 | `FileDbMutationReportTests.OperationMessages_PreserveOrderAndContinueAfterDisplayFailure`、`PackageInstallWorkflowOwnerTests.OperationDialogs_AreDispatchedWithoutBlockingQueueOrChangingMutationResult` / `DurableFinalizationFailure_PublishesTypedCompletionWithoutRegisteredPackages`（型付き session 異常結果と終了処理例外の terminal も受付解放後）、`PendingPackageWorkflowOwnerTests.SearchPackagesAsync_EndActivityFailureStillDetachesAndDispatchesDialogScope` / `SearchPackagesAsync_PreservesRequiredFailuresButDoesNotPromoteNotificationFailure` |
| `S5-INSTALL-BUSY-TEXT`: 汎用の競合拒否と queue 未受理の多言語表示 | 共通 resource accessor / resx / 全言語 JSON、各既存 UI terminal | `LocalizationResourceParityTests.InstallAdmissionMessages_AreAvailableWithoutFormatArgumentsInAllLanguages`、既存 resource parity と model / owner Busy tests。全文ではなく key・非空・placeholder parity と拒否結果を検証する |

テストは既存 fixture と local DB / FS / dialog port を使い、owner Task、queue idle、通知 Task / dispatcher の明示 drain を完了境界とする。受付のための全 catalog / resource index の追加走査・snapshot は行わず、S5 session の変更収集・DB apply 回数・操作内並列度は変更しない。
...
`LMS-S5-20260915` は、一回の利用者操作を一つの install session にする契約の識別子とする。先行 physical success を後続分類へ使うことと、canonical durable apply / publication を操作終端へ集約することを両立する。実装は `BMSLibrary.PackageInstall.cs` と `BmsLibraryPackageInstallService` の session route、検証は次の既存 fixture に置く。

| Contract ID | production ingress -> observable / durable impact | 必須結果 | 許容差分 / 検出すべき誤実装 | 検証 |
| --- | --- | --- | --- | --- |
| `S5-SESSION-AGGREGATE` | auto / pending estimated / force / manual shared install / resource overwrite -> catalog・install rows・package collections | 一つの user operation は一つの install session で canonical storage/install-row apply と required package publication を operation 単位に確定する | helper 名、入力順 physical I/O は可。package ごとの canonical apply / synthetic durable item receipt を検出する | `BmsLibraryPackageInstallServiceTests`、`PendingPackageWorkflowOwnerTests` |
| `S5-SUCCESS-OVERLAY` | 複数 package install -> 後続 duplicate / ownership 判定 | operation 開始 snapshot に先行 physical success だけを追加する。failed / skipped / cancelled / DST-clear / 未実行予約は追加しない | overlay 実装型は可。DB commit を package 間通信にする、予約を成功扱いする実装を検出する | `BmsLibraryPackageInstallServiceTests` |
| `S5-FAILURE-TERMINAL` | physical prefix success -> current unexpected failure / canonical apply failure -> terminal | physical failure は confirmed prefix、failed item、unprocessed suffix を同じ session terminal に保持する。canonical apply failure は prepared filesystem を成功 publication せず recovery candidate を保持し、自動全体 rollback / replay を行わない | diagnostic 文言、candidate 順は可。suffix 成功扱い、偽 per-item durable receipt、canonical apply 再実行を検出する | `BmsLibraryPackageInstallServiceTests`、`PendingPackageWorkflowOwnerTests` |
| `S5-LIFECYCLE-FINALIZE` | auto / estimated / force -> install row、pending/installed collection、DST clear、maintenance | install-row delete/upsert は canonical transaction、package collection/DST/maintenance は required finalization として一つの terminal に残り、failure を通常成功として公開しない | publication scope の内部型は可。commit 後の terminal 外 mutation、install-row 二重 write を検出する | `BmsLibraryPackageInstallServiceTests`、`PackageInstallWorkflowOwnerTests` |
| `S5-RESOURCE-CLEANUP` | installed-only resource overwrite / cleanup-only -> resource index・source cleanup | chart 追加 0 件でも session change を保持し、成功 destination directory は一括 scan、source cleanup は durable point 後に行う。cleanup failure で canonical apply を再実行しない | scan 内部順、warning 文言は可。resource-only drop、pre-durable source cleanup、cleanup retry/replay を検出する | `BmsLibraryPackageInstallServiceTests`、`PendingPackageWorkflowOwnerTests` |
...
`LMS-S6-20260916` は、依頼された S6 の採用済み session / failure 契約への移行を対象とする Test Contract Packet。authority は [実装計画 S6](../plan/library-mutation-session-batching-plan.md#s6--merge-を単一-change-session-と-post-commit-phase-へ統一する)、本書の mutation session / terminal 契約、[FS/DB整合](file-db-consistency.md) の `FSDB-SESSION` / `AUD-01` とする。恒久テストは既存 fixture の更新と、不足していた canonical / maintenance failure 条件の追加に限定する。内部 helper 名・collection 順・診断文言・候補 path 順は固定せず、確定結果・永続データ・失敗段階・解放時点を判定する。
