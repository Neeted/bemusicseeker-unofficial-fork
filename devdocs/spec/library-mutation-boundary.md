...
[共通並行性契約](workflow-concurrency-and-complexity.md#5-規範的な設計原則)に従い、導入元ではなく変更対象の競合で受付を決める。保留一覧の強制導入・推定先への導入を含む foreground 保留操作と、DnD / 単体 URL / 本体・差分 URL 一括取り込み / 外部 API 検索からの自動導入は、composition の同じ `ChartFileOperationSynchronizer` を使う。UI の非同期待ちや activity 表示を変更の同時受理の理由にしない。

保留操作は確認・対象解決の前に共通受付を取得し、既存の pending admission と共に必要な終端まで所有する。pending admission は背景の受理済み自動推定との調整に引き続き使う。競合する後続要求は Busy とし、確認、対象解決、再生停止、store 変更を始めず、拒否を保存・自動再実行しない。

自動導入は最初の queue 挿入時に共通受付を取得し、受理済みの追加 batch は同じ queue の所有下で FIFO 実行する。worker 開始前、batch 間、取消後の未引渡し source cleanup 中も受付を保持し、worker と cleanup が終端してから解放する。generation 切替 / shutdown により表示を捨てる場合も解放し、古い inactive 通知で新たな active batch の受付を解放しない。新しい gate、待機 scheduler、retry は追加しない。

通信待ちはこの共通受付に含めない。取得済み path を queue へ引き渡す時点で受理結果を返す。受理済みの導入予約と未受理要求を区別し、未受理を「予期しないエラー」、ブラウザへの fallback、後日の自動再試行へ変換しない。source の確保・回収は [DnD ingress](drop-install-ingress.md)、通信結果と導入受付の区別は [URL / API 取得](playlist-url-download-resolution.md#導入受付への引渡し)を参照する。

競合するライブラリ変更の拒否は、model の LR2 同期兼用 lease による拒否も含めて `Warn_LibraryOperationBusy` で案内する。導入 queue の `false` は Busy に加えて library 未接続・受付停止・cancel drain も含むため、導入元に依存しない `Warn_PackageInstallUnavailable` で案内する。両者の理由の違いは保持し、LR2 同期や DnD と断定する別名・互換キーを残さない。実際の同期進捗や入力形式不正の通知は別契約のままである。

保留操作・自動導入の model が蓄積した OK-only 情報通知は、operation dialog scope を閉じてから共通の `FileDbMutationReport.ShowOperationMessagesAsync` と既存 `IUiDialogService` へ渡す。一つの通知列は順次表示するが、worker はその表示完了を同期的に待たず、次の batch を停止させない。各表示失敗は診断へ残して後続通知を続け、確定済み mutation の成否へ混ぜない。確認の decision を情報通知へ置き換えない。session の異常結果を一度だけ報告する terminal は、従来どおり操作の受付解放後に呼ぶ。

| 契約 | 実装 | 検証・識別する誤実装 |
| --- | --- | --- |
| 通常変更の相互排他、確認取消・実行失敗後の解放 | `PendingPackageWorkflowOwner.TryEnterPendingOperation` / `InstallPendingAsync` / `InstallPackagesAsync`、`PackageInstallWorkflowOwner.TryEnqueue` | `PendingPackageWorkflowOwnerTests.PendingInstall_RejectsDropDuringConfirmationAndReleasesAdmission` / `DropQueue_RejectsPendingInstallUntilAllAcceptedBatchesFinish`。後者は worker 開始前・各 batch 実行中の拒否と、全 batch 終了後の完了通知時の受付解放を分けて検証する。別 gate の使用、確認後だけの受付、未受理対象への副作用を識別する |
| 予約時から drain 終端まで一つの lease | `PackageInstallWorkflowOwner.TryEnqueue` / `PublishQueueStatus`、既存 `DropInstallQueueProcessor` | `PackageInstallWorkflowOwnerTests.Enqueue_RejectsBusyBeforeQueueingThenRunsAfterRelease` / `GateBusy_AbandonsOwnedIngressWithoutCallingInstaller` / `Enqueue_PublishesCompletionAfterLiveInstallReturnsAndContinuesAfterFailure`、同 fixture の generation / cancel / shutdown tests。worker 開始時だけの排他、受理済み予約の消失、遅い通知による解放、後続 batch 実行中の先行 terminal 発行を識別する |
| URL / API 取得成功と導入未受理を分離 | `PlaylistWorkspaceViewModel.QueuePlaylistUrlInstallPathsAsync` → `PackageInstallWorkflowOwner.Enqueue` | `PlaylistUrlAcquisitionOwnershipTests.DownloadedPackages_BusyHandoffWarnsWithoutFallbackOrReplay`（single / url / diff / api）。通信中は通常変更を許可し、引渡しだけを拒否する。一般例外化、ブラウザ fallback、再実行、source 削除を識別する |
| 非同期情報表示と mutation 結果の分離 | `FileDbMutationReport.ShowOperationMessagesAsync`、保留 / 自動導入 owner の終了処理 | `FileDbMutationReportTests.OperationMessages_PreserveOrderAndContinueAfterDisplayFailure`、`PackageInstallWorkflowOwnerTests.OperationDialogs_AreDispatchedWithoutBlockingQueueOrChangingMutationResult` / `DurableFinalizationFailure_PublishesTypedCompletionWithoutRegisteredPackages`（型付き session 異常結果と終了処理例外の terminal も受付解放後）、`PendingPackageWorkflowOwnerTests.SearchPackagesAsync_EndActivityFailureStillDetachesAndDispatchesDialogScope` / `SearchPackagesAsync_PreservesRequiredFailuresButDoesNotPromoteNotificationFailure` |
| 汎用の競合拒否と queue 未受理の多言語表示 | 共通 resource accessor / resx / 全言語 JSON、各既存 UI terminal | `LocalizationResourceParityTests.InstallAdmissionMessages_AreAvailableWithoutFormatArgumentsInAllLanguages`、既存 resource parity と model / owner Busy tests。全文ではなく key・非空・placeholder parity と拒否結果を検証する |

テストは既存 fixture と local DB / FS / dialog port を使い、owner Task、queue idle、通知 Task / dispatcher の明示 drain を完了境界とする。受付のための全 catalog / resource index の追加走査・snapshot は行わず、session の変更収集・DB apply 回数を入口ごとに増やさない。
...
| 自動・推定先・強制導入、manual shared core、resource-only / cleanup-only導入 | [PackageInstall](../../BeMusicSeeker/Models/BMSLibrary.PackageInstall.cs) と `BmsLibraryPackageInstallService`。各packageの実FS結果を同じinstall sessionへ追加し、確定destinationの `ChartStorageTargetSet` / install-row / package lifecycle factsを集約する | 先行packageの実physical successだけをsession-local ownership overlayへ直ちに追加して後続分類へ使う。canonical storageとinstall-row delete/upsertは一つのtransaction、resource scan・maintenance・package collection/DST required publicationはoperation終端で集約する。source cleanupはdurable point後だけに行い、cleanup failureでcanonical applyを再実行しない。譜面追加0でもresource/packageの必要changeをsessionへ残し、terminalはsynthetic item receiptではなく`LibraryMutationSessionReceipt`を正本にする | [PackageInstallServiceTests](../../BeMusicSeeker.Tests/BmsLibraryPackageInstallServiceTests.cs)、[PendingPackageWorkflowOwnerTests](../../BeMusicSeeker.Tests/PendingPackageWorkflowOwnerTests.cs)、[PackageInstallWorkflowOwnerTests](../../BeMusicSeeker.Tests/PackageInstallWorkflowOwnerTests.cs)、[Lr2SongDbSyncTests](../../BeMusicSeeker.Tests/BmsLibraryLr2SongDbSyncTests.cs)：success overlay、canonical failure、physical prefix/suffix、resource/cleanup、terminal |
...
installed-only resource overwrite の成功件数は physical prepare ではなく session の durable / required apply 完了で確定する。canonical / required apply failure は成功集計にせず、cleanup だけの失敗では durable success を維持する。異常終端は `PendingPackageWorkflowOwner` が受付解放後に `FileDbMutationReport` へ渡し、通常集計に丸めず原因・確認候補を一度だけ通知する。DST clear の state apply と `PackageChartEntry.PropertyChanged` の公開は分離し、公開は model の file mutation lease 解放後とする。購読者の例外を required finalization failure にしない。

### 導入 session の実装・テスト対応

一回の利用者操作は一つの install session とする。先行 physical success を後続分類へ使うことと、canonical durable apply / publication を操作終端へ集約することを両立する。実装は `BMSLibrary.PackageInstall.cs` と `BmsLibraryPackageInstallService` の session route、検証は次の既存 fixture に置く。

| 契約 | production ingress -> observable / durable impact | 必須結果 | 許容差分 / 検出すべき誤実装 | 検証 |
| --- | --- | --- | --- | --- |
| 変更の一括確定 | auto / pending estimated / force / manual shared install / resource overwrite -> catalog・install rows・package collections | 一つの user operation は一つの install session で canonical storage/install-row apply と required package publication を operation 単位に確定する | helper 名、入力順 physical I/O は可。package ごとの canonical apply / synthetic durable item receipt を検出する | `BmsLibraryPackageInstallServiceTests`、`PendingPackageWorkflowOwnerTests` |
| 先行成功の所有判定 | 複数 package install -> 後続 duplicate / ownership 判定 | operation 開始 snapshot に先行 physical success だけを追加する。failed / skipped / cancelled / DST-clear / 未実行予約は追加しない | overlay 実装型は可。DB commit を package 間通信にする、予約を成功扱いする実装を検出する | `BmsLibraryPackageInstallServiceTests` |
| 失敗結果の保持 | physical prefix success -> current unexpected failure / canonical apply failure -> terminal | physical failure は confirmed prefix、failed item、unprocessed suffix を同じ session terminal に保持する。canonical apply failure は prepared filesystem を成功 publication せず recovery candidate を保持し、自動全体 rollback / replay を行わない | diagnostic 文言、candidate 順は可。suffix 成功扱い、偽 per-item durable receipt、canonical apply 再実行を検出する | `BmsLibraryPackageInstallServiceTests`、`PendingPackageWorkflowOwnerTests` |
| lifecycle の必須完了 | auto / estimated / force -> install row、pending/installed collection、DST clear、maintenance | install-row delete/upsert は canonical transaction、package collection/DST/maintenance は required finalization として一つの terminal に残り、failure を通常成功として公開しない | publication scope の内部型は可。commit 後の terminal 外 mutation、install-row 二重 write を検出する | `BmsLibraryPackageInstallServiceTests`、`PackageInstallWorkflowOwnerTests` |
| resource と cleanup | installed-only resource overwrite / cleanup-only -> resource index・source cleanup | chart 追加 0 件でも session change を保持し、成功 destination directory は一括 scan、source cleanup は durable point 後に行う。cleanup failure で canonical apply を再実行しない | scan 内部順、warning 文言は可。resource-only drop、pre-durable source cleanup、cleanup retry/replay を検出する | `BmsLibraryPackageInstallServiceTests`、`PendingPackageWorkflowOwnerTests` |

mutation 開始前に source の欠落を明示確認した package は失敗として保持し、pending / install row / DST を成功更新せず、後続の独立 package は処理する。欠落 package の hash を成功 overlay へ加えない。一方、コピー開始後に source が失われた場合は、同じ `FileNotFoundException` でも physical failure として unsafe な suffix を停止し、confirmed prefix / failed item / unprocessed suffix を保持する。対応は `BmsLibraryPackageInstallServiceTests.InstallPendingPackages_RejectsMissingSourceAndContinuesIndependentPackages` / `ForceInstallPendingPackages_PreservesPhysicalSuccessPrefixAndUnprocessedSuffix` で検証する。

共有資源は test-local filesystem / SQLite と既存 dialog・file-mutation port、完了は production model return または workflow Task とする。内部 helper 名やログ文言・候補順は固定せず、canonical apply の反復、未実行予約の成功扱い、suffix 成功扱い、cleanup retry を識別する。
...
canonical apply は `LibraryMutationOwner` の private な `CommitCatalogSessionChanges` / `CommitInstalledSessionChanges` に限定する。これらを呼ぶのは同 owner の `LibraryMutationSession.Commit` だけであり、coordinator、item loop、ViewModel に直接の apply callback を渡さない。catalog relocation/removal と install target の apply family は一つの session に混在させず、混在は失敗として返す。通常通知や LR2 同期を caller が別途補完する互換経路は設けない。row-path 通知の要否は `LibraryStorageRowPathNotificationPolicy` を使い、reason 文字列は診断専用とする。

`FileDbMutationReceipt` は共通 package helper の物理 prepare、source 保全、局所補償、durable 後の cleanup / live-state completion の結果に限る。そこから得た失敗・cleanup facts は session に引き継ぐが、局所 receipt を束ねて操作全体の durable result と解釈しない。model / ViewModel の操作終端は `LibraryMutationSessionReceipt` を正本とする。

自動 rename も他の catalog mutation と同じく、catalog の確定 receipt と対応する owned version から変更 directory / 祖先だけの現在 BMS facts を捕捉し、session の共通 apply 内で LR2 同期を一回行う。適用前 snapshot の投影、caller 側の追加 LR2 同期・通常 refresh は持たない。通常通知は session が required apply の成功時だけ公開リストへ渡し、解放後の owner は失敗診断を含むリストを処理する。

成功依存と commit 粒度を分ける理由、限定補償を残す判断は [操作単位 session の採用判断](../decisions/library-mutation-session.md)、反映回数の診断と検証は [性能 marker](performance-and-scale.md#43-mutation-session-の集約診断) を参照する。
...
公開後に必要な専門cacheの再計算、利用者による一覧全件列挙、cold索引の初回構築、明示全走査は残る。固定差分で不要な全構築・copy・sortを行わない構造と、大規模環境での wall-clock 短縮率は区別して検証する。
...
| package install の operation-scoped session、physical-success overlay、storage/install-row 一括apply、canonical failureの非rollback、physical prefix / failed / suffix、cleanup/resource/terminal | `BmsLibraryPackageInstallServiceTests` / `PendingPackageWorkflowOwnerTests` / `PackageInstallWorkflowOwnerTests` / `BmsLibraryLr2SongDbSyncTests` | remaining-bms-library / remaining、model return / workflow Task |
| install の事前 source 欠落は失敗を保持して後続継続し、コピー中の同種例外は physical failure として suffix 停止 | `BmsLibraryPackageInstallServiceTests.InstallPendingPackages_RejectsMissingSourceAndContinuesIndependentPackages` / `ForceInstallPendingPackages_PreservesPhysicalSuccessPrefixAndUnprocessedSuffix` | remaining-bms-library、固有 DB・FS / local file-mutation port、model return |
| resource-only / cleanup-only の canonical failure 時の成功件数抑止・source / pending 保持と、session 異常終端の受付解放後表示 | `BmsLibraryPackageInstallServiceTests.OverwritePendingInstalledOnlyPackagesResources_CanonicalFailureDoesNotPublishSuccessCounts` / `PendingPackageWorkflowOwnerTests.OverwriteInstalledOnlyPendingPackageResourcesAsync_ReportsSessionAfterRelease` | remaining-bms-library / remaining、固有 DB・FS / local ports、model return / workflow Task |
| resource overwrite の DST 通知時に file mutation lease と writer が解放済みであり、購読者例外でも durable success を維持 | `BmsLibraryPackageInstallServiceTests.OverwritePendingInstalledOnlyPackagesResources_ReleasesOuterWritersBeforeEstimatedInstallPublication` | remaining-bms-library、固有 DB・FS、model return |
...
アプリ全体の FS+DB の保証・非保証、前方回復、失敗の表示、レビューで受け入れる制限は [file-db-consistency.md](file-db-consistency.md) を正本とする。以下の `COMP-*` は `FileDbMutationExecutor` を使う局所経路の限定補償契約であり、削除等を含むすべての mutation に FS rollback を要求するものではない。

自動・推定先・強制導入、manual shared core、resource-only / cleanup-only 導入は operation-scoped install session を使い、package ごとの executor durable receipt を確定境界にしない。auto rename、手動 rename / 複数 folder move、library chart delete、通常 invalid-extension rename、導入先修正、duplicate folder merge も session を使う。導入・merge は共通 package helper の physical prepare / source 保全を再利用するが、canonical apply failure を executor の補償へ戻さず、session terminal に保持する。残存 executor helper の局所補償契約と operation-scoped session terminal は区別する。pending-only invalid-extension rename は owned catalog を変更しないため package/install lifecycle route のまま維持する。multi-change 操作の恒久契約は [Mutation session 契約](#mutation-session-契約) とし、immutable な preflight / destination type guard、source保全等の有用な局所安全策は維持してよい一方、DB durable receiptをitemごとに作ること自体は要件ではない。確認済みphysical successをchangeとして蓄積し、canonical durable applyをoperation境界へ集約する。source cleanupをdurable apply後へ遅延できる操作ではsession終端まで遅延する。
...
`FileDbMutationReport` は session receipt から操作全体の terminal を作る。`reportAtTerminal` は workflow が異常報告を所有する場合に model の個別 dialog と二重表示しないための明示 policy とする。per-item report の互換経路を増やさない。session のない preflight／destination-exists 拒否は既存通知を保持する。

### Exclusive lease and deferred effects

ファイルを変更する command は、preflight confirmation の後に短い sequence admission を行い、その sequence monitor を解放してから `LibraryFileMutationLease` を取得する。拒否時も sequence monitor と既に取得した scope を直ちに解放し、拒否 dialog を monitor 内で待たない。lease は LR2 同期と他の file mutation に対して排他的であり、session 内の filesystem changes、operation-scoped durable apply、required internal finalization と必要なcleanupが終端するまで保持する。局所 executor の compensation / cleanup も対応する lease の所有中に終える。
...
この契約は LibraryMutationOwner の session と共通反映、AutoRenameBatchCoordinator、LibraryFolderMoveCoordinator が実装する。BmsLibraryFolderRenameRefreshTestsの実rename（背景16/128・2操作）、auto batch/部分失敗、repairと、BmsLibraryPackageInstallServiceTests.RenameBMSFilesExtensions_UnregistersOnlySuccessfulChartsAndPreservesHashOwner、関連workflow testsで、FS/DB、索引、旧snapshot、通知と受付を確認する。
...
各 package move は `MovePackageFilesPhysicalWithReceipt` の detached physical result として扱い、executor-local receipt を user-operation terminal に昇格させない。衝突採番後の実 destination から `CreateFixMutationFacts` を作り、成功した path / install-destination facts を session へ append する。item loop 内で canonical apply を呼ばず、全 confirmed repair facts を操作終端で一回 apply する。重複は move 前に分類し、未承認なら source を保持する。承認済みなら filesystem delete phase を同じ outer lease 内で実行し、その confirmed removal / package-reference / resource-directory facts を repair session へ追加する。別の `RemoveLibraryChartsCore` や別 session へ再入しない。
...
`MergeChartDirectory` は source / destination の一組を `N=1` の `LibraryMutationSession` として扱う。source の owned chart がない場合は destination hash / installed lookup を取得せず終了する。共通 package helper は型衝突を変更前に全体検査し、衝突採番を含む actual destination の projection を固定する。physical prepare 成功後だけ `AppendFolderMerge` へ catalog / package facts と cleanup 待ちの physical mutation を渡す。storage owner の path を一時的に変更して destination facts を作らず、別の per-item durable callback / terminal receipt も作らない。

session commit が catalog / package reference apply を一回行い、canonical owner の移転直後・source cleanup 前に maintenance target を固定する。source cleanup は canonical durable point 後に行い、成功した統合先の一回の scan と source reverse lookup の置換を同じ session の derived-state phase に含める。resource-only でも change を捨てず、owned collection の更新は実際の chart removal / path change に従う。canonical apply failure では確認済み physical change、source / destination / staging / backup の確認候補と failure を保持し、自動 rollback / replay や source cleanup を行わない。

merge の lease を解放して共通通知を発行した後、導入先修正とは別の `applyMergeFolderMaintenanceAfterRelease` で統合先を再検査する。通常の `ApplyCatalogMaintenance` が新しい予約を取得し、既存の `forceUpdate: true` / `DeferOnUpdates` / `merge_folder` を維持する。maintenance は任意通知ではなく、同じ操作の required `PostCommitMaintenance` phase とする。例外または予約再取得拒否による `Canceled` は `SessionReceipt.FinalizationFailure` へ残し、`MergeApplied=true` / `DurableCommit=true` の確定済み merge と区別する。二つ目の library mutation session、別の成功表示、再試行、rollback は追加しない。取得済み予約用の処理を無予約で流用せず、導入先修正を通常受付へ戻すこともしない。

`DuplicateMergeMaintenanceReceipt.SessionReceipt` が merge から maintenance までの canonical terminal である。maintenance failure 時の workflow は非成功とし、cleanup failure が併発しても双方の事実を保持する。`DuplicateMaintenanceWorkflowOwner` は model lease・gate・activity・dialog scope の終了後に session 用 `FileDbMutationReport` へ一回渡す。全正常は silent、cleanup のみは Warning、必須反映・maintenance failure は Error、変更前の型衝突は既存のマージ専用リソースによる集約 Warning とする。reporter failure は元の結果を変えない。

##### 統合の契約・実装・テスト対応

統合の恒久テストは、本書の mutation session / terminal 契約と [FS/DB整合](file-db-consistency.md) に対応する。既存 fixture の実入口を使い、内部 helper 名・collection 順・診断文言・候補 path 順は固定せず、確定結果・永続データ・失敗段階・解放時点を判定する。

本番経路は `DuplicateMaintenanceWorkflowOwner.RunFolderMergeAsync` → `BmsLibraryDuplicateMaintenanceStore.MergeFolderWithReceipt` → `BMSLibrary.MergeChartDirectory` → `LibraryMutationOwner` → session commit → 通常 maintenance 予約 → 同じ workflow terminal。入口の group 検証・catalog readiness・既存 mutation 受付を維持する。DB 拒否は既存 writer の失敗、予約競合は merge 解放後に別操作が先に受理される経路として扱い、新しい試験専用 production seam を作らない。

| 仕様項目・主な条件 | 実装箇所 | テスト箇所・確認内容 |
| --- | --- | --- |
| 一統合一 apply、採番済み実宛先、resource-only、warm index の差分更新 | [Merge owner](../../BeMusicSeeker/Models/BMSLibrary.LibraryMutationOwner.Merge.cs) の `BuildMergeCatalogFacts`、[Session](../../BeMusicSeeker/Models/BMSLibrary.LibraryMutationOwner.Session.cs) の `AppendFolderMerge` / `Commit` | [BmsLibraryDuplicateServiceTests](../../BeMusicSeeker.Tests/BmsLibraryDuplicateServiceTests.cs) の `MergeChartDirectory_TwoWarmOperationsKeepIndexesCurrentWithoutFullRebuild`（16/128 background、独立2操作、N=1・所有一覧公開一回・FS/SQLite・全件再構築なし）、`DifferentContentCollisionUsesActualDestinationEverywhere` / `BmsonDuplicateSkip_CommitsResourceOnlySession`（いずれも `MergeChartDirectory_` prefix）。per-chart apply、宛先再計算、resource-only の脱落を識別する |
| 型衝突は全体拒否、source なしは lookup 前に終了 | Merge owner の prepare と [package service](../../BeMusicSeeker/Models/BmsLibraryInternal/BmsLibraryPackageInstallService.cs) の共通 preflight | 同 fixture の `MergeChartDirectory_RejectsTypeConflictAndPreservesSourceRegistration` / `MergeChartDirectory_NoSourceChartsDoesNotBuildLookups`（source directory 欠落・空、cold/warm）。部分変更・登録喪失・先行 lookup 構築を識別する |
| physical と canonical / durable finalization を区別し、confirmed facts と復旧候補を残す | Session `Commit`、共通 package physical prepare / durable cleanup、[DuplicateMergeMaintenanceReceipt](../../BeMusicSeeker/Models/BmsLibraryInternal/DuplicateMergeMaintenanceReceipt.cs) | 同 fixture の `MergeChartDirectory_CatalogFailureRetainsPreparedFilesAndSessionFacts`（BMS/BMSON の SQLite 拒否、両側 file と旧 row 保持）、`MergeChartDirectory_Lr2FinalizationFailureReturnsDurableNonSuccessWithoutMaintenancePublication` / `MergeChartDirectory_FailedDialogEffectIsIsolatedAndCanReenterAfterLeaseRelease`。失敗の成功化、canonical failure での cleanup / rollback、解放前の通知を識別する |
| 対象固定、解放・再予約、保守結果と同じ terminal | Merge owner の required target capture / `applyMergeFolderMaintenanceAfterRelease`、session receipt の `WithFinalizationFailure` | 同 fixture の `MergeChartDirectory_BmsonOnly_ReRegistersSongAtDestination`、`MergeChartDirectory_RechecksResourcesAfterReleasingMutationReservation`（BMS/BMSON、正常・保守 DB 拒否・予約競合）。正常は移動先 WAV・保守 DB・警告解除・deferred index、失敗は merge durable success と failure の併存を検証し、任意通知への格下げ・別 session・rollback を識別する |
| 操作資源の解放後に一回、cleanup warning / maintenance error、reporter failure の分離 | [DuplicateMaintenanceWorkflowOwner](../../BeMusicSeeker/ViewModels/MainWindow/DuplicateMaintenanceWorkflowOwner.cs) の `FromMergeReceipt` / `RunFolderMergeAsync`、`FileDbMutationReport.ShowAsync` | [DuplicateMaintenanceWorkflowOwnerTests](../../BeMusicSeeker.Tests/DuplicateMaintenanceWorkflowOwnerTests.cs) の `RunFolderMergeAsync_DurableFinalizationFailurePublishesFailureWithoutSuccess`（merge 必須反映 / 後続 maintenance）、`RunFolderMergeAsync_ReportsAfterReleaseAndPreservesFactsWhenReporterFails`（warning/error × reporter 成否）。durable / cleanup facts の消失、二重報告、gate/activity/priority 解放前の表示を識別する。[FileDbMutationReportTests](../../BeMusicSeeker.Tests/FileDbMutationReportTests.cs) の `DestinationTypeConflictsUseOperationWordingAndOnlyShowConfirmedDurableSuccess` は session 入力で既存マージ専用リソースの選択と成功0の案内抑制を維持する。[LocalizationResourceParityTests](../../BeMusicSeeker.Tests/LocalizationResourceParityTests.cs) の `FileDbMutationReportResources_PreserveRequiredFormattingArguments` は、予約競合時の保守未完了表示も含め、全言語のリソース存在と必要な書式引数を確認する |

検証の完了境界は同期 model return と awaited workflow Task。既存の固有 temp FS / SQLite / I/O・dialog port と Functional の `remaining-bms-library` / `remaining` を使い、新しい fixture・lane・固定待機・全件走査を加えない。採用済み契約への移行として、targeted mutant / negative control は追加要件にしない。
...
| repair move + approved duplicate removal の同一session、catalog / maintenance failure の非rollback | `BmsLibraryFolderRenameRefreshTests` | real repair return／session receipt、固有FS／SQLite trigger |
| C6: repair session terminal の解放後一回報告、無関係exception伝播 | `PendingPackageWorkflowOwnerTests` | awaited owner Task、local ports／post-release dialog |
