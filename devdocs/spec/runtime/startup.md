...
正常な起動の主な依存を示します。矢印は前提の成立、合流のANDは両入力が必要なことを表します。設定不備・走査省略などの分岐は以下の各節を正本とし、この図だけから全経路の実行を要求しません。

```mermaid
flowchart TB
    Prepare["登録先・設定の検査、DB準備"] --> Scan["譜面・リソース走査"]
    Prepare --> Load["補助メタデータ取込み・DB読込み"]
    Scan --> Inputs{"AND：走査とDBの入力が揃う"}
    Load --> Inputs
    Inputs --> Apply["差分反映・保留復元"]
    Apply --> Data["導入判定用データの準備"]
    Data --> Ui["必須の初期表示を適用"]
    Ui --> Operable["通常入力を解禁・スケジューラー開始"]
```

#### 操作可能と初期化完了の違い

矢印は開始・完了の前提です。必須処理と後続処理は重なり得ますが、出力修復・導入可能譜面の保守・GCの開始制限は[必須の処理と後続処理](#必須の処理と後続処理)に従います。線がない処理の並行実行を新たに許可する図ではありません。独立した順位・XML取得と遅延表示は図外で追跡し、`PostDone` から完了を推測しません。

```mermaid
flowchart TB
    Operable["通常入力を解禁"] --> Required["必須読込み・補完または不要判定"]
    ...
    Prewarm --> PostDone["スケジューラー内の後続完了"]
```

`Settled` の登録閉鎖には後続・動的なLR2登録も含みます。記録名との対応は上表、進捗の計算は[進捗仕様](progress.md)に従います。
...
| スコアとIRの要求条件、事前取得 | [`BMSLibrary`](../../../BeMusicSeeker/Models/Library/BMSLibrary.cs) | [`BmsLibraryIrStartupTests`](../../../BeMusicSeeker.Tests/Startup/BmsLibraryIrStartupTests.cs)、[`StartupRankingRefreshPolicyTests`](../../../BeMusicSeeker.Tests/Startup/StartupRankingRefreshPolicyTests.cs) |
...
| スコアだけの再読込み、失敗・取消後の受付解放 | [`ScoreOnlyReloadWorkflowOwner`](../../../BeMusicSeeker/ViewModels/Startup/ScoreOnlyReloadWorkflowOwner.cs) | [`ScoreOnlyReloadWorkflowOwnerTests`](../../../BeMusicSeeker.Tests/Startup/ScoreOnlyReloadWorkflowOwnerTests.cs) |
