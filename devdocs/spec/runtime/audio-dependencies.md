...
| `bassmix.dll` | 2.4.13.0 / `0x02040D00` | https://www.un4seen.com/files/bassmix24.zip | `9E7FE1AF3122C0D78D07CB2788CD52AEF4E47B6C6F8AD0022AD5E40320516796` | `bassmix24\x64\bassmix.dll` | `3A1777CD14C0FC6E2D6F879CE9B66CB60CD1C6607F344D70D2434164526063AB` | `libs/x64/bassmix.dll` |
...
| `bms_vorbis.dll` | C ABI 1 / libogg 1.3.6・libvorbisfile 1.3.7 | `native/VorbisBridge/CMakeLists.txt`; https://downloads.xiph.org/releases/ogg/libogg-1.3.6.tar.xz; https://downloads.xiph.org/releases/vorbis/libvorbis-1.3.7.tar.xz | `5C8253428E181840CD20D41F3CA16557A9CC04BAD4A3D04CCE84808677FA1061` / `B33CC4934322BCBF6EFCBACF49E3CA01AADBEA4114EC9589D1B1E9D20F72954B` | `native/VorbisBridge/build/Release/bms_vorbis.dll` | `C4AD0A911F75AE73815770EAFD67CB4EFEC853B8402CB28C02DE64CB03A4140A` | `libs/x64/bms_vorbis.dll` |

`bms_vorbis.dll`は追跡対象のbridgeとハッシュ固定したXiph source archiveから、Windows x64・Release DLLとしてビルドします。liboggとlibvorbisはMSVC静的runtimeで静的リンクするため、別のXiph DLLはロードしません。`BassNativeRuntime`は他のnative DLLと同じ`libs/x64`からbridgeを読み込み、既存のプロセス保持generationに含めます。`VorbisDecoder`は所有済みhandleからC ABI exportを取得し、main assemblyのManagedBass DllImport resolverではbridgeを解決しません。
...
依存更新では版、取得結果、アーカイブとDLLのハッシュ、PE形式、採用ファイル、固定済みパッケージ、出力を一つの互換単位として変更します。検証・コピーの失敗で戻す単位もネイティブDLL一式です。一部だけ違う版にして配布しません。
