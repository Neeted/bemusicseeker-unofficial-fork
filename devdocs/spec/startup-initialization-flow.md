...
LR2 linked / standalone の profile 切替は、同一プロセス内の `FullReinitialize` や hot reload では反映しない。起動済み profile が存在する通常運用時は、設定ダイアログで mode のトグルを切り替えた時点で再起動確認を出し、承認された場合だけ既存の UI dispatcher を通る shell request として受け付ける。Close や更新準備が先に受理されていない場合だけ、動作モード、履歴表示 target identity、runtime placement を同じ atomic persistence 境界で保存して設定値を reload し、通常の `MainWindow.Close` と terminal cleanup の末尾から後継 process を起動する。他の未保存設定は保存しない。mode request の保存が失敗した場合は終了準備を開始せず、caller が失敗を観測できる。キャンセルまたは request の拒否時は保存済み mode へ表示を戻し、実行中の library profile と startup progress は変更しない。切替後の設定不足は、再起動後の `Startup` validation と既存の設定ダイアログ表示で案内する。
...
mode 切替は上記 operation ではなく process restart として扱う。これは active `song.db`、LR2 config provider、score source、background scheduler、UI cache の境界が変わるためで、current process で `InitializeAsync()` を再実行して profile を差し替えない。mode request は first-wins の terminal intent として一度だけ受理し、後続の Close/update request は既存の準備経路へ合流させる。terminal 中の後継 process start failure は terminal task 内で localized notification の完了まで await し、その後に既存の MainWindow shutdown authorization を完了する。
...
`StartupBackgroundTasksDone` は scheduler 全体の empty ではなく、`runningRequiredCount == 0` かつ required request が queue に無い状態を表す。required scheduling enrollment が閉じ、他の expected startup phase も完了したときだけ進捗上の完了になる。LR2 `song.db` 同期の自動 queue はこの判定や startup progress の分母に含めず、`startup_initialization_complete` 後に専用 status として開始する。初回完了ダイアログの終了は queue の待機条件にしない。
