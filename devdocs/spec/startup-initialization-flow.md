...
通常の file diff と LR2 full generation は、入力 surface を共有しても差分の意味を混ぜない。file diff の「差分あり」は owned BMS / bmson 実ファイルの追加・削除・mtime / hash 変更を正本にし、LR2 `folder` table の incomplete / stale / expected row 欠落だけで通常 file diff を重くしない。LR2 normal folder の全件再同期、startup-scan diagnostic の残件処理、expected set 外 row pruning は full generation sync / cleanup stage の責務である。file diff 中に LR2 `folder` row を更新する必要がある場合も、変更 path と prune scope に基づく scoped sync を使い、既存 `folder` row も生成対象と prune scope だけを読む。差分 0 件や完全生成未完了 status だけで全件 normal folder sync を走らせない。
