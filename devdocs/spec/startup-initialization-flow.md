...
起動時の app-managed custom folder repair は、playlist 正本と出力 generator version から作る軽量 projection、`playlist_custom_folder_output_status`、起動 scan で得た `.lr2folder` mtime surface を使う。status は最後に本アプリが materialize / LR2 `folder` row sync を完了した playlist の `output_directory`、出力 bit、root flag、entry type、folder sort、header/data hash、`last_update`、期待 file 数、LR2 `folder` table の行数、generator version を保存する。status が current で、LR2 `folder` table 行数も変わらず、scan surface 上でも期待 `.lr2folder` が存在する playlist は repair の DB row lookup / materialize 対象にしない。status が無い、入力が変わった、LR2 `folder` table 行数が変わった、期待 file が scan surface 上で欠ける、または LR2 `folder` row の軽量構造検査で不一致がある場合だけ pending とし、batch materialization と単発 LR2 `folder` row sync で修復する。起動時 scan surface が利用できない場合は、対象出力 directory 群に対する grouped enumeration で `.lr2folder` mtime surface を作り直す。個別 file ごとの `File.Exists` / `GetLastWriteTimeUtc` を repair target selection の通常経路にはしない。table ごとに既存出力を削除して DB sync を繰り返さない。
