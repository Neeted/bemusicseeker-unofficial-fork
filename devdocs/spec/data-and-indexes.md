...
primary-only/full lookupの状態、専用lock、initialized、generation、observerとread/build/apply/invalidateは、所持collectionと同じ `CatalogOwnedCollectionOwner` が所有する（`CatalogOwnedCollectionOwner.Installed.cs`）。BMSLibraryの公開facadeと保留再推定のcurrentness gateは維持し、snapshotとgenerationを同時捕捉して同じ操作境界で照合する。rootへ別の世代値やindex状態を保持しない。
...
| primary-only snapshotと同MD5差分の相殺 | `PrimaryHashLookupState.CreateSnapshot`、[CatalogOwnedCollectionOwner.ApplyInstalledChartLookupMutation](../../BeMusicSeeker/Models/BmsLibraryInternal/CatalogOwnedCollectionOwner.Installed.cs) | [OwnedChartCollectionInstalledOverlayTests.CreateInstalledChartKeySnapshotExcludingCharts_ExcludesOnlyPrimaryHashCounts](../../BeMusicSeeker.Tests/OwnedChartCollectionInstalledOverlayTests.cs)、PackageInstallServiceの実force導入・同digest追加、inline digest coverage。導入前処理のfull取得とtarget反映の仕事量は区別する。primary-only/hash-onlyのままtargetへ到達する本番経路は未確認で、その状態の統合テストは追加しない。 |
...
| 集合不変時のcontent Versionとsummary再利用 | 同root、[CatalogOwnedCollectionOwner](../../BeMusicSeeker/Models/BmsLibraryInternal/CatalogOwnedCollectionOwner.cs) | [BmsLibraryPackageInstallServiceTests.ForceInstallPendingPackages_SameDigestAdditionKeepsHashVersionAndPlaylistSummaryCache](../../BeMusicSeeker.Tests/BmsLibraryPackageInstallServiceTests.cs) |
...
| digest反映・通知時の可視性・DB failure | [CatalogOwnedCollectionOwner.ApplyDigestChanges](../../BeMusicSeeker/Models/BmsLibraryInternal/CatalogOwnedCollectionOwner.cs)、[LibraryMutationOwner.DispatchOwnedChartCollectionMutation](../../BeMusicSeeker/Models/BMSLibrary.LibraryMutationOwner.Common.cs) | [OwnedChartCollectionInlineDigestTests](../../BeMusicSeeker.Tests/OwnedChartCollectionInlineDigestTests.cs) のMD5/SHA-only/failure各case。通知時点の新hashと旧snapshotを確認 |
...
Missing／無視済みMissing一覧の読取りで現在のresource health snapshotを再利用できない場合は、既存のinitialized-min reader・storage reader scope内でfull targetを捕捉して構築する。所持collectionが構築済みでもこの同期を省略せず、導入writerの途中状態を空の保守一覧として確定しない。warm snapshotの再利用経路に新しい待機は加えない。
...
| cold保守一覧とcatalog writerの整合 | [LibraryMutationOwner.GetResourceHealthIndexSnapshot](../../BeMusicSeeker/Models/BMSLibrary.LibraryMutationOwner.Common.cs)、既存 `LibraryFileOperationSynchronization.EnterFolderMoveReadScope` | [BmsLibraryMaintenanceServiceTests](../../BeMusicSeeker.Tests/BmsLibraryMaintenanceServiceTests.cs) の `ChartFilesNeedResourceFix_WaitsForCatalogStorageWriterBeforeColdResourceHealthRead`。実ChartFilesNeedResourceFixの早期完了を拒否し、解放後の既知警告membershipを確認。実install全体の競合再現とは区別する |
