...
出力先は既定の通常出力先 `LR2CustomFolderOutputBaseDir`、追加通常出力先 `LR2CustomFolderAdditionalOutputBaseDirs`、root 用の `LR2CustomFolderOutputBaseDirRootType` と、playlist の `output_dir` / `custom_folder_output_base_name` から決まる。既定通常出力先は LR2 `config.xml` の `<jukebox>` 候補から選ぶ値ではなく user.config の一意な app-managed output root として扱う。追加通常出力先のフルパスは user.config の JSON string array に保存し、表示名と DB 保存値は出力先フォルダの末尾 directory 名を使う。LR2 linked profile では既定通常出力先と追加通常出力先を LR2 `config.xml` の `<jukebox>` BMS search root として同期し、登録時は追加、設定から削除された時は当該 path を自動削除する。既定通常出力先と追加通常出力先はアプリ管理 custom folder root であり、登録済み BMS search root や他の custom folder 出力先と同一または親子関係になる配置は設定入口で拒否する。一般タブの BMS directory 一覧とインストールタブの新規インストール先候補には、アプリ管理 custom folder root を表示しない。`playlist.custom_folder_output_base_name` が NULL の場合は既定の通常出力先、登録済み追加出力先名の場合はその追加出力先を使う。保存名が設定から消えている場合は実効値として既定の通常出力先へ fallback するが、設定 UI から追加出力先を削除した場合は参照 playlist を NULL へ戻し、リネームした場合は新しいフォルダ名へ更新する。`ignore_folder_output` により level/user/alphabet/clear などの folder 種別を除外できる。

LR2 linked profile の起動時には、user.config の既定通常出力先と追加通常出力先をアプリ管理 custom folder root の正本として扱い、LR2 `config.xml` の `<jukebox>` から欠けている場合は補完する。補完時も通常の BMS root 追加と同じく、登録済み root や追加対象同士が同一または親子関係になる場合は許容しない。root 出力は user.config の root 用 base directory 自体ではなく、`playlist.is_root_folder` と `playlist.output_dir` から決まる各 playlist/table 出力 directory を BMS root 相当として扱い、起動・`ReloadTables` 時に `<jukebox>` へ収束させる。

プレイリストプロパティのカスタムフォルダタブにある `OUTPUT` は `custom_folder_output_base_name` を操作し、「フォルダ名」は実効値として `BMSTable.Output_dir` を操作する。入力値は trim 後に Shift-JIS 互換文字列へ丸める。空、または playlist name を同じ規則で丸めた値と同じ場合、DB の `playlist.output_dir` は NULL にし、実効出力フォルダ名は playlist name 由来の値に戻す。`is_root_folder = false` の場合は選択された通常出力先 `\Output_dir`、`is_root_folder = true` の場合は `LR2CustomFolderOutputBaseDirRootType\Output_dir` を出力 directory にする。ROOT ON の間も `custom_folder_output_base_name` は保存するが、実効出力先は常に root 出力先を優先する。UI の「ルートフォルダにする」は `is_root_folder`、出力フォルダ種別のチェック群は `ignore_folder_output` の反転として扱う。DB には「出力する種別」ではなく「出力しない種別」の bit flag が保存される。旧形式の「既存全種別を出力しない」値 `0x7F` は、追加種別も含めた全無効として解釈する。

出力 directory には `0000.lr2folder` からの連番ファイルを Shift-JIS で作る。階層を持つ種別では、各相対 directory の中で `0000.lr2folder` から採番する。各ファイルは少なくとも `#COMMAND`、`#MAXTRACKS`、`#CATEGORY`、`#TITLE`、`#INFORMATION_A`、`#INFORMATION_B` を持つ。BeMusicSeeker 生成ファイルでは `#CATEGORY` は playlist name、`#TITLE` はフォルダ表示名である。生成順は root 直下の `UserFolder`、`LevelFolder`、`AlphabetFolder`、`CategoryAllFolder`、`OtherFolder` と、階層付きの `ClearFolder`、`DJLevelFolder`、`BpmSortFolder`、`BpSortFolder`、`PlayCountSortFolder` で構成され、`ignore_folder_output` に含まれる種別は生成しない。`RandomFolder` は単独の抽出条件ではなく、User / Level / Clear / DJ level の各出力に `RANDOM` 版を追加する modifier として扱う。同一 directory 内では通常フォルダを先に連番化し、その後ろに `RANDOM` 版をまとめて配置する。各 playlist の出力 directory 配下は BeMusicSeeker の管理領域であり、同じ出力先への再出力時は期待 projection に含まれない `.lr2folder` を削除対象にする。出力先変更、root 切替、playlist 削除では旧 playlist 出力 directory 全体を削除し、LR2 `folder` row は旧 directory scope で prune する。通常出力先 / root 出力先の直下にある別フォルダや別ファイルは、対応する playlist の `Output_dir` でない限り触らない。管理 playlist の出力先同士が包含関係になる設定は通常経路では想定しない。
...
`playlist_custom_folder_output_status` は、本アプリが最後に custom folder materialization と LR2 `folder` row sync を完了した playlist の durable no-op 判定である。status には、`output_directory`、root flag、出力 bit、entry type、folder sort、未送信出力設定、header/data hash、`last_update`、実出力 surface から作る物理 `.lr2folder` file / 出力 directory / 格納先 ancestor directory の mtime 署名だけを保存する。起動時 repair は先に playlist header と status を読み、現在の入力 identity と output directory 単位の物理署名が一致する playlist は、playlist entry hydration / projection / DB row lookup / materialization 対象にしない。status が無い、入力が変わった、物理 `.lr2folder` が欠けた、物理 `.lr2folder` / 出力 directory / 格納先 ancestor directory の mtime が変わった場合は pending とし、pending の場合だけ playlist 正本から軽量 projection を作って LR2 `folder` row の layout 検査へ進み、file row / directory row の欠落、`date` / `parent` 不一致、期待外 row を修復対象にする。列挙基盤が使えない場合は対象 file を再 materialize する方向へ倒し、個別 file ごとの `File.Exists` を repair target selection の通常経路にはしない。status が current の場合、外部から LR2 `folder` row の値だけを書き換えたケースまでは監査しない。
