...
出力先は既定の通常出力先 `LR2CustomFolderOutputBaseDir`、追加通常出力先 `LR2CustomFolderAdditionalOutputBaseDirs`、root 用の `LR2CustomFolderOutputBaseDirRootType` と、playlist の `output_dir` / `custom_folder_output_base_name` から決まる。追加通常出力先のフルパスは user.config の JSON string array に保存し、表示名と DB 保存値は出力先フォルダの末尾 directory 名を使う。LR2 linked profile では追加通常出力先を LR2 `config.xml` の `<jukebox>` BMS search root として同期し、登録時は追加、設定から削除された時は当該 path を自動削除する。追加通常出力先はアプリ管理 custom folder root であり、登録済み BMS search root や他の custom folder 出力先と同一または親子関係になる配置は設定入口で拒否する。`playlist.custom_folder_output_base_name` が NULL の場合は既定の通常出力先、登録済み追加出力先名の場合はその追加出力先を使う。保存名が設定から消えている場合は実効値として既定の通常出力先へ fallback するが、設定 UI から追加出力先を削除した場合は参照 playlist を NULL へ戻し、リネームした場合は新しいフォルダ名へ更新する。`ignore_folder_output` により level/user/alphabet/clear などの folder 種別を除外できる。
