...
`PlaylistPropertyDialog` is an owner-modal edit surface for one playlist property session. Its navigation is presentation state only: the window opens with `General` selected, exposes exactly the three top-level categories `General`, `Folder`, and `Custom Folder`, and never persists the selected category. Navigation is a single-select WPF `ListBox` using the shared top-navigation resources. The selected item has the common bottom selection indicator and common focus / disabled states, and the standard `Selection` / `SelectionItem` automation peers remain available. Left / Right, Home, and End move selection and keyboard focus through the categories; a disabled Custom Folder item is not selectable. The category header, selected content, and scroll viewport are separate presentation regions. Changing category resets the content viewport to its top, while values already edited in each category remain in the active edit session. Reopening a dialog starts at `General` and creates no navigation persistence.
