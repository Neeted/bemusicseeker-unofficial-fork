...
アプリ管理 playlist の custom folder は `playlist` / `playlist_entry` / `playlist_course` と出力設定が正本であり、出力済み `.lr2folder` ファイルの存在だけを正本にしない。出力先変更、root 出力切替、entry/folder 編集、明示的な LR2 song.db 同期データ再同期では、playlist 正本から `.lr2folder` file と LR2 `folder` row を再 materialize する。起動時 repair は期待 file path、物理 file の存在と mtime、LR2 `folder` row の date / projection 定義を見て rewrite 対象を決め、`.lr2folder` 本文を全量読み直さない。期待される階層 directory や file が欠落している場合や、管理 playlist directory 配下に期待 projection 外の LR2 `folder` row だけが残っている場合は構造不一致として rewrite 対象にするが、構造・mtime・DB row が一致しているのに file 本文だけが異なるケースは no-op として扱う。起動時に物理 `.lr2folder` が欠落している場合も、欠落 table をまとめて batch materialization に流し、LR2 `folder` row は単発 sync で収束させる。手動再同期の playlist materialization は、全対象 playlist の期待 `.lr2folder` projection を作り、物理ファイルは差分だけ書き換え、期待 projection に含まれない管理 playlist directory 配下の `.lr2folder` は削除し、LR2 `folder` row は batch sync として 1 回で収束させる。table ごとに既存出力を削除して DB sync を繰り返さない。stage 開始、table 単位 projection、batch materialization / sync 完了を performance log と status bar に出し、設定画面全体を同期的に無効化して隠れた長時間処理にしない。外部ツールが作った `.lr2folder` は、アプリ管理 playlist directory の外にある場合だけ LR2 song.db 同期側の discovery result として扱う。LR2 song.db 同期が playlist materialization 後に実行される場合は、materialization 前に捕捉した startup scan surface を再利用せず、新しい `.lr2folder` file surface を使う。
