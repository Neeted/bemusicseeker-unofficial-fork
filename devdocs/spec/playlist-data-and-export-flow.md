...
プレイリストサマリー行の context menu には一括変更 UI がある。対象は選択行 snapshot で、`output_dir`、playlist 名、URI、`compat_prefix`、folder 順序は扱わない。ダイアログ全体の OK は持たず、custom folder 出力種別、OUTPUT、root folder、外部同期、`.bmt` 出力それぞれの「選択行へ一括適用」だけが変更を発生させる。OUTPUT は `custom_folder_output_base_name` だけを変更し、ROOT ON の playlist でも保存値は更新するが実効出力先は root 出力先のままにする。custom folder 出力種別は tri-state で、checked は出力、unchecked は非出力、indeterminate は変更なしを表す。`entry_type = Folder` の playlist では `LevelFolder` は単体プロパティと同じく出力不可として扱う。外部同期 ON は `BMSTable.EnableExternalSync()` の実結果を採用し、URL 不備の playlist は警告なしで OFF のまま残す。custom folder 出力種別の一括変更は `playlist.ignore_folder_output` だけを header 保存し、LR2 linked profile では対象 playlist の custom folder projection を batch materialization / batch DB sync に流す。OUTPUT / root folder の変更では、影響する playlist の旧出力先を prune scope に含めて新出力先へ移行し、`.bmt` 出力予約は行わない。custom folder 出力種別も `.bmt` 投影には影響しないため `.bmt` 出力予約は行わず、処理中はステータスバーに custom folder 出力の件数進捗と処理中 playlist 名を表示する。外部同期と `.bmt` 出力の一括変更は `.bmt` 投影または出力対象に影響するため、既存の `.bmt` 同期経路へ流す。
