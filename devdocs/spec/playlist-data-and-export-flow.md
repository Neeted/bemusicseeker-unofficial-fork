...
| `output_dir` / `custom_folder_output_base_name` / `is_root_folder` / `ignore_folder_output` | LR2 custom folder 出力設定。`custom_folder_output_base_name` は通常出力時に使う追加出力先名で、NULL は既定の通常出力先を表す。 |
...
外部表の新規ロード、またはプレイリストサマリーの外部データによる初期化では、header に `compat_prefix` が明示されている場合は空文字や CP932 範囲外の文字を含めてその値を初期値として保存する。`compat_prefix` が無い場合は header の `tag` を優先し、無ければ `symbol` を候補にする。選択した候補を LR2 の custom folder 名として CP932 で厳密にエンコードできる場合だけそのまま採用し、範囲外の文字を含む場合は文字の削除や置換、次候補の再試行をせず `LEVEL ` を初期値にする。候補がどちらも無い場合も `LEVEL ` とする。`level_order` / `folder_order` / data JSON の `level` 値が数字か記号かは初期値判定には使わない。`compat_prefix` が無い header の `folder_order` は、読み込まれた entry folder と一致する値は materialized folder 名として保持し、一致しない値は compatible level 名として初期 `compat_prefix` で materialize する。保存時は従来通り header に `compat_prefix` を出力するため、一度保存された table では推定値が明示値として固定される。外部同期更新で既存 playlist を読み直す場合は、source header に明示 `compat_prefix` があっても DB に保存済みの `compat_prefix` を維持し、既存 DB の値は移行しない。外部データで初期化を明示的に実行した場合だけ外部値を再推定する。
...
起動・ファイル差分リロード・初期化再実行の前に、LR2 連携モードでは設定済みの通常・追加・ルート型出力ベースをすべて検査する。ディレクトリとして読取り可能であり、自己所有の新規一時ファイルを作成・書込み・削除できることを要求する。利用不能なベースを検査や起動時の自動修復で作成して成功扱いにしない。設定保存など明示的な操作による既存の作成機能は維持する。

管理ベース配下の playlist ごとの子フォルダや `.lr2folder` が未生成であることは、この事前検査の失敗にしない。既定通常出力ベースの chart / resource 走査互換と、追加・ルート型の chart 走査除外を維持する。standalone では残存する LR2 出力先設定を理由に初期化・リロードを止めない。失敗は用途と対象ベースを示して設定のプレイリスト項目へ案内し、BMS 登録の削除とは区別する。
...
出力先は既定の通常出力先 `LR2CustomFolderOutputBaseDir`、追加通常出力先 `LR2CustomFolderAdditionalOutputBaseDirs`、root 用の `LR2CustomFolderOutputBaseDirRootType` と、playlist の `output_dir` / `custom_folder_output_base_name` から決まる。既定通常出力先は user.config の値を正本にしつつ、従来版で楽曲と同居していた可能性があるため LR2 BMS search root として chart / resource scan root に残す。ただし楽曲インストール先としては扱わず、一般タブの BMS directory 一覧、インストールタブの新規インストール先候補、library folder tree node からは除外する。追加通常出力先のフルパスは user.config の JSON string array に保存し、表示名と DB 保存値は出力先フォルダの末尾 directory 名を使う。LR2 linked profile では既定通常出力先と追加通常出力先を LR2 `config.xml` の `<jukebox>` BMS search root として同期する。ただし追加通常出力先と root 用 base directory / root playlist 出力 directory はアプリ管理 custom folder root として chart / resource scan root、一般タブの BMS directory 一覧、インストールタブの新規インストール先候補、library folder tree node から除外する。既定通常出力先、追加通常出力先、root 用 base directory は他の custom folder 出力先と同一または親子関係になる配置を設定入口で拒否する。新しく既定通常出力先を選択する場合、user.config で既に通常出力先として管理されていない LR2 `<jukebox>` 登録済み root と同一または親子関係になる path は validation error とし、保存時確認による採用にはしない。最終的な `<jukebox>` の BMS search root 同士に同一・親子関係の重なりがある状態は異常として扱い、手動編集などで既に存在する場合は保存時 validation で是正を促す。親 BMS root 配下の一部 directory だけを chart / resource scan から除外する通常仕様は持たない。既定通常出力先を変更する場合は、変更前の通常出力先そのものが `<jukebox>` から外れ、通常出力先として自動追加される search root ではなくなることを値設定時に通知する。追加通常出力先または root 用出力先が登録済み BMS search root と同一または親子関係になる配置は、再セットアップで既存 `<jukebox>` を BeMusicSeeker 管理出力先として採用するケースがあるため、保存時に警告し、ユーザーが続行した場合だけ許可する。保存時に採用された追加通常出力先 / root 用出力先は通常楽曲の検出対象ではなく、カスタムフォルダ出力専用の管理領域として扱う。`playlist.custom_folder_output_base_name` が NULL の場合は既定の通常出力先、登録済み追加出力先名の場合はその追加出力先を使う。保存名が設定から消えている場合は実効値として既定の通常出力先へ fallback するが、設定 UI から追加出力先を削除した場合は参照 playlist を NULL へ戻し、リネームした場合は新しいフォルダ名へ更新する。`ignore_folder_output` により level/user/alphabet/clear などの folder 種別を除外できる。新規ローカル playlist と、外部 URL から新規追加する通常外部 table の `ignore_folder_output` 初期値は user.config の `PlaylistDefaultIgnoreFolderOutput` を使う。既存 table の外部同期・再読み込みでは既存 table の値を引き継ぎ、推定表 playlist とおすすめ表 playlist はそれぞれ専用の固定初期値を使う。

LR2 linked profile の起動時には、user.config の既定通常出力先と追加通常出力先をアプリ管理 custom folder root の正本として扱い、LR2 `config.xml` の `<jukebox>` から欠けている場合は補完する。root 出力は user.config の root 用 base directory 自体ではなく、`playlist.is_root_folder` と `playlist.output_dir` から決まる各 playlist/table 出力 directory を BMS root 相当として扱う。root 出力 playlist/table directory は物理 directory がまだ存在しない場合も期待 `<jukebox>` root として扱い、必要に応じて directory を作成してから登録する。保存時と起動時には root 用 base directory や stale child を残さず、playlist/table 出力 directory へ収束させる。起動時の root 出力先収束は playlist 一覧が必要なため、起動前 validation ではなく playlist load 後に実行する。`ReloadTables` 時は root 出力 playlist/table directory の補完を行うが、root 用 base directory や stale child の削除まで含む完全な収束は保存時または次回起動時の修復で行う。
...
| 種別 | bit | `PlaylistDefaultIgnoreFolderOutput` の既定 | 推定表 playlist の既定 | おすすめ表 playlist の既定 |
| --- | ---: | --- | --- | --- |
| User | `0x001` | 出力 | 出力 | 出力 |
...
`playlist_custom_folder_output_status` は、本アプリが最後に custom folder materialization と LR2 `folder` row sync を完了した playlist の durable no-op 判定である。status は物理出力の高速 no-op 判定を主責務とし、LR2 `folder` row が後から外部変更されていないことまでは保証しない。status には、`output_directory`、root flag、出力 bit、entry type、folder sort、未送信出力設定、header/data hash、`last_update`、実出力 surface から作る物理 `.lr2folder` file / 出力 directory / 格納先 ancestor directory の mtime 署名だけを保存する。起動時 repair は先に playlist header と status を読み、現在の入力 identity と output directory 単位の物理署名が一致する playlist は、原則として playlist entry hydration / projection / DB row lookup / materialization 対象にしない。status が無い、入力が変わった、物理 `.lr2folder` が欠けた、物理 `.lr2folder` / 出力 directory / 格納先 ancestor directory の mtime が変わった場合は pending とし、pending の場合だけ playlist 正本から軽量 projection を作って LR2 `folder` row の layout 検査へ進み、file row / directory row の欠落、`date` / `parent` 不一致、期待外 row を修復対象にする。列挙基盤が使えない場合は対象 file を再 materialize する方向へ倒し、個別 file ごとの `File.Exists` を repair target selection の通常経路にはしない。status が current の場合、外部から LR2 `folder` row の値だけを書き換えたケースまでは通常監査しない。ただし root 出力 playlist/table directory を `<jukebox>` に補完した直後は、未登録状態で起動した LR2 が当該 root directory row を削除している可能性があるため、root 出力 playlist/table directory 自身の `folder` row だけを exact path lookup で確認する。この検査は末尾 directory separator 付きの root directory `path`、`type = 1`、`parent = e2977170` の最小条件に限定し、中間 directory row や `.lr2folder` row の全件監査には広げない。不一致の場合は当該 playlist を pending に戻し、既存の batch materialization / LR2 `folder` row sync で収束させる。

起動時に物理 `.lr2folder` が欠落している場合も、欠落 table をまとめて batch materialization に流し、LR2 `folder` row は単発 sync で収束させる。手動再同期の playlist materialization は、全対象 playlist の期待 `.lr2folder` projection を作り、物理ファイルは差分だけ書き換え、期待 projection に含まれない管理 playlist directory 配下の `.lr2folder` は削除し、LR2 `folder` row は batch sync として 1 回で収束させる。table ごとに既存出力を削除して DB sync を繰り返さない。stage 開始、table 単位 projection、batch materialization / LR2 `folder` table sync / status 保存完了を performance log と status bar に出し、設定画面全体を同期的に無効化して隠れた長時間処理にしない。カスタムフォルダ操作内の LR2 `folder` table sync は汎用の LR2 `song.db` 同期とは別の責務として扱い、UI 文言も分離する。外部ツールが作った `.lr2folder` は、アプリ管理 playlist directory の外にある場合だけ LR2 song.db 同期側の discovery result として扱う。外部 `.lr2folder` discovery は app-managed playlist output directory を列挙 API の除外 subtree として渡し、管理領域配下の大量 `.lr2folder` を候補化してから捨てたり、外部 sync の prune scope で管理 row を materialize したりしない。scan surface に保持した app-managed 除外 scope と diff preparation 時点の current scope が異なる場合は、旧 scope で除外済みの候補をそのまま使わず current scope で `.lr2folder` discovery を再実行する。LR2 song.db 同期が playlist materialization 後に実行される場合は、materialization 前に捕捉した startup scan surface を再利用せず、新しい `.lr2folder` file surface を使う。

LR2 song.db の generated-data preparation は一度の explicit admission で outer mutation lease を取得し、その lease から作った capability を concrete runtime の playlist / builtin nested bridge だけに渡す。request / DTO / coordinator / ordinary helper は capability-free semantic operation とし、busy の場合は待機・内部 retry・lease 解放後の自動再開をせず false を terminal に返す。lease 解放後の再実行は新しい明示 request に限る。preparation の stage / table / batch progress は既存 `BMSLibrary` facade dispatcher queue の latest-state 配信を使い、lease 保持中に public subscriber を同期実行しない。dispatcher drain 内の subscriber 例外はログ後に継続し、物理 `.lr2folder`、LR2 `folder` row、DB status の durable 結果を変更しない。
...
プレイリストサマリー行の context menu には一括変更 UI がある。対象は選択行 snapshot で、通常の一括変更では URI と folder 順序は扱わない。ダイアログ全体の OK は持たず、custom folder 出力種別、OUTPUT、root folder、外部同期、`.bmt` 出力、外部データによる初期化それぞれの操作ボタンだけが変更を発生させる。外部データによる初期化では、選択された playlist 名、symbol、`compat_prefix`、`output_dir` だけを、外部同期フラグに関わらず対象 playlist の外部 URL から再取得した header/data に基づいて再設定する。外部 URL が読めない playlist は警告なしでスキップする。`output_dir` の初期化は DB 保存値を NULL 相当に戻し、実効フォルダ名は初期化後の playlist 名を Shift-JIS 互換化しファイル名禁止文字を除去した値にする。`compat_prefix` の初期化は新規ロード相当の推定値を使い、保存済み `compat_prefix` の維持処理は適用しない。`compat_prefix` 変更で folder 名衝突が起きる playlist や、LR2 linked profile で実効 `output_dir` が空または他 playlist と重複する playlist は、その playlist の初期化をスキップする。外部データ取得は選択範囲リロードと同じ外部取得経路・同時実行制御を使い、取得フェーズを先に完了させてからプロパティ適用と必要な custom folder 移行を行う。`compat_prefix` 変更で entry folder projection が変わる playlist は、対象 entry を table ごとに個別保存せず、複数 playlist を 1 transaction の batch 保存に流す。実効出力先が変わる playlist は旧出力先 prune を含む custom folder 移行 batch に流し、実効出力先は同じだが playlist 名や entry folder projection により custom folder 内容が変わる playlist は同一出力先再出力 batch に流す。外部データによる初期化中は、外部取得、適用、DB 保存、custom folder 出力、一覧再構築までステータスバーの進捗表示を維持し、操作ボタンが再び有効になる前に進捗だけが消えた状態を作らない。`.bmt` 投影へ影響する playlist は最後にまとめて `.bmt` 出力キューへ登録し、custom folder 再出力の単件 API から副次的に `.bmt` 出力予約を発生させない。OUTPUT は `custom_folder_output_base_name` だけを変更し、ROOT ON の playlist でも保存値は更新するが実効出力先は root 出力先のままにする。custom folder 出力種別は tri-state で、checked は出力、unchecked は非出力、indeterminate は変更なしを表す。`entry_type = Folder` の playlist では `LevelFolder` は単体プロパティと同じく出力不可として扱う。外部同期 ON は `BMSTable.EnableExternalSync()` の実結果を採用し、URL 不備の playlist は警告なしで OFF のまま残す。custom folder 出力種別の一括変更は `playlist.ignore_folder_output` だけを header 保存し、LR2 linked profile では対象 playlist の custom folder projection を batch materialization / batch DB sync に流す。OUTPUT / root folder / `output_dir` の変更では、影響する playlist の旧出力先を prune scope に含めて新出力先へ移行し、`.bmt` 出力予約は行わない。custom folder 出力種別も `.bmt` 投影には影響しないため `.bmt` 出力予約は行わず、処理中はステータスバーに custom folder 出力の件数進捗と処理中 playlist 名を表示する。外部同期、`.bmt` 出力、playlist 名、symbol、`compat_prefix` の一括変更は `.bmt` 投影または出力対象に影響するため、既存の `.bmt` 同期経路へ流す。
...
## Playlist property dialog presentation contract

`PlaylistPropertyDialog` is an owner-modal edit surface for one playlist property session. Its navigation is presentation state only: the window opens with `General` selected, exposes exactly the three top-level categories `General`, `Folder`, and `Custom Folder`, and never persists the selected category. Navigation is a single-select top-navigation role using the shared top-navigation resources; its WPF selector/control/template implementation may vary. The selected item has the common bottom selection indicator and common focus / disabled states, and the standard `Selection` / `SelectionItem` automation peers remain available. Left / Right, Home, and End move selection and keyboard focus through the categories; a disabled Custom Folder item is not selectable. The category header, selected content, and scroll viewport are separate presentation regions. Changing category resets the content viewport to its top, while values already edited in each category remain in the active edit session. Reopening a dialog starts at `General` and creates no navigation persistence.

The editable inventory is unchanged. `General` contains playlist `name`, `symbol`, `compat_prefix`, the entry type list, external synchronization, and Page / Header / Data URLs. `Folder` contains the folder sort key list, ascending / descending order, extended folder order, automatic sorting, move-up / move-down commands, and natural sorting. `Custom Folder` contains the output base, `output_dir`, root-folder option, and the thirteen output flags `AllSongsFolder`, `UserFolder`, `LevelFolder`, `AlphabetFolder`, `ClearFolder`, `DJLevelFolder`, `CategoryAllFolder`, `OtherFolder`, `RandomFolder`, `BpmSortFolder`, `BpSortFolder`, `PlayCountSortFolder`, and `LastPlaySortFolder`. Output flags retain their semantic help tooltips. Editable values use TwoWay bindings; owner-provided lists and informational content flow from owner to view.

Page composition follows the shared unboxed Settings section grammar. `General` gives the playlist name and each source URI the full editing width, places symbol and compatibility prefix in a roomy shared row, and keeps entry unit as a distinct field before external synchronization. `Folder` presents chart sorting first; the folder-order list then owns its own vertical scrolling and gains height when the dialog is made taller while its move actions remain reachable. Its page uses the body viewport as a responsive minimum, not an exact height: at minimum window size, long explanatory text creates reachable outer vertical extent without removing the list's independent scroll extent. `Custom Folder` presents Output Folder first, with its explanation in the section heading hierarchy before the thirteen flags, and follows it with Output Destination. The destination editors stretch with the dialog. Both sections remain in natural top-origin flow, so increasing only the window height does not move Output Destination downward. The flags use two equal wrapping columns, including at minimum width. General and Folder remain horizontally contained at minimum width, and no page uses horizontal scrolling.
...
The reset-backed native title-bar close contract is covered by `MainWindowPlaylistWorkspaceWpfTests.PlaylistPropertyDialog_NativeCloseRequestClosesAfterSynchronousReset` in the `serial-state-b` / `ClassLevel` Functional route. The fixture uses the actual MainWindow playlist-summary Property command and owner-modal window; completion is observed from the window `Closed` signal, modal-route return, and existing DataContext / active-dialog cleanup signals.
