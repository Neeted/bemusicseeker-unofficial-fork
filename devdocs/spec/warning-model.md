...
- `Lr2Compatibility` は BMS の maintenance facts が評価済みなら category 単位で再構築します。`Lr2PathEncodingUnsupported` は譜面 path / folder scan path が CP932 で表せない場合、`Lr2PathTooLong` は LR2 の legacy path 長を超える可能性がある場合、`Lr2ResourcePathUnsupported` は resource raw/resolved path が CP932 非対応の場合、`Lr2ResourcePathTooLong` は resource path が LR2 の legacy path 長を超える可能性がある場合に付与します。親ディレクトリ参照は OpenLR2 が相対パスとして扱えるため、LR2 非対応理由にはせず、解決時に LR2 へ渡る path の byte 長 / CP932 判定だけに含めます。maintenance facts が未評価の row では、既存の direct warning を placeholder attach だけで消しません。
