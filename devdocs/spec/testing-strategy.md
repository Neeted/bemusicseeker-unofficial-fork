...
Functional の orchestration は、exclusive portable-settings lane完了後に `lr2-songdb-sync`、`settings-edit-foreground-classwide`、`settings-window-nonactivating-classwide` を各exact 1回、専用の別 processで起動し、3 entryを同じ ownership ledgerへ直ちに記録してから `method-level-pre-wave` を開始する。3 entryはpre-waveと重なり、pre-wave完了時点でもrunningなら同じentryを `remaining` / early settings以外の named shard fanoutとも重ねる。LR2 shardは1 worker / `ClassLevel` の `BmsLibraryLr2SongDbSyncTests` だけを含み、settingsの2 shardも各1 worker / `ClassLevel`で、各process-localな `Application`、`Settings.Default`、resources、testごとにGUIDで一意な temporary directory / song databaseを所有するため、pre-waveおよびfanoutのfixtureと共有 mutable stateを持たない。pre-wave完了直後に3 early entryの状態を確認し、completed / exit 0は一度だけ結果へaccountし、runningは同じentryを共通の結果・global deadline・diagnostics / process-tree cleanupへjoinする。nonzero exit、canceled、invalid stateならdiagnosticsを保存して全entryをcleanupし、fanoutを起動せず失敗する。early / fanoutのlaunch failure、pre-wave failure、任意のshard failure、timeout、例外では記録済みentryとraw process ownershipを重複起動せず、同じcleanup setで回収する。
...
Functional の名前付き shard は、それぞれ別 testhost/process で ownership を分離するが、性能上は `remaining` shard と並行起動する。exclusive portable-settings lane完了後にLR2とsettingsの3 early entryを各1回起動し、entryとraw process ownershipを記録してから専用 `method-level-pre-wave` を実行する。pre-wave完了直後にcompleted / exit 0のearly entryは一度だけaccountし、runningは同じentryを維持したまま `remaining` とearly settings以外のnamed shardを起動する。nonzero / canceled / invalid stateはfanout前に失敗させ、3 early entryと部分起動済みfanoutを同じglobal deadline・result・diagnostics・cleanup setへ合流させる。process start後のentry構築例外もraw identityを失わず、primary failureをcleanup failureで置換しない。runnerは実際に起動へ渡す同一 shard object / arrayをlaunch前validatorへ渡し、early membership、fanout exclusion、exact object identity、remaining exclusion、cross-route uniqueness、重複なしを実行用構成そのもので検証する。
