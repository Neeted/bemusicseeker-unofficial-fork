...
library/chart initialization、metadata、startup library construction は、`BmsLibraryZeroNoteRefreshTests` と、chart metadata 5 fixture、library initialization 5 fixture、startup construction 3 fixtureを同じ `library-chart-classwide` testhost/process に割り当て、`ClassLevel` scope の6 workerで実行する。exact 14 class membership、6 worker、`ClassLevel` scope、remaining shardからの assigned-class exclusion、他 routeとの重複なしを test 起動前に独立した allowlist で検証する。これは同一 process 内の owner fixture scheduling を広げる変更であり、named shard の process 数、exclusive portable-settings lane、`method-level-pre-wave` の先行順、diagnostics / process-tree cleanup、既存の160秒 test-phase観測と180秒 command budgetは変更しない。
