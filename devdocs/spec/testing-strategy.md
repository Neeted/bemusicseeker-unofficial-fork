...
library/chart initialization、metadata、startup library construction は、`BmsLibraryZeroNoteRefreshTests` と、chart metadata 1 fixture、library initialization 5 fixture、startup construction 3 fixtureを同じ `library-chart-classwide` testhost/process に割り当て、`ClassLevel` scope の6 workerで実行する。exact 10 class membership、6 worker、`ClassLevel` scope、remaining shardからの assigned-class exclusion、他 routeとの重複なしを test 起動前に独立した allowlist で検証する。ChartInfoの5 owner-local source fileは、hydration / backfill / installがprocess ThreadPoolへscheduleするbackground completionを同期watchdogで観測するため、単一partial `ChartInfoMetadataOwnerTests` fixtureを構成してClassLevelで直列化する。library / startup owner fixtureは引き続き独立にscheduleする。named shard のprocess数、exclusive portable-settings lane、`method-level-pre-wave` の先行順、diagnostics / process-tree cleanup、既存の160秒test-phase観測と180秒command budgetは変更しない。
...
owned chart collection / playlist summary / regular chart owner coverage is assigned to the existing `owned-chart-collection` launch route. It contains the seven `OwnedChartCollection*Tests` fixtures, four `PlaylistSummary*Tests` fixtures, and the five Unit 4c regular-chart fixtures listed in the chart abstraction verification map. The route starts one process with exactly six `ClassLevel` workers; its exact class allowlist is validated before launch and all 16 classes are excluded from `remaining`. The split preserves each fixture's GUID-owned database / filesystem, immutable snapshot, mutation or presentation completion signal, failure watchdog, cleanup, category, and DNP contract. The retired `OwnedChartCollectionStateTests`, `PlaylistSummaryAggregationTests`, and `RegularChartListOwnerTests` selectors are absent, and no additional process, fixed wait, timeout change, or production seam is introduced.
...
### Unit 4d-C: feature-global safety owner map

`feature-process-global-state` は15 launch shard / 14 retained fanout descriptor / 14 fanout launch object / 1 LR2 early entryのtopologyを変えず、実在する class-wide `DoNotParallelize` owner 5 classだけを1 worker / `ClassLevel`で保持する。保持対象は `InstalledOnlyResourceOverwriteValidationTests`、`LibraryFileScanPipelineOwnerTests`、`Lr2PlayHistorySchemaUiTests`、`MainWindowExternalShellTests`、`PlayHistoryReadModelTests` である。以前の11 class（`AudioContractsTests`、`AudioDeviceTestWorkflowOwnerTests`、`BmsLibraryInstallEstimationServiceTests`、`CatalogMutationOwnerTests`、`ChartListVirtualViewTests`、`InstallDestinationStateOwnerTests`、`Lr2PlayHistorySchemaServiceTests`、`MainWindowViewModelStartupProgressTests`、`PlaylistOperationNotificationOwnerTests`、`PlaylistUrlAcquisitionOwnershipTests`、`PlaylistUrlCompletionTests`）は専用routeのclass selectorとremaining exclusionから除外し、既存remainingで一度だけdiscoverする。process数、pre-wave、LR2 early、180秒command budget、170秒process deadline、10秒cleanup reserve、logical test set、DNP属性は変更しない。

`Assert-FunctionalShardConfiguration` は実際の `New-FunctionalShardPlan` objectに対して、5 classのexact membership、11 classのdedicated-route不在 / remaining復帰、1 worker / `ClassLevel`、remaining exclusion、cross-route uniquenessを検証する。`VerificationRunnerContractTests` はこのactual planを実行し、15 / 14 / 14 / 1のtopologyを確認したうえで、5 classのMSTest / class-wide `DoNotParallelize` metadataと11 classのMSTest discovery / non-DNP metadataをreflectionで検証する。既存 fixtureのcompletion signal、resource cleanup、failure watchdogには変更を加えない。
