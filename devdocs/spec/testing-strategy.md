...
Functional の名前付き shard は、それぞれ別 testhost/process で ownership を分離するが、性能上は `remaining` shard と並行起動する。専用 lane である `method-level-pre-wave` だけは exclusive lane 完了後に先行し、その完了後に `remaining` と名前付き shard を起動する。したがって「別 process であること」と「並行起動しないこと」は別の契約であり、後者は pre-wave にのみ適用する。
...
- `settings-presentation-classwide` includes `SettingsWindowPresentationTests` and `SettingDialogEditCompletionTests`; their six foreground-sensitive methods remain explicit `TestWindowActivation.ForegroundInteraction` cases and are class-serialized with the other settings/presentation fixtures in the same testhost. A separate foreground shard is not created. allowlist の追加、visible / foreground window が必要な受入検証、OS focus policy に依存する操作は通常の Functional へ安易に追加せず、まず Full / release acceptance の責務として分離できるか検討する。待機時間の延長や固定 sleep を foreground 成功条件にしない。
