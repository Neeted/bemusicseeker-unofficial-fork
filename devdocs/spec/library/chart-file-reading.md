...
```mermaid
flowchart TB
    Read["上限付き読取り：バイト列とファイル情報"] --> Parse["並列：同じバイト列でハッシュ生成・軽量解析"]
    Parse --> Post["並列後処理：保守・詳細情報を準備"]
    Post --> ResultQueue["上限付き待ち行列：変更不能な結果"]
    ResultQueue --> Aggregate["単一集約：入力順に結果を取り込む"]
    Aggregate --> CommitQueue["上限付き待ち行列：順序確定済みの保存候補"]
