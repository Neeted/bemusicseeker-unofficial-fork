...
```mermaid
flowchart TB
    Bms["BMS：song"] -- "hash（MD5）" --> Map["chart_digest_map"]
    Map --> Sha["SHA-256"]
    Bmson["BMSON：bmson_song"] -- "sha256" --> Sha
    Sha --> Info["chart_info：sha256（主キー） / md5（補助照合）"]
    Bms -. "hash（MD5）で補助照合" .-> Info
    Bmson -. "md5で補助照合" .-> Info
    Info --> Projection["セッション索引から表示へ解決"]
```
