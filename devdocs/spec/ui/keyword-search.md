...
入力欄からShift+Tabを押すと、選択中の保存済み検索式の最初の行内ボタンへ移ります。対象がなければ表示中の最初の操作可能な行を使います。お気に入りは削除、履歴は追加、削除の順です。Tab / Shift+Tabでボタンと入力欄の間を移動しても候補を閉じません。ボタンはEnter / Spaceで操作でき、Escで入力欄へ戻って候補の再表示を抑制します。
