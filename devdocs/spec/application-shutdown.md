...
自動アップデートでは、更新パッケージのダウンロード後、updater の起動情報を先に作成する。この時点で updater exe の作業ディレクトリへのコピーも済ませる。その後、通常終了と同じ `PrepareShutdownAsync` を通し、終了準備が完了してから updater process を起動して `Application.Current.Shutdown()` を呼ぶ。updater は現在の PID 終了を待ってから上書きを開始する。
...
- 起動/リロード用 `_semaphore`、startup background task、drop install、playlist build、prewarm、BMSLibrary/BMSPlaylist の終了待ち対象、LR2 DB process lock が idle になるまで待つ
...
read-only DB 接続そのものはグローバルな operation lease にまだ統合していない。既知の read-only worker は終了待ち対象に含めるが、将来新しい read-only DB 処理を追加する場合は shutdown 対象に登録する必要がある。
