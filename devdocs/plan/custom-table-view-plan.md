...
- `FOLDER` rename は playlist 行では不可、保留インストール選択中も不可とし、commit は `GridRowResolver.TryGetFolderEditChartOperationTarget` で chart capability を確認してから `MainWindowViewModel.RenameChartFolder` へ委譲する。owned bmson row は BMS row と同じ chart folder rename 経路で扱う。
