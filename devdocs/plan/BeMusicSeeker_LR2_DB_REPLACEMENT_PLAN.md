...
- Phase 8 は `lr2_full_generation_status` table の単一 row (`name = "default"`) から開始する。
  ...
  この段階は chart_info full build / LR2 compatibility warning / text group freshness の完全統合前であるため、sync 後も `Completed` にはせず、`Incomplete`
  (`remaining_backfill_stages_not_implemented`) を記録する。startup-scan blocker diagnostic と、
  missing chart_info build / LR2 compatibility warning / text group mutation freshness が入るまで完全生成完了とは扱わない。
