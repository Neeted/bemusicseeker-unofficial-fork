...
- `DialogPresentationTests` now identifies every materialized production action button by a stable internal `AutomationProperties.AutomationId`; expected and actual identifiers must be nonempty, unique, and have an exact one-to-one match. Layout traversal order, display copy, geometry, and child order are not used as the button identity oracle. The production builder used by `ThemedMessageBox.ShowWithStatus` assigns the same result-specific IDs used by its presentation fixture.
- Button role assertions resolve the most-specific canonical family from the effective style chain before accepting a Settings alias. Therefore a Neutral expectation rejects `SettingsPrimaryButtonStyle`, `SettingsQuietButtonStyle`, and `SettingsDangerButtonStyle`, while specialized expectations require their exact family.
- Overlay assertions retain only a nonempty untemplated descendant and the same-host canonical overlay role/effective style-chain checks; fixed opacity, fill, and hit-test values are not asserted.
