...
- Final amendment maps Property categories from public SelectionItem choices to visible authority-backed binding paths without control/container-type coupling, extends the allowlisted foreground route with Left/Right and disabled SelectionItem rejection, and exercises overflow adoption in TextBox/ListBox/popup and the canonical bulk dialog. The final restored target Quick `tests-quick-20260828-060001` passed 104/104; a wrong-initial-category mutant failed at the General content-role oracle (`tests-quick-20260828-055736`), and a consumer template mutant failed at canonical scrollbar materialization (`tests-quick-20260828-055245`). Both mutations were restored.
- Final bounded amendment removes private scrollbar child-name dependencies. Line/page affordances are found by their public RoutedCommand and Invoke Automation role, invoked from a verified interior offset, and required to move the actual viewer offset in the expected direction. Property helpers now capture the initially selected SelectionItem before mutation, map it through authority-backed visible content, and verify the authority-bound custom-folder availability host against the provider's public Automation role/bounds plus disabled-selection rejection without localized text. Restored target Quick `tests-quick-20260828-064300` passed 104/104; a no-op affordance invocation failed the line-start offset oracle (`tests-quick-20260828-062408`), and a temporary non-General initial selection failed the captured-provider oracle (`tests-quick-20260828-062500`). Both mutations were restored.
- Final Context/Chart scrollbar amendment retires private line/page child names and Track child-property authority. The canonical Context consumer resolves line/page affordances through public RoutedCommand/ICommandSource and Invoke Automation, invokes each from an interior offset, and asserts strict actual viewer movement and scrollbar-value synchronization for both orientations; Chart retains materialized track orientation plus adoption/theme/visibility coverage. Restored adjacent Quick `tests-quick-20260828-065639` passed 86/86; a no-op affordance invocation failed the line-start offset oracle (`tests-quick-20260828-065423`) and was restored.
