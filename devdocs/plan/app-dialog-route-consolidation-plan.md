...
| Unit 7: DispatcherMessageBox Retirement | In progress | View code-behind の `DispatcherMessageBox.Show` は `UiDialogRoute.ShowMessageBox` へ移行中。次は ViewModel の `DispatcherMessageBox` / OK `ConfirmationMessage` 通知 route を coordinator へ寄せる。 |
