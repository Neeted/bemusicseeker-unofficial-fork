...
| Unit 2: Message / Confirmation Route Replacement | In progress | Legacy entrypoint `DispatcherMessageBox` と Livet `Themed*InteractionMessageAction` を coordinator-backed に置換済み。譜面ビューア登録確認と設定系 decision confirmation は coordinator route へ直接置換済み。次は残りの playlist sync / app schema / play temp install decision call site を消す。 |
