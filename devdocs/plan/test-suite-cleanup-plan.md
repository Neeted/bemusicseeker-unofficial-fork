...
| 2B | WPF shell behavior + `SourceTextTestHelper` retirement | pending | — | pending | pending | pending | Unit 2B-1 evidence recorded below; remaining 2B units are still pending. |
...
#### Unit 2B-1 (external process/settings source-text cleanup)

- Scope: removed exactly the five approved brittle methods `AppRestartUsesApplicationPathSnapshotAndRestartGateway`, `ExternalPlayersUseProcessGatewayInsteadOfOwningRawProcessRoutes`, `ExternalPlayersUseWindowHostForEmbeddingOperations`, `PlayerSettingsGatewayUsesTechnologyNeutralContracts`, and `BmiIdxView_RegistersSuppliedExitHandlerForAutoAdvance` from the four owned test fixtures. The `SourceTextTestHelper` definition and all out-of-scope consumers were left unchanged; no production, runner, helper, playlist, or reflection unit was edited.
- Typed replacement coverage: `UbmplayUsesWindowHostForAttachAndKeyMapping` now asserts the `uBMplay` discovery name, executable path, `-SP` chart argument, and normal window style through the recording process gateway. `BmiIdxViewInvokesSuppliedExitHandlerWhenProcessRaisesExited` supplies an `EventHandler`, invokes the recording session's `RaiseExited()`, and asserts one callback with the session sender and `EventArgs.Empty`.
- Owned-file audit: the five approved method names remaining in the four test fixtures: `0`; `SourceTextTestHelper` references remaining in those fixtures: `0`. The four test files changed by `0/11`, `33/56`, `0/21`, and `0/9` added/deleted lines respectively; the plan is the only additional changed path.
- Focused Quick passed `70/70` tests, `0` failed, in 16.5 s test time / 43.8 s command time, using `tests-quick-20260821-020006`; no Functional run was performed for this subunit.
- `git diff --check` passed. Targeted `dotnet format whitespace` verification for the four changed test files passed. The full-solution format verification was also run but reported pre-existing whitespace diagnostics in unrelated `SettingDialogEditCompletionTests.cs`, `SettingDialogOpenCommandTests.cs`, and `SettingsWindowPresentationTests.cs`; those files were not changed. No commit or staging was performed.

#### Unit 2B-2 (external-shell cohort)

- Scope: retired the two applicable source-text methods present in the owned `ExplorerOpenServiceTests.cs` fixture: `MainWindowExplorerContextMenusUseExplorerOpenService` (one `SourceTextTestHelper` consumer) and `ShellDirectoryOpenDoesNotUseSelectionApi` (one raw `File.ReadAllText` consumer). No third applicable source-text method was present in this owned file; no outside fixture was modified to satisfy the count discrepancy. Existing typed `ExplorerOpenService` behavior tests remain intact.
- Composed replacement coverage: added `MainWindowExternalShellTests.LibraryFolderContextMenu_OpensDirectoryThroughComposedExternalShellGateway`, which constructs the public `ApplicationComposition`/`MainWindow`, supplies the public application `vm` resource contract, raises the typed library-folder context-menu item, and asserts the injected `IExternalShellGateway.OpenDirectory` request. Its recording fake independently captures `OpenFileAndSelect` and the test asserts that the directory route makes zero file-selection calls. The test uses the existing typed shutdown completion task and a dispatcher-frame barrier for cleanup; it adds no production API, reflection, fallback, or fixed completion wait. `[DoNotParallelize]` documents the process-wide WPF resource dictionary and dispatcher boundary.
- Owned-file audit after the change: applicable source-text methods remaining: `0`; `SourceTextTestHelper` references remaining in `ExplorerOpenServiceTests.cs`: `0`; raw `File.ReadAllText` references remaining in `ExplorerOpenServiceTests.cs`: `0`. The helper definition, production code, runner, playlist, and reflection units were not changed.
- Reviewer correction added independent `SelectFilePaths`/`OpenFileAndSelect` recording and zero-count assertions to every relevant directory-open behavior in the Explorer fixture and the composed MainWindow fake.
- Focused Quick passed `14/14` tests, `0` failed, in 3.1850 s test time / 18.2 s command time, using `tests-quick-20260821-022647`; after the reviewer correction, the final focused Quick again passed `14/14` in 3.4017 s test time / 17.7 s command time, using `tests-quick-20260821-023340`.
- Functional acceptance triplet passed `3,969/3,958/11` (total/passed/opt-in skipped), `0` failed, with every command under the 180 s budget: `tests-functional-20260821-023541` in 160.3 s, `tests-functional-20260821-023832` in 154.2 s, and `tests-functional-20260821-024116` in 159.8 s. All three runs retained the unchanged tracked fingerprint prefix `D3F545...`; no `testhost` or `vstest` process remained after the runs.
- Final `git diff --check` and targeted `dotnet format whitespace --verify-no-changes` verification passed; no commit or staging was performed.
