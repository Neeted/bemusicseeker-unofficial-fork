...
- 統合時に通常拡張子unregister:trueの実model coverage不足を確認。rootはBmsLibraryPackageInstallServiceTestsへの配置追加を許可し、RenameBMSFilesExtensions_UnregistersOnlySuccessfulChartsAndPreservesHashOwnerで既存packetの登録解除・同hash別owner・last owner・FS失敗対象保持・warm索引/旧snapshotを補完した。判定の意味は変更していない。既存unregister:falseの2失敗caseは代用にせず維持する。
