...
> Historical note: 現行実装では、LR2 `song.db` に app-owned tables を初回連携用に追加するだけの場合は警告対象外とする。警告対象は、既存 `playlist_entry` schema の変更、既存 `bmson_app_schema` version の更新、または version row が無い状態で既存 `chart_digest_map` / `bmson_song` を現行化する場合に限定する。
