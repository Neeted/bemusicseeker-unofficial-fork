...
> 2026-09-14追記: 本計画で維持した「packageごとの公開」および一部のper-item durable receiptは、その後採用した `1 user operation = 1 mutation session / N changes` 契約で置き換える。現行の確定・公開契約は [mutation session 仕様](../spec/library-mutation-boundary.md#mutation-session-契約) を正本とする。本計画はU1～U5の履歴記録として残す。
