...
U5c1 fresh reviewは修正必須指摘なし、P2解消。helper修正とfacts移行を別commitとして記録し、U5c2へ進む。

U5c1レビュー修正後のFunctional `tests-functional-20260914-043732` は250.9秒成功。fresh reviewへ渡す。

U5c1レビューP2: mergeの既定policyがNotifyへ変わり、移動だけの種類へstorage row全同期が追加される取り落としを修正。mergeはSuppressed、auto renameは捕捉済みpolicyを明示。実BMS cleanup＋BMSON移動でBMSのみ通知、FS/DB集合を確認。Quick40件・関連275件成功、公開前提を補完したためFunctionalを再実行する。

U5c1 Functional `tests-functional-20260914-041052` は257.9秒で成功。凍結した全差分（独立commit用にstageしたsetup helper修正を含む）を独立レビューへ渡す。

U5c1はtyped facts/reportへの移行、旧Deltaとroot任意apply/helper退役を実装。Catalog ownerの汎用追加引数からDB/storage/ownedまでの到達不能な追加枝も削除し、実installed upsert/scan replacementは保持した。fake-only削除以外の必要coverageは実Force/estimated install、削除、rename、ReloadFileDiffへ移行。testが索引deltaを自作して反映を代行する案は撤回した。同hash集合のversion/summary cache再利用を実same-bytes追加で確認する。

U5c1の最終関連Quickと統合Quickは成功。統合時に旧HEADでも再現したsetup通知待ち問題は、OwnedChartCollectionTestSupportのsetter実行を既存TestUiDispatcherHostへ移し、worker自身のlocal queueを同期待機で塞がないよう修正。同一521件・Workers12/ClassLevelで成功（`tests-quick-20260914-040757`、58.9929秒）。timeout/DNP/worker数は変更せず、本番PreferFairness案は撤回。共有helperのこの差分だけを独立commitへ分離する。resolver toolが実装workerで利用不可だったためworker/rootで同じ診断手順を代替した。

U5c2への受入残件: 実Force/estimated入口のprimary-only事前warmは確認したが、操作自体が必要とするfull lookupとtarget反映単体の不要構築は分離できていない。共通target反映を実managerへ移した後、実callerと同じtarget/capability/receiptを通す境界で補完する。手作り索引deltaや任意rootapplyを再導入しない。
...
- 既存file ownerをLibraryMutationOwnerへ再編・改名する。共通mutation result/storage/installed receipt、factsからの組立て、semantic index反映、dispatch、file facts apply core、installed targetのrequired completion、scan/digest/maintenanceの共通反映を同ownerへ移す。rootの同義実装を削除し、producerからこの管理主体へ直接接続する。名前だけの変更やrootへの大きな反映callbackは不可。
- Catalog/OwnedCollection/StorageRows/ResourceHealth/ResourceIndex/PackageLifecycle/InstallDestination/LR2等は既存ownerへの明示依存で使う。BMSLibrary本体や広いhost/interfaceを渡さない。既存rootが所有するparent/duplicate/install-estimation metadata cacheの命令とUI通知は、必要な狭い接続に限定して残せるが、共通facts判定・反映順序をroot側へ戻さない。各接続の理由を引継ぎで列挙する。
- 操作内のmutable phase bookkeepingは管理主体のprivate実装に限定する。他ownerへmutable result/collectionを公開せず、既存immutable receiptまたは確定した公開内容・deferred actionを渡す。即時/解放後/batch終端の既存経路を明示し、外向き多数booleanの同義包み直しをしない。新command bus/queue/global gate/永続stateや既存許可操作の一律Busy化はしない。
- BMSLibraryはpublic facade/composition、既存currentness同期と専門cache、表示側の通知接続を保持する。state/lock移動済み索引は戻さない。所有範囲は既存ownerのpartial、BMSLibraryの該当commonとpackage/scan/metadata橋渡し、composition callerと同契約testへ限定。実装は一人直列、必要ならcommon反映移管とfile/target completionの移管をさらに直列で検証する。
- U5c packetの公開/失敗/prefix/LR2/batch/旧snapshotを再利用。target共通ownerの実入口でprimary-only/hash-only/coldの残件を補完する。通知方針P2の種類別実mergeも最終受入に含める。最終状態で全操作→要求→writer→索引→公開の対応表、旧API残存0と残す専門callbackの理由を更新する。
