...
| C | [P0-02 BMSLibrary facade](./P0-02_BMSLibrary_ドメインFacade化計画.md) | `BMSLibrary` を compatibility facade として薄くする | `REF-MVP-C72` completed checkpoint / 次は `REF-MVP-C73` maintenance dispatch bridge review |
...
1. Lane C 後続候補: `REF-MVP-C73` として C72 後に `DispatchMaintenanceHydrationResult` の bridge を薄くできるか、または `OwnedChartCollectionMutationResult` / `ResourceHealthIndexDispatchResult` の contract 境界を先に整理すべきかを 1 件に絞る。
