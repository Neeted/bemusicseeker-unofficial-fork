...
| C | [P0-02 BMSLibrary facade](./P0-02_BMSLibrary_ドメインFacade化計画.md) | `BMSLibrary` を compatibility facade として薄くする | `REF-MVP-C69` completed checkpoint / 次は `REF-MVP-C70` resource health mutation contract extraction |
...
1. Lane C 後続候補: `REF-MVP-C70` として `ResourceHealthIndexUpdateMode` / `ResourceHealthIndexMutation` / `BuildMaintenanceResourceHealthIndexMutation` を top-level internal contract / planner へ移し、remaining private reflection tests を direct planner test へ寄せる。
