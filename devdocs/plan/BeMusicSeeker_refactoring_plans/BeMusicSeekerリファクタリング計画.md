...
| C | [P0-02 BMSLibrary facade](./P0-02_BMSLibrary_ドメインFacade化計画.md) | `BMSLibrary` を compatibility facade として薄くする | `REF-MVP-C94` completed implementation / 次は `REF-MVP-C95` installed chart storage targets apply seam |
...
1. Lane C 後続候補: `REF-MVP-C95` として `ApplyInstalledChartStorageTargets` の upsert workflow を top-level coordinator + host seam へ移し、該当 private reflection helper を削減する。
