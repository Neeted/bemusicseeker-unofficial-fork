...
| C | [P0-02 BMSLibrary facade](./P0-02_BMSLibrary_ドメインFacade化計画.md) | `BMSLibrary` を compatibility facade として薄くする | `REF-MVP-C100` completed checkpoint / 次は `REF-MVP-C101` diagnostics storage row seed seam |
...
1. Lane C 後続候補: `REF-MVP-C101` として `_BMSFiles` / `_BmsonSongs` direct field set を diagnostics storage row seed seam へ置き換える。
