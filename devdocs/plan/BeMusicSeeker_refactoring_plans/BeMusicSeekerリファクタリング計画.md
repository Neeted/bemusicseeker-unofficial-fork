...
| C | [P0-02 BMSLibrary facade](./P0-02_BMSLibrary_ドメインFacade化計画.md) | `BMSLibrary` を compatibility facade として薄くする | `REF-MVP-C67` completed checkpoint / 次は `REF-MVP-C68` resource maintenance target contract extraction |
...
1. Lane C 後続候補: `REF-MVP-C68` として `ResourceMaintenanceTargetSet` / `StorageRowsVersionSnapshot` を top-level internal contract へ移し、nested type reflection test helper を direct contract 利用へ寄せる。
