...
| C | [P0-02 BMSLibrary facade](./P0-02_BMSLibrary_ドメインFacade化計画.md) | `BMSLibrary` を compatibility facade として薄くする | `REF-MVP-C77` completed checkpoint / 次は `REF-MVP-C78` resource health dispatch result contract extraction |
...
1. Lane C 後続候補: `REF-MVP-C78` として `ResourceHealthIndexDispatchResult` を top-level internal contract へ移す。`OwnedChartCollectionMutationResult` と stale full target private reflection test はまだ残してよい。
