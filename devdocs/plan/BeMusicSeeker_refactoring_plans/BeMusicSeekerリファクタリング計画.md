...
| C | [P0-02 BMSLibrary facade](./P0-02_BMSLibrary_ドメインFacade化計画.md) | `BMSLibrary` を compatibility facade として薄くする | `REF-MVP-C96` completed checkpoint / 次は `REF-MVP-C97` library file scan storage mutation seam |
...
1. Lane C 後続候補: `REF-MVP-C97` として `ApplyLibraryFileScanStorageMutation` workflow を top-level coordinator + host seam へ移し、該当 private reflection helper を削減する。
