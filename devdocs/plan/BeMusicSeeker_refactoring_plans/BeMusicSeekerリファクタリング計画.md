...
| `BMSLibrary.cs` | 18,301 行 | 12,000 行以下 |
...
| C | [P0-02 BMSLibrary facade](./P0-02_BMSLibrary_ドメインFacade化計画.md) | `BMSLibrary` を compatibility facade として薄くする | `REF-MVP-C14` completed checkpoint / 後続は workflow 単位で再計画 |
...
1. Lane C 後続候補: `REF-MVP-C14` 完了後の input builder / scan surface follow-up、maintenance、file operation などから workflow 単位で 1 件だけ active ticket 化。
