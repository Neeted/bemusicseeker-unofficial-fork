...
| C | [P0-02 BMSLibrary facade](./P0-02_BMSLibrary_ドメインFacade化計画.md) | `BMSLibrary` を compatibility facade として薄くする | `REF-MVP-C71` completed checkpoint / 次は `REF-MVP-C72` maintenance hydration resource health mutation planner seam |
...
1. Lane C 後続候補: `REF-MVP-C72` として maintenance hydration full rebuild 用 resource health mutation 構築を `ResourceHealthIndexMutationPlanner` へ寄せる。`DispatchMaintenanceHydrationResult` 本体と `OwnedChartCollectionMutationResult` はまだ root に残す。
