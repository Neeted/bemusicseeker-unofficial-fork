...
| C | [P0-02 BMSLibrary facade](./P0-02_BMSLibrary_ドメインFacade化計画.md) | `BMSLibrary` を compatibility facade として薄くする | `REF-MVP-C93` completed checkpoint / 次は `REF-MVP-C94` owned collection read-only snapshot seam |
...
1. Lane C 後続候補: `REF-MVP-C94` として owned collection / installed lookup の read-only snapshot diagnostics facade を追加し、該当 private reflection helper を削減する。
