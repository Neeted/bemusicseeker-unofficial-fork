...
| C: BMSLibrary domain facade 化 | `REF-MVP-C27: LR2 sync scan surface selection boundary` | completed checkpoint | [P0-02](./P0-02_BMSLibrary_ドメインFacade化計画.md) |
...
現在の active ticket: なし。`REF-MVP-C27` completed checkpoint。

次にやる 1 件: Lane C を続ける場合は `REF-MVP-C28: LR2 sync directory target selection boundary` として、`CreateLr2SongDbSyncInput` に残る normal directory metadata targets / LR2 folder parent directory targets / merged directory entry targets の選択を dedicated selection DTO と helper 経由へ寄せる。重すぎる場合は maintenance、folder/file operation、package install follow-up のいずれか 1 件を workflow 単位で active 化する。
