...
- APP-01 の残作業: custom-folder output の mode gating / migration / prune / materialization / status が一つの operation snapshot を使う境界、`MainWindowViewModel` の summary/property/remove before/after path capture と LR2Config 直接 mutation、`PlaylistPropertyDialogViewModel` の search-root mutation、`BMSPlaylist` と `SettingDialogViewModel` の残る global settings 取得、settings dialog の open/edit/save/reload/shutdown behavior test。これらを一つの巨大 facade にせず、production 経路へ接続する用途別縦切りで継続する。
