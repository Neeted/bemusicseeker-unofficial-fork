...
| C: BMSLibrary domain facade 化 | `REF-MVP-C22: LR2 folderInfo candidate selection boundary` | completed checkpoint | [P0-02](./P0-02_BMSLibrary_ドメインFacade化計画.md) |
...
`REF-MVP-C22` として `CreateLr2SongDbSyncInput` の folderInfo candidate selection と enumeration 時の text metadata candidate capture を `BmsLibraryInternal/Lr2SongDbSyncFolderInfoCandidateSelection.cs` の top-level internal DTO 境界へ切った。`BMSLibrary.cs` は 18,319 行、追加 DTO は 10 行。build、LR2 sync 関連 tests、format、diff check、Roslynator 対象確認、静的レビュー、full test は完了。静的レビューの指摘は新規 DTO の add 漏れ注意のみで、commit 対象に含めることで解消する。

現在の active ticket: なし。`REF-MVP-C22` completed checkpoint。

次にやる 1 件: Lane C を続ける場合は directory entry selection follow-up を 1 ticket だけ active 化する。重すぎる場合は maintenance、folder/file operation、package install follow-up のいずれか 1 件を workflow 単位で active 化する。
