...
| C: BMSLibrary domain facade 化 | `REF-MVP-C37: LR2 sync input builder boundary review` | completed checkpoint | [P0-02](./P0-02_BMSLibrary_ドメインFacade化計画.md) |
...
現在の active ticket: なし。`REF-MVP-C37` completed checkpoint。

次にやる 1 件: `REF-MVP-C38: LR2 sync input folder candidate selection builder ownership` として、folder candidate selection を builder / internal helper 側へ移し、`CreateLr2SongDbSyncFolderCandidateSelection` delegate を builder constructor から外す。app-managed output scope 採取、directory target selection、DB schema、setting name、private reflection entry point は触らない。
