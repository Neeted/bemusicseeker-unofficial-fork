...
| C: BMSLibrary domain facade 化 | `REF-MVP-C23: LR2 directory entry selection boundary` | completed checkpoint | [P0-02](./P0-02_BMSLibrary_ドメインFacade化計画.md) |
...
`REF-MVP-C23` として `CreateLr2SongDbSyncInput` の directory entry selection と missing count を `BmsLibraryInternal/Lr2SongDbSyncDirectoryEntrySelection.cs` の top-level internal DTO 境界へ切った。`BMSLibrary.cs` は 18,337 行、追加 DTO は 13 行。build、LR2 sync 関連 tests、format、diff check、Roslynator 対象確認、静的レビュー、full test は完了。初回 full test で `CreateLr2SongDbSyncInputWithoutScanSurface_ExcludesManagedOutputDirectoryFiles` が 1 回失敗したが、同テスト単体 rerun、LR2 targeted rerun、full test rerun は pass した。

現在の active ticket: なし。`REF-MVP-C23` completed checkpoint。

次にやる 1 件: Lane C を続ける場合は `CreateLr2SongDbSyncInput` builder / request composition follow-up を 1 ticket だけ active 化する。重すぎる場合は maintenance、folder/file operation、package install follow-up のいずれか 1 件を workflow 単位で active 化する。
