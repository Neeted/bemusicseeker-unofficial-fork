...
| C: BMSLibrary domain facade 化 | `REF-MVP-C59: merge / install boundary after repair seam` | completed checkpoint | [P0-02](./P0-02_BMSLibrary_ドメインFacade化計画.md) |
...
現在の active ticket: なし。`REF-MVP-C59` completed checkpoint。

次にやる 1 件: `REF-MVP-C60: MergeChartDirectory coordinator seam` として、`MergeChartDirectory` の orchestration を dedicated coordinator seam へ移す。source-text test は root method body 固定ではなく、新しい coordinator 配置を検査する形へ更新する。`installChartPackages` callback 契約、`MoveChartPackageFiles` core behavior、maintenance result apply は触らない。
