...
`REF-MVP-A1` の続きとして、`PlaylistMainViewApplyResult` を追加し、`ApplyPlaylistDetailViewRowsToMainView` の `out` timing を戻り値 contract に変えた。rebuild / view-only 両経路が同じ main view apply result を読めるようになり、次 slice の finalize 共通化に進みやすくなった。

次にやる 1 件: 標準確認とサブエージェントレビューで重大指摘がなければこの slice を commit し、次 slice で rebuild / view-only finalize 共通化を進める。
