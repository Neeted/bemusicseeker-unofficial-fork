...
| C: BMSLibrary domain facade 化 | `REF-MVP-C19: LR2 song.db sync prepared surface selection boundary` | completed checkpoint | [P0-02](./P0-02_BMSLibrary_ドメインFacade化計画.md) |
...
`REF-MVP-C19` として `CreateLr2SongDbSyncInput` の pending prepared surface 取得と scan surface 適用済み判定を `BmsLibraryInternal/Lr2SongDbSyncPreparedSurfaceSelection.cs` の top-level internal DTO 境界へ切った。`BMSLibrary.cs` は 18,259 行、追加 DTO は 20 行。build、LR2 sync 関連 tests、format、diff check、Roslynator 対象確認、静的レビュー、full test は完了。

現在の active ticket: なし。`REF-MVP-C19` completed checkpoint。

次にやる 1 件: Lane C を続ける場合は LR2 folder candidate selection / input builder follow-up を 1 ticket だけ active 化する。重すぎる場合は maintenance、folder/file operation、package install follow-up のいずれか 1 件を workflow 単位で active 化する。
