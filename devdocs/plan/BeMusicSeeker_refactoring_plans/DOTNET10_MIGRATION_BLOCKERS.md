...
| CONC-01 | Mutation serialization / lock ordering | LR2 synchronization ownerのfacade-fed lock・mutable stateをCatalogMutationOwnerへ移管し、file-operation workflowもpurpose-specific mutation scope、ChartFile projection、canonical mutation deltaへ閉じた | canonical ownerのguard / transaction / live apply / receipt順序を維持し、owner間はimmutable request / receipt /用途別capabilityで接続する。別ownerのlock / mutable stateをhostで列挙しない | `GATE-01 B2` | cancellation、synchronization primitive、failure / shutdown orderingを再検証 | boundary met |
...
| UIH-01 | WPF + WinForms + WebBrowser / COM | WPF Dispatcher型は`WpfUiScheduler` / `WpfPlaybackUiDispatcher`のterminal adapterに限定し、Model / ViewModel / library / playlistはtechnology-neutralな`IUiScheduler` contractを利用する | WPF / WinForms / COM型をView / view-host adapterの外へ出さない。View固有code量は問題にせずdependency directionで判定する | `GATE-01` | WinForms継続、WebBrowser / WebView2、System.Drawingの扱いを決定 | boundary met |
