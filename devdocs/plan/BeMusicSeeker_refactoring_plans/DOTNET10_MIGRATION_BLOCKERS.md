...
| CONC-01 | Mutation serialization / lock ordering | LR2 synchronization ownerのfacade-fed lock・mutable stateをCatalogMutationOwnerへ移管し、typed request / receipt、scoped transaction writer、failure sinkへ閉じた。file-operation ownerのprivate lock・mutable state残件をB2で閉じる | canonical ownerのguard / transaction / live apply / receipt順序を維持し、owner間はimmutable request / receipt /用途別capabilityで接続する。別ownerのlock / mutable stateをhostで露出しない | `GATE-01 B2` | cancellation、synchronization primitive、failure / shutdown orderingを再検証 | open |
...
| UIH-01 | WPF + WinForms + WebBrowser / COM | WPF / WinForms / COM型の大半はView / view-host adapter内にあるが、Modelの`IUiScheduler.Dispatcher`と`BMSLibrary` / `BMSPlaylist`の直接`Dispatcher`操作が残る | WPF / WinForms / COM型をView / view-host adapterの外へ出さない。View固有code量は問題にせずdependency directionで判定する | `UI-05` / `MIG-03` / `GATE-01` | WinForms継続、WebBrowser / WebView2、System.Drawingの扱いを決定 | open |
