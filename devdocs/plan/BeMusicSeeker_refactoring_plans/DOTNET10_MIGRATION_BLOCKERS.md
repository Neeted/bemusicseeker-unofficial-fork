...
|---|---|---|---|---|
| `TFM-01` | in-progress | updaterと2 toolsが`net472`。app／testsは`net10.0-windows` | `NET10-01` | 全projectのtarget matrix、Release build／test |
...
N1ではapp／testsの`net10.0-windows` restore／Release build、settings／code-page／startup DB-open targeted tests、repository Release executableのstartup smokeが通っている。これはapp／testsのbaseline evidenceであり、updater、2 tools、dependency replacement、Self-contained publish、existing data、updater acceptanceを完了した証拠ではない。
