...
| `PRF-006` | manual | `OBSERVABILITY_REQUIRED` | post-init forced GC | synthetic componentだけでは削除判断不能。generation、queue depth、直前／直後managed bytes、collection delta、pauseをcurrent markerへ追加 | 削除を推測せず`MANUAL-02` |
