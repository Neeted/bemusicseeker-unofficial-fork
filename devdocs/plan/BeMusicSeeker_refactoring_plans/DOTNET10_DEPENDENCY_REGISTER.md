...
| `DEP-NATIVE-01` | Everything3_x64 3.0.0.9／EverythingBridge_x64 (tracked first-party source) | x64 bridge＋SDKの固定native ship setを`native`へretain。bridgeはabsolute sibling load、ABI／contract probe、active-call lease、SDK shutdownを所有 | SHA-256、PE x64、export／delegate ABI、installed／absent behavior、mandatory project／publish path、MIT／source | `NET10-06 A4` |
...
## NET10-06 A4 closure evidence

| ID | Source / license | Version / ABI authority / runtime owner | Evidence |
|---|---|---|---|
| `DEP-NATIVE-01` | `native/Everything3_x64.dll` 3.0.0.9 / MIT (`third_party/licenses/16-Everything3-MIT.txt`), SHA-256 `BE25B01C73BBF359B50DDF30255133225F93B4BC40A8D208173319373BCDAA5C`; `native/EverythingBridge_x64.dll` first-party tracked source / MIT project license, SHA-256 `24863BFD06BFDFE0419DF767152575ADCD38B4104DC2063ED411AA7F956AA835` | `EverythingNative` loads only `native\\EverythingBridge_x64.dll` from the application base directory; the bridge loads only its sibling `Everything3_x64.dll` by absolute path. The bridge requires the six scan/free exports plus `EBridge_Shutdown`; SDK delegates use the fixed x64 `__stdcall` ABI and bridge exports use `__cdecl`. `EverythingNative` holds an active-call lease and invokes shutdown before releasing the bridge module. | `EverythingNativeRuntimeTests` pins both PE x64 images and hashes, verifies missing-bridge fallback without current-directory probing, and runs the production scan route twice to prove export／SDK load, service-absent fallback, shutdown, and reload. The same test passes with `BMS_EVERYTHING_NATIVE_RUNTIME_ROOT` set to the actual win-x64 self-contained publish folder. `ManagedDependencyOutputPolicyTests` verifies the mandatory project items and Release `native` layout. The bridge build exports `EBridge_EnumerateGroupedFiles`, `EBridge_FreeGroupedFilesResult`, `EBridge_FreeResult`, `EBridge_FreeSourceRootsResult`, `EBridge_ScanChartAndResources`, `EBridge_ScanSourceRoots`, and `EBridge_Shutdown`; the x64 native build completed with 0 warnings / 0 errors. |
