...
未所持譜面のプレイリスト詳細では、ファイル操作やインストール操作は表示されず、`BMS-IRを開く` / `Mochaを開く` / `MinIRを開く` / `本体URLを開く` / `差分URLを開く` / `譜面ビューアで開く` / `ランキングデータの更新` / `エントリー削除` など、プレイリスト行に対する操作だけが表示されます。`BMS-IRを開く` は MD5 がある行でのみ表示されます。
