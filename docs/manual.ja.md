...
完全生成中は、所持譜面と LR2 `song.db` を同時に変更する操作が一時的に止まる場合があります。ステータスバーに失敗や未完了が表示された場合は、`再試行` を押してください。必要に応じて設定画面の `LR2完全生成データを再同期` から、所持譜面とプレイリストを正本として `song` / `folder` を再生成できます。
...
`LR2用song.dbをBeMusicSeekerで完全生成する` はデフォルト有効の設定です。従来は DB 上にアプリから値を埋めていない項目が存在していました、LR2 が必要とする情報をできるだけ埋めるようになります。この設定が有効の場合、LR2 のデータベース自動更新を手動のみで運用していた場合でも、BeMusicSeeker 側の差分更新によって LR2 側の余計なスキャンを抑制します。
...
![LR2互換性警告](img/一覧_LR2非対応パス.PNG)

`LR2互換性警告` には、Shift_JIS / CP932 で表現できない譜面パス、LR2 の互換パス長を超える可能性がある譜面、参照リソースの raw / resolved path が LR2 で扱えない譜面など、LR2 側で選曲や再生に支障が出る可能性がある譜面が表示されます。
...
`LR2互換性警告` に表示されていないか確認してください。Shift_JIS / CP932 で表現できない文字を含むパスや、LR2 が扱いにくいリソース参照を含む譜面は、LR2 側で選曲または再生できない可能性があります。
