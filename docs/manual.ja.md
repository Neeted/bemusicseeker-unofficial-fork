...
期間は `すべて`、`今日`、`昨日`、`最近 7 日`、`最近 30 日`、`未確定 / 診断` です。日付範囲はローカル日付の 0:00 区切りで、最近 7/30 日は今日を含みます。`未確定 / 診断` は未確定行も含め、schema、score DB、読み取り、投影の問題を確認するときに使います。
...
ユーザー定義では、プレイリスト全体をまとめた `ALL` と、プレイリスト内フォルダ別の `.lr2folder` が出力されます。クリア / DJ レベルは状態別のサブフォルダに `ALL` とフォルダ別の `.lr2folder` を出します。ランダムを有効にすると、ユーザー定義、レベル、クリア、DJ レベルに `#MAXTRACKS 1` の RANDOM 版が追加されます。同じ場所に出る通常版と RANDOM 版は、通常版が先、RANDOM 版が後ろにまとまる順番で出力されます。クリア状態別フォルダは LR2 の TITLE 順表示でも状態順に並ぶよう、`0 NO PLAY` から `7 P.A` までの番号付きフォルダ名になります。BPM 順は `chart_info.mainbpm`、BP 順は `score.minbp`、プレイ回数順は `score.playcount` を使って並び替えます。
