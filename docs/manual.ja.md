...
`追加通常出力先` には、プレイリストごとに選べる通常出力先を登録できます。登録名は出力先フォルダ名そのもので、プレイリストサマリーの `OUTPUT` 列やプロパティ画面ではこの名前で選択します。LR2 連携モードでは追加通常出力先も LR2 の BMS ルートに同期され、削除すると BMS ルートからも自動で外れます。他のカスタムフォルダ出力先と同じ場所、または親子関係になる場所は指定できません。追加出力先を削除する場合、その出力先を使っているプレイリストは通常出力先へ戻ります。
`ルートフォルダ出力先` には、プレイリストのプロパティで `ルートフォルダにする` を有効にしたものが出力されます。選曲画面のルートに表示されるため、よく使う表へ素早くアクセスできます。

LR2 に既に登録されている BMS ルートと同じ場所、または親子関係になる場所を出力先に指定すると、保存時に確認が表示されます。続行すると、その場所は通常楽曲の検索先ではなく BeMusicSeeker のカスタムフォルダ出力管理下として扱われます。出力更新やクリーンアップで既存ファイルが削除される場合があるため、重要なデータを置かないでください。以前 BeMusicSeeker で使っていた出力先を、新しくダウンロードした本アプリで再登録する場合は、この確認を承認して採用できます。
