...
複数行を選択して右クリックメニューの `一括変更...` を開くと、選択行に対してカスタムフォルダ出力種別、ルートフォルダ、外部同期、`.bmt` 出力をまとめて変更できます。ダイアログ全体の OK はなく、各セクションの `選択行へ一括適用` を押した項目だけが変更されます。カスタムフォルダ出力種別は、チェックありが出力、チェックなしが非出力、中間状態が変更なしです。カスタムフォルダ出力種別の一括適用中はステータスバーに進捗が表示され、`.bmt` は再生成されません。外部同期を ON にした場合、URL 情報が不足しているプレイリストは警告なしで OFF のままになります。
...
プレイリストの `プロパティ` から、出力名、`ルートフォルダにする`、出力するフォルダ種別を設定します。複数プレイリストの出力種別や `ルートフォルダにする` は、プレイリストサマリーの `一括変更...` から選択行へ一括適用できます。

出力先には重要なデータを置かず、通常出力先とルートフォルダ出力先は別の場所を指定してください。出力時は各プレイリスト出力フォルダ配下の `.lr2folder` を BeMusicSeeker 管理ファイルとして書き直し、不要になったものを削除します。通常出力先やルートフォルダ出力先の直下にある別フォルダは触りませんが、プレイリスト出力フォルダ内に手作業の LR2 カスタムフォルダを同居させると削除される可能性があります。BeMusicSeeker 専用の空フォルダを用意することをおすすめします。
