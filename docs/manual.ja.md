...
`追加通常出力先` には、プレイリストごとに選べる通常出力先を登録できます。登録名は出力先フォルダ名そのもので、プレイリストサマリーの `OUTPUT` 列やプロパティ画面ではこの名前で選択します。LR2 連携モードでは追加通常出力先が LR2 の BMS ルートにも登録され、削除すると BMS ルートからも自動で外れます。登録済み BMS ルートや他のカスタムフォルダ出力先と同じ場所、または親子関係になる場所は指定できません。追加出力先を削除する場合、その出力先を使っているプレイリストは通常出力先へ戻ります。
