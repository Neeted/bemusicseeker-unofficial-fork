...
public sealed class AudioDeviceCatalogTests
{
    ...
    public void Refresh_ReflectsConnectedAndDisconnectedFakeDevices()
    {
        ...
        enumerator.Results[BassAudioPlayer.DeviceDriver.DIRECT_SOUND] =
        [new("Speakers", "speaker-1", 3, true)];
        var catalog = new BassAudioDeviceCatalog(enumerator);

        catalog.Refresh();
        IReadOnlyList<AudioDeviceInfo> first = catalog.GetDevices(AudioDriver.DirectSound);
        ...
        catalog.Refresh();
        IReadOnlyList<AudioDeviceInfo> second = catalog.GetDevices(AudioDriver.DirectSound);
    ...
    public void Refresh_BackendFailureRetainsOnlyThatBackendsLastGoodList()
    {
        ...
        Assert.AreEqual("headphones", catalog.GetDevices(AudioDriver.DirectSound)[1].Driver);
