...
public sealed class BmsLibraryPendingPackageRegroupTests
{
    ...
    public void TryRegroupPendingPackagesForSourceDirectories_DoesNotRegroupWhenDirectoryPackageAlreadyExists()
    {
        ...
        WithTemporaryLibrary(delegate (string tempRootPath, string songDbPath, BMSLibrary library)
        {
            ...
            ChartPackage directoryPackage = new ChartPackage
            {
    ...
    public void SearchEstimatedInstallationDirectory_LowConfidenceLeavesInstallDestinationEmptyAndShowsRepresentativeMetadata()
    {
        ...
        WithTemporaryLibrary(delegate (string tempRootPath, string songDbPath, BMSLibrary library)
        {
            ...
            ChartPackage pendingPackage = new ChartPackage(new[] { pendingFile })
            {
    ...
    public void SearchEstimatedInstallationDirectory_MixedPackageUnresolvedInstalledDestination_DefersWithoutFallbackEstimation()
    {
        ...
        WithTemporaryLibrary(delegate (string tempRootPath, string songDbPath, BMSLibrary library)
        {
            ...
            ChartPackage pendingPackage = new ChartPackage(new[] { pendingInstalledA, pendingInstalledB, pendingMissing })
            {
                path = sourceDirectoryPath,
                delete_parent = false
            };
    ...
    public void SearchMergeDestinationForPendingPackage_ResolvedPathUpdatesRepresentativeMetadata()
    {
        ...
        WithTemporaryLibrary(delegate (string tempRootPath, string songDbPath, BMSLibrary library)
        {
            ...
            ChartPackage pendingPackage = new ChartPackage(new[] { pendingFileA, pendingFileB })
            {
                path = sourceDirectoryPath,
                delete_parent = false
            };
    ...
    public void SearchMergeDestinationForPendingPackage_ResolvesBmsonFromInstalledBmsonHash()
    {
        ...
        WithTemporaryLibrary(delegate (string tempRootPath, string songDbPath, BMSLibrary library)
        {
            ...
            ChartPackage pendingPackage = new ChartPackage(new BMSFile[] { pendingBmson })
            {
                path = sourceDirectoryPath,
                delete_parent = false
            };
    ...
    public void SearchMergeDestinationForPendingPackage_ResolvesMixedBmsAndBmsonPackage()
    {
        ...
        WithTemporaryLibrary(delegate (string tempRootPath, string songDbPath, BMSLibrary library)
        {
            ...
            PendingChartEntry pendingBmson = PendingChartEntry.CreateFromFilePath(pendingBmsonPath);
            ChartPackage pendingPackage = new ChartPackage(new BMSFile[] { pendingBms, pendingBmson })
            {
                path = sourceDirectoryPath,
                delete_parent = false
            };
    ...
    public void SearchMergeDestinationForPendingPackage_MixedSplitInstalledDirectoriesUsesMostMatchingDirectory()
    {
        ...
        WithTemporaryLibrary(delegate (string tempRootPath, string songDbPath, BMSLibrary library)
        {
            ...
            PendingChartEntry pendingBmson = PendingChartEntry.CreateFromFilePath(pendingBmsonPath);
            ChartPackage pendingPackage = new ChartPackage(new BMSFile[] { pendingBmsA, pendingBmsB, pendingBmson })
            {
                path = sourceDirectoryPath,
                delete_parent = false
            };
    ...
    private static void AssertPendingPackagePaths(BMSLibrary library, params string[] expectedPaths)
    {
        ...
        CollectionAssert.AreEquivalent(
            expectedPaths,
            library.ChartPackagesPending.Select((ChartPackage package) => package.path).ToArray());
    }
    ...
    {
        foreach (BMSFile file in (packages ?? Array.Empty<ChartPackage>()).Where((ChartPackage package) => package != null).SelectMany((ChartPackage package) => package.ChartFiles ?? new List<BMSFile>()).Where((BMSFile file) => file != null))
        {
