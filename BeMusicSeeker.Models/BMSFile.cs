...
public class BMSFile : LR2SongDB.song
{
	...
	public LR2ScoreDB.score.ClearType clear
	{
		get
		{
			...
			{
				return LR2ScoreDB.score.ClearType.NO_SONG;
			}
			if (bmsScore == null)
			{
				return LR2ScoreDB.score.ClearType.NO_PLAY;
			}
			if (bmsScore.clear >= LR2ScoreDB.score.ClearType.EASY && rank == LR2ScoreDB.score.RankType.INVALID)
			{
				return LR2ScoreDB.score.ClearType.INVALID;
			}
			return bmsScore.clear;
		}
	}

	public LR2ScoreDB.score.RankType rank
	{
		get
		{
			if (bmsScore != null)
			{
				if (bmsScore.rank != LR2ScoreDB.score.RankType.INVALID)
				{
					return bmsScore.rank;
				}
				return LR2ScoreDB.score.RankType.F;
			}
			return LR2ScoreDB.score.RankType.INVALID;
		}
	}
