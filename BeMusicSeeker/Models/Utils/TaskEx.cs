...
public static class TaskEx
{
    ...
    private static void log2file(Task x, string memberName, string filePath, int lineNumber)
    {
        ...
        if (x.Exception != null)
        {
            ...
            {
                DispatcherMessageBox.Show("操作の途中でファイルのアクセスに失敗しました。" + Environment.NewLine + "エラー概要:" + Environment.NewLine + string.Join(Environment.NewLine, x.Exception.Flatten().InnerExceptions.Select(e => e.Message)) + Environment.NewLine + Environment.NewLine + "プログラムを終了しファイルへのアクセスが可能か確認して下さい", "エラー", MessageBoxButton.OK, MessageBoxImage.Hand, MessageBoxResult.OK);
                return;
            }
        ...
        if (DispatcherMessageBox.Show("予期しないエラーが発生しました。" + Environment.NewLine + Environment.NewLine + "エラーの発生状況を開発者に送信してもよろしいでしょうか?" + Environment.NewLine + "送信される情報は発生箇所の特定に使用され、" + Environment.NewLine + "個人の情報は含まれません。" + Environment.NewLine + Environment.NewLine + "エラー概要:" + Environment.NewLine + string.Join(Environment.NewLine, x.Exception.Flatten().InnerExceptions.Select(e => e.Message)), "エラー", MessageBoxButton.YesNo, MessageBoxImage.Hand, MessageBoxResult.Yes) == MessageBoxResult.Yes)
        {
            logger = loggerPost;
        }
        try
