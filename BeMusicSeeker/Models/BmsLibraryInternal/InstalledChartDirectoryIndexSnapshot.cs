...
internal sealed class InstalledChartDirectoryIndexSnapshot
{
    ...
    public int DirectoryReferenceCount => Md5Directories.Sum((KeyValuePair<string, List<string>> x) => x.Value.Count)
        + Sha256Directories.Sum((KeyValuePair<string, List<string>> x) => x.Value.Count);
    ...
    {
        InstalledChartDirectoryIndexSnapshot snapshot = new InstalledChartDirectoryIndexSnapshot();
        foreach (KeyValuePair<string, List<string>> item in Md5Directories)
        {
            snapshot.Md5Directories[item.Key] = item.Value.ToList();
        }
        foreach (KeyValuePair<string, List<string>> item2 in Sha256Directories)
        {
            snapshot.Sha256Directories[item2.Key] = item2.Value.ToList();
        }
