...
internal sealed class BmsLibraryDbGateway(string songDbPath, string scoreDbPath = null)
{
    ...
    internal bool HasIncompleteLr2CompatibilityMaintenanceFacts()
    {
        using LR2SongDBExtended songDb = OpenSongDbReadOnly();
        return HasIncompleteLr2CompatibilityMaintenanceFacts(songDb);
    }

    internal static bool HasIncompleteLr2CompatibilityMaintenanceFacts(LR2SongDBExtended songDb)
    {
        if (songDb == null)
        {
            throw new ArgumentNullException(nameof(songDb));
        ...
        string bmsSongPredicate = CreateBmsPathPredicate("s." + songPathColumn);
        if (!TableExists(songDb, SQLiteTable<LR2SongDBExtended.maintenance>.GetTableName()))
        {
            return songDb.ExecuteScalar<int>(
                "SELECT EXISTS(SELECT 1 FROM " + songTableName + " s"
                + " WHERE " + bmsSongPredicate
                + " LIMIT 1);") != 0;
        }

        string maintenanceTableName = SQLiteTable<LR2SongDBExtended.maintenance>.GetTableName();
        string[] requiredColumns =
        [
            "lr2_path_warning_flags",
            "lr2_chart_path_cp932_bytes",
            "lr2_folder_scan_cp932_bytes",
            "lr2_resource_warning_flags",
            "lr2_resource_max_raw_cp932_bytes",
            "lr2_resource_max_resolved_cp932_bytes",
            "lr2_resource_unsupported_count"
        ];
        HashSet<string> columns = GetTableColumns(songDb, null, maintenanceTableName);
        ...
        string missingFactsPredicate = "m." + maintenancePathColumn + " IS NULL"
            + " OR m.lr2_path_warning_flags IS NULL"
            + " OR m.lr2_chart_path_cp932_bytes IS NULL"
            + " OR m.lr2_folder_scan_cp932_bytes IS NULL"
            + " OR m.lr2_resource_warning_flags IS NULL"
            + " OR m.lr2_resource_max_raw_cp932_bytes IS NULL"
            + " OR m.lr2_resource_max_resolved_cp932_bytes IS NULL"
            + " OR m.lr2_resource_unsupported_count IS NULL";
        return songDb.ExecuteScalar<int>(
            "SELECT EXISTS(SELECT 1"
            + " FROM " + songTableName + " s"
            + " LEFT JOIN " + maintenanceTableName + " m ON m." + maintenancePathColumn + " = s." + songPathColumn + " COLLATE NOCASE"
            + " WHERE " + bmsSongPredicate
            + " AND (" + missingFactsPredicate + ")"
            + " LIMIT 1);") != 0;
    }

    private static string CreateBmsPathPredicate(string pathExpression)
    {
        return pathExpression + " IS NOT NULL"
            + " AND (lower(" + pathExpression + ") LIKE '%.bms'"
            + " OR lower(" + pathExpression + ") LIKE '%.bme'"
            + " OR lower(" + pathExpression + ") LIKE '%.bml'"
            + " OR lower(" + pathExpression + ") LIKE '%.pms')";
    }
