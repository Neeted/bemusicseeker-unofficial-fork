...
internal static class LibraryChartRowSortEngine
{
    internal static List<LibraryChartRow> SortForMainView(IEnumerable<LibraryChartRow> source, MainWindowViewModel.cSortParameters sortParameters, bool isPlaylistDetailView, out string sortProfile)
    {
        ...
        if (property == null)
        {
            sortProfile = "library_chart_string_fallback";
            return SortByString(safeSource, _ => string.Empty, direction);
        }
        ...
        {
            sortProfile = "library_chart_legacy_string";
            return SortByString(safeSource, row => NormalizeSortKey(property.GetValue(row), property.PropertyType), direction);
        }
    ...
    private static List<LibraryChartRow> SortByString(IEnumerable<LibraryChartRow> source, Func<LibraryChartRow, string> keySelector, ListSortDirection direction)
    {
