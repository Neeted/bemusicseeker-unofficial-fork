...
16. デコンパイルで失われたローカル変数名のリファクタリング
    - 変数名が `list1, list2...` や `item1, item2...` といった無意味な命名になってしまっている箇所が多数あり、可読性が致命的（AIによるコード理解時にも支障が出る）なため整理が必要。
17. 多言語対応の強化
    - 元々備わっている多言語対応の仕組みだが、現状は日本語リソースのみが存在している。
    - DLLに埋め込む形式ではなく、テキストエディタで外部から容易に編集できる言語ファイル（Json等）の形式に変更できないか要検討。
18. 「リコメンド（自動更新）」を導入している状態でプレイリストの全リロード（起動時または手動実行）が行われると、リコメンド表の更新だけが終わらずにハングアップする問題の調査
