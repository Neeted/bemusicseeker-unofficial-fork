...
14. デコンパイルで失われたローカル変数名のリファクタリング
    - 変数名が `list1, list2...` や `item1, item2...` といった無意味な命名になってしまっている箇所が多数あり、可読性が致命的（AIによるコード理解時にも支障が出る）なため整理が必要。
15. 「リコメンド（自動更新）」を導入している状態でプレイリストの全リロード（起動時または手動実行）が行われると、リコメンド表の更新だけが終わらずにハングアップする問題の調査
